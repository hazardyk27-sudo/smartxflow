...
- ANALİZLER MODAL (renderAnalysisList): Manuel analist kartları kaldırıldı (Galway vb.). Modal artık SADECE 5 AUTO sinyal chip'i (Underdog Pressure ⚡ #d29922, Confirmed Money 💰 #34d399, Confirmed Money V2 💎 #6366f1, Early Money Lock 🔒 #58a6ff, Fake Sharp ⚠️ #f85149) + "Tümü" chip'i yatay sırada gösterir. `buildAnalystCards()` fonksiyonu kodda korundu (geriye dönük uyumluluk için) ama `renderAnalysisList` artık empty-state placeholder render eder. Canvas'ta 4 redesign varyantı var: `artifacts/mockup-sandbox/src/components/mockups/analyzer-modal/{Hub,PillRail,Magazine,Spectrum}.tsx`.
