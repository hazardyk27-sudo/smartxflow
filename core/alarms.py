...
BIG_MONEY_TOTAL_THRESHOLD = 3000
BIG_MONEY_SIDE_THRESHOLD = 1500
MOMENTUM_CONSECUTIVE_COUNT = 3
LINE_FREEZE_THRESHOLD = 0.02
...
def analyze_match_alarms(history: List[Dict], market: str) -> List[Dict]:
    ...
    for sc in side_changes:
        if sc['money_diff'] <= 100:
            continue
            
        money_up = sc['money_diff'] > 100
        odds_down = sc['odds_diff'] < -0.02
        odds_up = sc['odds_diff'] > 0.02
    ...
    if total_diff >= BIG_MONEY_TOTAL_THRESHOLD or max_side_diff >= BIG_MONEY_SIDE_THRESHOLD:
    ...
    for alarm in detected_alarms:
        if alarm['type'] not in seen_types:
            seen_types.add(alarm['type'])
            unique_alarms.append(alarm)
        if len(unique_alarms) >= 3:
            break
...
def check_momentum(history: List[Dict], sides: List[Dict]) -> Optional[Dict]:
    for side in sides:
        ...
        if consecutive_increase >= MOMENTUM_CONSECUTIVE_COUNT:
...
def check_line_freeze(history: List[Dict], sides: List[Dict]) -> Optional[Dict]:
    for side in sides:
        ...
        for i in range(1, len(history)):
            ...
            if first_odds > 0 and abs(curr_odds - first_odds) > LINE_FREEZE_THRESHOLD:
                odds_stable = False
                break
        
        if odds_stable and total_money >= BIG_MONEY_SIDE_THRESHOLD:
...
def format_alarm_for_modal(alarm: Dict) -> Dict:
    ...
    if alarm['type'] == 'sharp':
        ...
        detail_text = f"Para arttı, oran yükseldi."
    elif alarm['type'] == 'big_money':
        detail_text = f"+£{int(money_diff):,} (son 5 dk)"
    elif alarm['type'] == 'public_surge':
        detail_text = f"Para artıyor, oran sabit."
