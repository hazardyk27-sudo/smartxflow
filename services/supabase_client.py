...
def fetch_alarms_from_supabase(table_name: str, order_by: str = 'created_at', limit: int = 500, filter_recent: bool = True) -> Optional[List[Dict[str, Any]]]:
    ...
    Args:
        filter_recent: If True, only return alarms triggered in last 12 hours
    """
    ...
    try:
        # Build URL with optional recent filter
        url = f"{client._rest_url(table_name)}?select=*&order={order_by}.desc&limit={limit}"
        
        # Filter to only show recent alarms (last 12 hours) - prevents finished matches like Auckland
        if filter_recent:
            from datetime import datetime, timedelta
            import pytz
            turkey_tz = pytz.timezone('Europe/Istanbul')
            cutoff = datetime.now(turkey_tz) - timedelta(hours=12)
            cutoff_iso = cutoff.strftime('%Y-%m-%dT%H:%M:%S')
            # trigger_at format is ISO - filter with gte
            url += f"&trigger_at=gte.{cutoff_iso}"
...
def write_volumeshock_alarms_to_supabase(alarms: List[Dict[str, Any]]) -> bool:
    ...
    for alarm in alarms:
        mapped = {
            ...
            'incoming_money': alarm.get('incoming_money') or alarm.get('volume') or alarm.get('stake'),
