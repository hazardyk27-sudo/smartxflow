...
def compute_result_distribution(similar_matches):
    ...
    simple = {
        ...
        "surpriz": round(safe_div(sur_count, total_with_result) * 100, 1) if total_with_result > 0 else 0,
...
def compute_ou25_distribution(similar_matches):
    ...
    simple = {
        "over": round(safe_div(over_count, total) * 100, 1) if total > 0 else 0,
        "under": round(safe_div(under_count, total) * 100, 1) if total > 0 else 0,
        "total": total,
    }
    weighted = {"over": 0, "under": 0}
    if weight_sum > 0:
...
def compute_btts_distribution(similar_matches):
    ...
    simple = {
        "yes": round(safe_div(yes_count, total) * 100, 1) if total > 0 else 0,
        "no": round(safe_div(no_count, total) * 100, 1) if total > 0 else 0,
        "total": total,
    }
    weighted = {"yes": 0, "no": 0}
    if weight_sum > 0:
