...
**Why:** these are non-obvious API quirks (silent pagination cap, unreliable startDate, title-based real-match filtering, Yes/No-only market structure) discovered through trial and error; getting them wrong silently drops most events, returns futures markets mixed with real matches, or causes users to think market data is missing when it's just mislabeled.
