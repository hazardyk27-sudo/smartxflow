...
- Over/Under and BTTS submarkets for a match live on a *separate sibling event* whose slug is `<main-slug>-more-markets`, not on the main event. Fetch it as its own Gamma API event lookup. These binary markets also expose no per-outcome volume field, unlike the main 1X2 markets — approximate per-side volume by summing sampled trades' `usdcSize` per outcome instead.
