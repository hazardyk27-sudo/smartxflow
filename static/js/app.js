...
async function loadLiveMatches(){try{var resp=await fetch('/api/live/matches');var data=await resp.json();_liveData=data.matches||[];var _goalHashes={};for(var gi=0;gi<_liveData.length;gi++){var gm=_liveData[gi];var gh=gm.match_id_hash;var gs=gm.score||'';if(gh&&gs&&_prevLiveScores[gh]&&_prevLiveScores[gh]!==gs){_goalHashes[gh]=true;}
if(gh&&gs)_prevLiveScores[gh]=gs;}
renderLiveMatches(_liveData,_goalHashes);var now=new Date();var lut=document.getElementById('liveUpdateTime');if(lut)lut.textContent=String(now.getHours()).padStart(2,'0')+':'+
...
var sorted=filtered.slice();sorted.sort(function(a,b){var va=is1x2?_liveCalcVol1x2(a):_liveCalcVolOU(_liveGetOu(a,ouLine));var vb=is1x2?_liveCalcVol1x2(b):_liveCalcVolOU(_liveGetOu(b,ouLine));return vb-va;});var rowsHtml='';var cardsHtml='';for(var i=0;i<sorted.length;i++){var m=sorted[i];var o=m.odds||{};var matchMin=_calcLiveMatchMin(m);var scoreStr=m.score?escLiveHtml(m.score):'';var onclickAttr='onclick="openLiveDetail(\''+m.match_id_hash+'\',\''+escLiveHtml(m.home_team)+'\',\''+escLiveHtml(m.away_team)+'\')"';var showScore=(matchMin==='MS')?(scoreStr||'-'):scoreStr;var isGoal=!!(m.match_id_hash&&goalHashes[m.match_id_hash]);var scoreCls='time-line live-score-badge'+(isGoal?' goal-flash':'');var minCell='<td class="match-date">'+'<div class="date-line live-min-badge">'+escLiveHtml(matchMin)+'</div>'+
(showScore?'<div class="'+scoreCls+'">'+showScore+'</div>':'')+'</td>';if(is1x2){var vol=_liveCalcVol1x2(m);var b1=_liveMwBlock('1',o['1']);var bX=_liveMwBlock('X',o['X']);var b2=_liveMwBlock('2',o['2']);rowsHtml+='<tr '+onclickAttr+'>'+
minCell+'<td class="match-league" title="'+escLiveHtml(m.league)+'">'+escLiveHtml(m.league)+'</td>'+'<td class="match-teams">'+escLiveHtml(m.home_team)+'<span class="vs">-</span>'+escLiveHtml(m.away_team)+'</td>'+'<td class="mw-outcomes-cell" colspan="3"><div class="mw-grid mw-grid-3">'+b1+bX+b2+'</div></td>'+'<td class="volume-cell">'+formatLiveVol(vol)+'</td></tr>';cardsHtml+=_liveMobileCard(m,'1x2',onclickAttr,isGoal);}else{var ou=_liveGetOu(m,ouLine);var volOU=_liveCalcVolOU(ou);var bU=_liveMwBlock('Alt',ou['U']);var bO=_liveMwBlock('Ust',ou['O']);rowsHtml+='<tr '+onclickAttr+'>'+
minCell+'<td class="match-league" title="'+escLiveHtml(m.league)+'">'+escLiveHtml(m.league)+'</td>'+'<td class="match-teams">'+escLiveHtml(m.home_team)+'<span class="vs">-</span>'+escLiveHtml(m.away_team)+'</td>'+'<td class="mw-outcomes-cell" colspan="2"><div class="mw-grid mw-grid-2">'+bU+bO+'</div></td>'+'<td class="volume-cell">'+formatLiveVol(volOU)+'</td></tr>';cardsHtml+=_liveMobileCard(m,ouLine,onclickAttr,isGoal);}}
if(tbody)tbody.innerHTML=rowsHtml;if(cardList)cardList.innerHTML=cardsHtml;document.querySelectorAll('.goal-flash').forEach(function(el){el.addEventListener('animationend',function(){el.classList.remove('goal-flash');},{once:true});});document.querySelectorAll('.goal-flash-card').forEach(function(el){el.addEventListener('animationend',function(){el.classList.remove('goal-flash-card');},{once:true});});var mc=document.getElementById('matchCount');if(mc)mc.textContent=sorted.length;}
...
function _liveMobileCard(m,marketOrLine,onclickAttr,isGoal){var is1x2=(marketOrLine==='1x2');var cardCls='match-card odds-card moneyway-card'+(isGoal?' goal-flash-card':'');var html='<div class="'+cardCls+'" '+onclickAttr+'>';html+='<div class="odds-card-header">';html+='<div class="odds-card-teams">'+escLiveHtml(m.home_team)+' \u2013 '+escLiveHtml(m.away_team)+'</div>';var vol=0;if(is1x2){vol=_liveCalcVol1x2(m);}else{var ou=_liveGetOu(m,marketOrLine);vol=_liveCalcVolOU(ou);}
