...
function renderChartLegendFilters(datasets,market){const container=document.getElementById('chartLegendFilters');if(!container)return;const legendLabels={'moneyway_1x2':[{key:'1',label:'1',color:'#3b82f6'},{key:'X',label:'X',color:'#22c55e'},{key:'2',label:'2',color:'#eab308'}],'moneyway_ou25':[{key:'Under',label:_t('app.dyn.alt','Alt')+' 2.5',color:'#3b82f6'},{key:'Over',label:_t('app.dyn.ust','Üst')+' 2.5',color:'#22c55e'}],'moneyway_btts':[{key:'Yes',label:_t('app.j.btts','KG')+' '+_t('app.dyn.evet','Evet'),color:'#22c55e'},{key:'No',label:_t('app.j.btts','KG')+' '+_t('app.dyn.hayir','Hayır'),color:'#ef4444'}],'dropping_1x2':[{key:'1',label:'1',color:'#3b82f6'},{key:'X',label:'X',color:'#22c55e'},{key:'2',label:'2',color:'#eab308'}],'dropping_ou25':[{key:'Under',label:_t('app.dyn.alt','Alt')+' 2.5',color:'#3b82f6'},{key:'Over',label:_t('app.dyn.ust','Üst')+' 2.5',color:'#22c55e'}],'dropping_btts':[{key:'Yes',label:_t('app.j.btts','KG')+' '+_t('app.dyn.evet','Evet'),color:'#22c55e'},{key:'No',label:_t('app.j.btts','KG')+' '+_t('app.dyn.hayir','Hayır'),color:'#ef4444'}]};const items=legendLabels[market]||[];const timeRanges=[{key:'10min',label:_t('app.dyn.10_dakika','10 dk')},{key:'30min',label:_t('app.dyn.30_dakika','30 dk')},{key:'1hour',label:_t('app.dyn.1_saat','1 saat')},{key:'6hour',label:_t('app.dyn.6_saat','6 saat')},{key:'12hour',label:_t('app.dyn.12_saat','12 saat')},{key:'1day',label:_t('app.dyn.1_gun','1 gün')}];const seriesButtons=items.map(item=>{const stateKey=`${market}_${item.key}`;const isActive=chartVisibleSeries[stateKey]!==false;const activeClass=isActive?'active':'inactive';return`
