...
async function openMatchModalFromAPI(homeTeam,awayTeam,league,kickoff){const reqId=++_modalRequestId;resetModalState();league=league||'';kickoff=kickoff||'';var apiHome=homeTeam;var apiAway=awayTeam;var apiLeague=league;var apiKickoff=kickoff;var prematchFound=null;var hLow=(homeTeam||'').toLowerCase().trim();var aLow=(awayTeam||'').toLowerCase().trim();var searchSrc=(typeof filteredMatches!=='undefined'&&filteredMatches.length>0)?filteredMatches:((typeof matches!=='undefined'&&matches.length>0)?matches:[]);for(var si=0;si<searchSrc.length;si++){var pmH=(searchSrc[si].home_team||'').toLowerCase().trim();var pmA=(searchSrc[si].away_team||'').toLowerCase().trim();if(pmH===hLow&&pmA===aLow){prematchFound=searchSrc[si];break;}
if((pmH.includes(hLow)||hLow.includes(pmH))&&(pmA.includes(aLow)||aLow.includes(pmA))&&pmH.length>2&&pmA.length>2){if(!prematchFound)prematchFound=searchSrc[si];}}
if(prematchFound){apiHome=prematchFound.home_team||homeTeam;apiAway=prematchFound.away_team||awayTeam;if(!league&&prematchFound.league)apiLeague=prematchFound.league;if(!kickoff&&prematchFound.date)apiKickoff=prematchFound.date;console.log('[ModalFromAPI] Prematch found:',apiHome,'vs',apiAway,'league:',apiLeague);}else{console.log('[ModalFromAPI] No prematch match for',homeTeam,'vs',awayTeam,'searched',searchSrc.length);}
...
async function _openLiveInMainModal(hash,home,away){var match=null;var matchIdx=-1;var dataSource=(typeof filteredMatches!=='undefined'&&filteredMatches.length>0)?filteredMatches:((typeof matches!=='undefined')?matches:[]);var homeLow=(home||'').toLowerCase().trim();var awayLow=(away||'').toLowerCase().trim();console.log('[LiveModal] Searching',homeLow,'vs',awayLow,'in',dataSource.length,'matches');for(var i=0;i<dataSource.length;i++){var mh=(dataSource[i].home_team||'').toLowerCase().trim();var ma=(dataSource[i].away_team||'').toLowerCase().trim();if(mh===homeLow&&ma===awayLow){match=dataSource[i];matchIdx=i;break;}
if((mh.includes(homeLow)||homeLow.includes(mh))&&(ma.includes(awayLow)||awayLow.includes(ma))){if(!match){match=dataSource[i];matchIdx=i;console.log('[LiveModal] Fuzzy matched:',mh,'vs',ma,'at index',i);}}}
if(match&&matchIdx>=0){console.log('[LiveModal] Found prematch match, opening with openMatchModal idx=',matchIdx);await openMatchModal(matchIdx);}else{console.log('[LiveModal] No prematch match found, using openMatchModalFromAPI');await openMatchModalFromAPI(home,away,'','');}
