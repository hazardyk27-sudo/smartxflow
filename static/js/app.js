...
const blockCount=currentMarket.includes('1x2')?'three':'two';const matchStatus=getMatchStatus(match.date);const liveCapsule=getMatchLiveCapsule(match.date,match.home_team,match.away_team,match.match_id);const _mk=_getMatchKey(match);const _isFav=_userFavorites.has(_mk);const _fc=_favCounts[_mk]||0;const _mobLockSvg='<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#4a5068" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>';const _mobLockCell='<div class="test-lock-cell test-lock-mob">'+_mobLockSvg+'</div>';const _mwGrayRow=_mTestBlur?(_mobLockCell+_mobLockCell+(blockCount==='three'?_mobLockCell:'')):oddsBlocks;return`
    ...
    `;}
function buildCardInsight(d,market){var parts=[];if(market.includes('1x2')){var p1=parseFloat(String(d.PctHome||d.Pct1||d.PctYes||'0').replace('%',''));var pX=parseFloat(String(d.PctDraw||d.PctX||'0').replace('%',''));var p2=parseFloat(String(d.PctAway||d.Pct2||d.PctNo||'0').replace('%',''));if(p1>0||pX>0||p2>0){var best=Math.max(p1,pX,p2);var leader=best===p1?'1':(best===pX?'X':'2');parts.push('<span class="insight-tag">'+_t('app.j.fav','Favori')+': <span class="insight-val">'+leader+'</span></span>');parts.push('<span class="insight-tag">'+_t('app.j.share','Pay')+': <span class="insight-val positive">'+best.toFixed(0)+'%</span></span>');var spread=(best-Math.min(p1>0?p1:999,pX>0?pX:999,p2>0?p2:999)).toFixed(0);var label=parseInt(spread)>30?_t('app.j.strong_fav','Güçlü favori'):(parseInt(spread)>10?_t('app.j.open_fav','Favori açık'):_t('app.j.balanced','Dengeli'));parts.push('<span class="insight-label">'+label+'</span>');}}else if(market.includes('2.5')){var pOver=parseFloat(String(d.PctYes||d.PctOver||'0').replace('%',''));var pUnder=parseFloat(String(d.PctNo||d.PctUnder||'0').replace('%',''));if(pOver>0||pUnder>0){var lbl=pOver>pUnder?'Üst':'Alt';var pct=Math.max(pOver,pUnder);parts.push('<span class="insight-tag">'+_t('app.j.leader','Lider')+': <span class="insight-val">'+lbl+'</span></span>');parts.push('<span class="insight-tag">'+_t('app.j.share','Pay')+': <span class="insight-val positive">'+pct.toFixed(0)+'%</span></span>');}}else if(market.includes('kg')){var pY=parseFloat(String(d.PctYes||'0').replace('%',''));var pN=parseFloat(String(d.PctNo||'0').replace('%',''));if(pY>0||pN>0){var lbl=pY>pN?'Evet':'Hayır';var pct=Math.max(pY,pN);parts.push('<span class="insight-tag">'+_t('app.j.btts','KG')+': <span class="insight-val">'+lbl+'</span></span>');parts.push('<span class="insight-tag">'+_t('app.j.share','Pay')+': <span class="insight-val positive">'+pct.toFixed(0)+'%</span></span>');}}
...
function formatDateTwoLine(dateStr){if(!dateStr||dateStr==='-')return'<div class="date-line">-</div>';const dt=toTurkeyTime(dateStr);if(dt&&dt.isValid()){const months=['Oca','Şub','Mar','Nis','May','Haz','Tem','Ağu','Eyl','Eki','Kas','Ara'];const day=dt.date();const month=months[dt.month()];const time=dt.format('HH:mm');return`<div class="date-line">${day}.${month}</div><div class="time-line">${time}</div>`;}
...
function setMobileTimeRange(range){mobileTimeRange=range;const rangeMap={'10':'10min','30':'30min','60':'1hour','360':'6hour','720':'12hour','1440':'1day','all':'1day'};chartTimeRange=rangeMap[range]||'1day';const labels={'all':'Tümü','10':'10 dk','30':'30 dk','60':'1 saat','360':'6 saat','720':'12 saat','1440':_t('app.j.tf_1day','1 gün')};const mobTimeLabel=document.getElementById('mobTimeLabel');if(mobTimeLabel)mobTimeLabel.textContent=labels[range]||'Tümü';document.querySelectorAll('.mob-time-option').forEach(opt=>{opt.classList.toggle('active',opt.dataset.range===range);});const mobTimeMenu=document.getElementById('mobTimeMenu');if(mobTimeMenu)mobTimeMenu.classList.remove('open');document.querySelectorAll('.time-pill').forEach(pill=>{pill.classList.toggle('active',pill.dataset.range===range);});if(selectedMatch&&isMobile()){loadChartHistory(selectedMatch.MatchId,selectedChartMarket);}}
...
function updateMobileSelectionButtons(market){const toggle=document.getElementById('mobSelectionToggle');if(!toggle)return;let buttons=[];let validSelections=[];if(market.includes('1x2')){buttons=[{sel:'1',label:'1'},{sel:'X',label:'X'},{sel:'2',label:'2'}];validSelections=['1','X','2'];}else if(market.includes('ou25')){buttons=[{sel:'Under',label:'Alt'},{sel:'Over',label:'Üst'}];validSelections=['Under','Over'];}else if(market.includes('btts')){buttons=[{sel:'Yes',label:'Evet'},{sel:'No',label:'Hayır'}];validSelections=['Yes','No'];}
...
function renderChartLegendFilters(datasets,market){const container=document.getElementById('chartLegendFilters');if(!container)return;const legendLabels={'moneyway_1x2':[{key:'1',label:'1',color:'#3b82f6'},{key:'X',label:'X',color:'#22c55e'},{key:'2',label:'2',color:'#eab308'}],'moneyway_ou25':[{key:'Under',label:'Alt 2.5',color:'#3b82f6'},{key:'Over',label:'Üst 2.5',color:'#22c55e'}],'moneyway_btts':[{key:'Yes',label:'KG Evet',color:'#22c55e'},{key:'No',label:'KG Hayır',color:'#ef4444'}],'dropping_1x2':[{key:'1',label:'1',color:'#3b82f6'},{key:'X',label:'X',color:'#22c55e'},{key:'2',label:'2',color:'#eab308'}],'dropping_ou25':[{key:'Under',label:'Alt 2.5',color:'#3b82f6'},{key:'Over',label:'Üst 2.5',color:'#22c55e'}],'dropping_btts':[{key:'Yes',label:'KG Evet',color:'#22c55e'},{key:'No',label:'KG Hayır',color:'#ef4444'}]};const items=legendLabels[market]||[];const timeRanges=[{key:'10min',label:'10 dk'},{key:'30min',label:'30 dk'},{key:'1hour',label:'1 saat'},{key:'6hour',label:'6 saat'},{key:'12hour',label:'12 saat'},{key:'1day',label:_t('app.j.tf_1day','1 gün')}];const seriesButtons=items.map(item=>{const stateKey=`${market}_${item.key}`;const isActive=chartVisibleSeries[stateKey]!==false;const activeClass=isActive?'active':'inactive';return`
...
return dateStr;}
function formatMatchDateShort(dateStr){if(!dateStr)return'';const monthNames=['Oca','Şub','Mar','Nis','May','Haz','Tem','Ağu','Eyl','Eki','Kas','Ara'];const monthMap={'Jan':0,'Feb':1,'Mar':2,'Apr':3,'May':4,'Jun':5,'Jul':6,'Aug':7,'Sep':8,'Oct':9,'Nov':10,'Dec':11};const match1=dateStr.match(/^(\d{2})\.([A-Za-z]{3})\s+(\d{2}):(\d{2}):?(\d{2})?$/);if(match1){const[,day,monthStr,hour,min]=match1;const monthIdx=monthMap[monthStr]??0;const utcDate=dayjs.utc(`2025-${String(monthIdx + 1).padStart(2, '0')}-${day}T${hour}:${min}:00`);const trDate=utcDate.tz('Europe/Istanbul');const monthName=monthNames[trDate.month()];return`${trDate.format('DD')} ${monthName} ${trDate.format('HH:mm')}`;}
...
let currentAlarmDateFilter='all';const dateFilterLabels={all:'Tümü',today:'Bugün',yesterday:'Dün',future:'Gelecek'};const dateFilterColors={all:'#ffffff',today:'#4ade80',yesterday:'#f85149',future:'#3B82F6'};function toggleDateFilterDropdown(){const dropdown=document.getElementById('alarmDateDropdown');const options=document.getElementById('alarmDateOptions');if(dropdown&&options){dropdown.classList.toggle('open');options.style.display=options.style.display==='block'?'none':'block';}}
function selectDateFilter(dateFilter){console.log('[selectDateFilter] Date filter changed to:',dateFilter);currentAlarmDateFilter=dateFilter;const labelEl=document.getElementById('dateLabelDisplay');if(labelEl){labelEl.textContent=dateFilterLabels[dateFilter]||'Tümü';}
...
function formatTriggerTimeFull(dateStr){if(!dateStr)return'-';const monthNames=['Oca','Şub','Mar','Nis','May','Haz','Tem','Ağu','Eyl','Eki','Kas','Ara'];if(dateStr.includes('T')&&(dateStr.includes('+')||dateStr.includes('Z'))){const dt=dayjs(dateStr).tz(APP_TIMEZONE);if(dt&&dt.isValid()){const day=dt.format('DD');const month=monthNames[dt.month()];const time=dt.format('HH:mm');return`${day} ${month} • ${time}`;}}
...
function formatSmartMoneyTime(dateStr){if(!dateStr)return'-';const monthNames=['Oca','Şub','Mar','Nis','May','Haz','Tem','Ağu','Eyl','Eki','Kas','Ara'];const match1=dateStr.match(/^(\d{2})\.(\d{2})\.(\d{4})\s+(\d{2}):(\d{2})$/);if(match1){const[,day,month,year,hour,min]=match1;const dt=new Date(parseInt(year),parseInt(month)-1,parseInt(day),parseInt(hour),parseInt(min));dt.setHours(dt.getHours()+3);const monthName=monthNames[dt.getMonth()];const h=String(dt.getHours()).padStart(2,'0');const m=String(dt.getMinutes()).padStart(2,'0');const d=String(dt.getDate()).padStart(2,'0');return`${d} ${monthName} • ${h}:${m}`;}
...
if(_prevCount===0)console.log('[GOL] İlk yükleme - '+Object.keys(_prevLiveScores).length+' maç kaydedildi');renderLiveMatches(_liveData,_goalHashes);var now=new Date();var lut=document.getElementById('liveUpdateTime');if(lut)lut.textContent=String(now.getHours()).padStart(2,'0')+':'+
