...
currentChartHistoryData=[];modalOddsData=null;previousOddsData=null;bulkHistoryCache={};bulkHistoryCacheKey='';const card=document.getElementById('matchInfoCard');if(card)card.innerHTML='';const grid=document.getElementById('smartMoneyGrid');if(grid){grid.innerHTML='';grid.style.display='none';}
...
document.getElementById('mobileTabLiveBtn').style.display='none';_liveDetailData=null;_liveDetailHash=null;var liveBody=document.getElementById('liveDetailBody');if(liveBody)liveBody.innerHTML='';var liveTabs=document.getElementById('liveDetailTabs');if(liveTabs)liveTabs.innerHTML='';showSmartMoneyLoading();showChartLoading();}
...
function switchMobileTab(tabName){document.querySelectorAll('.mobile-tab-bar .mob-tab').forEach(t=>{t.classList.remove('active');if(t.dataset.tab===tabName)t.classList.add('active');});document.querySelectorAll('.mobile-tab-content').forEach(c=>{c.classList.remove('active');});const targetTab=document.getElementById('mobileTab'+tabName.charAt(0).toUpperCase()+tabName.slice(1));if(targetTab)targetTab.classList.add('active');if(tabName==='chart'&&selectedMatch&&!modalOddsData){var home=selectedMatch.home_team;var away=selectedMatch.away_team;var league=selectedMatch.league||'';var reqId=_modalRequestId;var chartLibsPromise=window.loadChartLibs?window.loadChartLibs().then(function(){registerChartPlugins();}):Promise.resolve();var marketsPromise=loadAllMarketsAtOnce(home,away,league);Promise.all([marketsPromise,chartLibsPromise]).then(function(){if(reqId!==_modalRequestId)return;return loadChartWithTrends(home,away,selectedChartMarket||currentMarket,league,reqId);}).catch(function(e){console.error('[Modal] Chart pipeline error:',e);});}
if(tabName==='events'&&selectedMatch){var grid=document.getElementById('smartMoneyGrid');if(grid&&grid.innerHTML===''){renderMatchAlarmsSection(selectedMatch.home_team,selectedMatch.away_team,_modalRequestId);}}}
...
document.addEventListener('click',function(e){const dropdown=document.querySelector('.mob-time-dropdown');const menu=document.getElementById('mobTimeMenu');if(dropdown&&menu&&!dropdown.contains(e.target)){menu.classList.remove('open');}});function closeModal(){++_modalRequestId;document.getElementById('modalOverlay').classList.remove('active');document.getElementById('mobileTabLiveBtn').style.display='none';_liveDetailData=null;_liveDetailHash=null;var liveBody=document.getElementById('liveDetailBody');if(liveBody)liveBody.innerHTML='';var liveTabs=document.getElementById('liveDetailTabs');if(liveTabs)liveTabs.innerHTML='';switchMobileTab('chart');const chartTooltip=document.getElementById('chartjs-tooltip');if(chartTooltip){chartTooltip.style.opacity=0;chartTooltip.style.visibility='hidden';chartTooltip.style.display='none';}
