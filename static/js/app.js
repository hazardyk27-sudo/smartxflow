...
function _liveMobileBlock(label,d,totalVol){var odds=(d&&d.odds)?d.odds.toFixed(2):'-';var pct='';var moneyStr='';if(d&&d.share!==null&&d.share!==undefined){var pn=parseFloat(String(d.share));if(!isNaN(pn)){pct=pn.toFixed(0)+'%';if(totalVol&&totalVol>0){var bm=totalVol*pn/100;moneyStr=bm>=1000000?'£'+(bm/1000000).toFixed(1)+'M':bm>=1000?'£'+(bm/1000).toFixed(1)+'k':'£'+Math.round(bm);}}}
return'<div class="odds-block mw-block">'+'<div class="odds-block-label">'+label+'</div>'+'<div class="ob-data-row">'+'<span class="ob-odds">'+odds+'</span>'+
(moneyStr?'<span class="ob-money">'+moneyStr+'</span>':'')+
(pct?'<span class="ob-pct">'+pct+'</span>':'')+'</div>'+'</div>';}
