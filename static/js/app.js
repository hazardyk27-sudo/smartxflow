...
async function loadAllMarketsAtOnce(home,away,league=''){const cacheKey=`${home}|${away}|${league}`;if(bulkHistoryCacheKey===cacheKey&&Object.keys(bulkHistoryCache).length>0){console.log('[Bulk] Using cached data for',cacheKey);return bulkHistoryCache;}
try{console.log('[Bulk] Fetching all markets for',home,'vs',away,'league:',league);const startTime=performance.now();const response=await fetch(`/api/match/history/bulk?home=${encodeURIComponent(home)}&away=${encodeURIComponent(away)}&league=${encodeURIComponent(league || '')}`);const data=await response.json();const elapsed=performance.now()-startTime;console.log(`[Bulk] Loaded all 6 markets in ${elapsed.toFixed(0)}ms`);if(data.markets){bulkHistoryCache=data.markets;bulkHistoryCacheKey=cacheKey;}
