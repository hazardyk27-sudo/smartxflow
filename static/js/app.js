...
var scoreStr=s.score||'-';var dataGroup=is1x2?(s['1x2']||{}):((s.ou_lines||{})[ouLine]||{});var prevDataGroup=prevS?(is1x2?(prevS['1x2']||{}):((prevS.ou_lines||{})[ouLine]||{})):null;html+='<div class="mlive-period">';html+='<div class="mlive-status"><span class="mlive-min">'+escLiveHtml(minStr)+'</span><span class="mlive-score">'+escLiveHtml(scoreStr)+'</span></div>';html+='<div class="mlive-cards">';for(var j=0;j<sels.length;j++){var sel=sels[j];var sd=dataGroup[sel]||{};var odds=(sd.odds!==undefined&&sd.odds!==null)?parseFloat(sd.odds):null;var pct=sd.share||0;var vol=sd.volume||0;var oddsStr=odds!==null?odds.toFixed(2):'-';var pctStr=pct?pct.toFixed(1)+'%':'-';var changes=[];if(prevDataGroup&&odds!==null){var prevOdds=(prevDataGroup[sel]||{}).odds;if(prevOdds!==undefined&&prevOdds!==null){var delta=odds-parseFloat(prevOdds);if(Math.abs(delta)>=0.005){var dSign=delta>0?'+':'';var dCls=delta>0?'ch-up':'ch-down';changes.push('<span class="mlive-pill '+dCls+'">Odds '+dSign+delta.toFixed(2)+'</span>');}}}
var prevVol=0;if(prevDataGroup){prevVol=(prevDataGroup[sel]||{}).volume||0;}
var volDiff=vol-prevVol;if(prevS&&volDiff!==0){var vCls=volDiff>0?'ch-up':'ch-down';changes.push('<span class="mlive-pill '+vCls+'">Money '+formatLiveVol(volDiff)+'</span>');}
var prevPct=0;if(prevDataGroup){prevPct=(prevDataGroup[sel]||{}).share||0;}
var pctDelta=pct-prevPct;if(prevS&&Math.abs(pctDelta)>=0.05){var pdSign=pctDelta>0?'+':'';var pCls=pctDelta>0?'ch-up':'ch-down';changes.push('<span class="mlive-pill '+pCls+'">Share '+pdSign+pctDelta.toFixed(1)+'%</span>');}
html+='<div class="mlive-card">';html+='<div class="mlive-card-lbl">'+selLabels[j]+'</div>';html+='<div class="mlive-card-odds">'+oddsStr+'</div>';html+='<div class="mlive-card-pct">'+pctStr+'</div>';html+='<div class="mlive-card-vol">'+(vol?formatLiveVol(vol):'-')+'</div>';if(changes.length>0){html+='<div class="mlive-card-changes">'+changes.join('')+'</div>';}
html+='</div>';}
html+='</div></div>';}
