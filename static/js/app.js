...
if(selectedMatch){showChartLoading();loadChartWithTrends(selectedMatch.home_team,selectedMatch.away_team,selectedChartMarket,selectedMatch.league||'');var arbH=selectedMatch.arbworld_hash||selectedMatch.match_id||'';loadMatchbookChartForModal(selectedMatch.home_team,selectedMatch.away_team,selectedMatch.league||'',arbH,selectedChartMarket);}});});}
...
const _matchesMarketCache={};const _MATCHES_CACHE_TTL=90000;async function loadMatches(appendMode=false){if(_liveMode)return;if(_loadMatchesLock){if(_loadMatchesPending){return _loadMatchesPending;}
return;}
...
var leagueHtml=escapeHtml(selectedMatch.league||'');if(isFinishedMatch)leagueHtml+=modalFtCapsule;else if(headerDateText)leagueHtml+=' • '+headerDateText;document.getElementById('modalLeague').innerHTML=leagueHtml;document.querySelectorAll('#modalChartTabs .chart-tab').forEach(t=>{t.classList.remove('active');if(t.dataset.market===currentMarket){t.classList.add('active');}});document.getElementById('modalOverlay').classList.add('active');const home=selectedMatch.home_team;const away=selectedMatch.away_team;const league=selectedMatch.league||'';const chartLibsPromise=window.loadChartLibs?window.loadChartLibs().then(()=>registerChartPlugins()):Promise.resolve();const marketsPromise=loadAllMarketsAtOnce(home,away,league);const chartPipeline=Promise.all([marketsPromise,chartLibsPromise]).then(()=>{if(reqId!==_modalRequestId)return;return loadChartWithTrends(home,away,selectedChartMarket,league,reqId);}).catch(e=>console.error('[Modal] Chart pipeline error:',e));const alarmsPipeline=renderMatchAlarmsSection(home,away,reqId);const livePipeline=_checkModalLiveData(home,away,reqId);var arbHash3=selectedMatch.arbworld_hash||selectedMatch.match_id||'';const mbPipeline3=loadMatchbookChartForModal(home,away,league,arbHash3,selectedChartMarket,reqId);await Promise.all([chartPipeline,alarmsPipeline,livePipeline,mbPipeline3]);}}
