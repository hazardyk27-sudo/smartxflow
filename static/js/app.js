...
var sorted=filtered.slice();sorted.sort(function(a,b){var va=is1x2?_liveCalcVol1x2(a):_liveCalcVolOU(_liveGetOu(a,ouLine));var vb=is1x2?_liveCalcVol1x2(b):_liveCalcVolOU(_liveGetOu(b,ouLine));return vb-va;});var rowsHtml='';var cardsHtml='';for(var i=0;i<sorted.length;i++){var m=sorted[i];var o=m.odds||{};var matchMin=_calcLiveMatchMin(m);var scoreStr=m.score?escLiveHtml(m.score):'';var onclickAttr='onclick="openLiveDetail(\''+m.match_id_hash+'\',\''+escLiveHtml(m.home_team)+'\',\''+escLiveHtml(m.away_team)+'\')"';var minCell='<td class="match-date">'+'<div class="date-line live-min-badge">'+escLiveHtml(matchMin)+'</div>'+
...
function _calcLiveMatchMin(m){var rawMin=(m.minute||'').trim();var markers=['HT','FT','1H','2H','ET','PEN'];for(var k=0;k<markers.length;k++){if(rawMin.toUpperCase()===markers[k])return markers[k];}
if(!m.kickoff_utc)return escLiveHtml(rawMin);try{var ko=new Date(m.kickoff_utc);if(isNaN(ko.getTime()))return escLiveHtml(rawMin);var now=new Date();var diff=Math.max(0,Math.floor((now-ko)/60000));if(diff>130)return escLiveHtml(rawMin);return diff+"'";}catch(e){return escLiveHtml(rawMin);}}
