...
async function _openLiveInMainModal(hash,home,away){var match=null;if(typeof allMatches!=='undefined'&&allMatches.length>0){for(var i=0;i<allMatches.length;i++){var mh=allMatches[i].home_team||'';var ma=allMatches[i].away_team||'';if(mh===home&&ma===away){match=allMatches[i];break;}}}
if(match){var idx=allMatches.indexOf(match);await openMatchModal(idx);}else{await openMatchModalFromAPI(home,away,'','');}
