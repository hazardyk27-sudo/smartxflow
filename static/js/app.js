...
function _calcLiveMatchMin(m){if(m.status==='ft')return'MS';var rawMin=(m.minute||'').trim();if(!rawMin){if(m.kickoff_utc){var ko=new Date(m.kickoff_utc);if(!isNaN(ko.getTime())){var diff=Math.floor((Date.now()-ko.getTime())/60000);if(diff>95)return'MS';if(diff>=0)return diff+"'";}}
return'';}
...
if(/^\d{1,3}'?$/.test(rawMin)){var n=parseInt(rawMin,10);if(n>95)return'MS';if(n>=0&&n<=130){return n+"'";}}
