...
return null;}
var fArr=(typeof filteredMatches!=='undefined'&&filteredMatches.length>0)?filteredMatches:[];var mArr=(typeof matches!=='undefined'&&matches.length>0)?matches:[];console.log('[LiveModal] Searching',homeLow,'vs',awayLow,'in filtered:',fArr.length,'all:',mArr.length);for(var di=0;di<mArr.length;di++){var dh=(mArr[di].home_team||'').toLowerCase().trim();if(dh.indexOf('estud')>=0||dh.indexOf(homeLow.substring(0,5))>=0){console.log('[LiveModal DEBUG] Found candidate in all['+di+']:',mArr[di].home_team,'vs',mArr[di].away_team);}}
var res=_fuzzySearch(fArr,'filteredMatches');if(res){match=res.m;matchIdx=res.idx;usedSource='filtered';}
