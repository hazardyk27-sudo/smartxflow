...
if(isMoneyway){html+=renderMobileMoneywayCard(match,i,d,volume,dateStr,_mStar);}else if(isDropping){html+=renderMobileOddsCard(match,i,d,volume,dateStr,_mStar);}else{const matchStatus=getMatchStatus(match.date);const _mk=_getMatchKey(match);const _isFav=_userFavorites.has(_mk);const _fc=_favCounts[_mk]||0;html+=`
            ...
            `;}}
cardList.insertAdjacentHTML('beforeend',html);_renderedCount=end;_updateRenderedCount();if(_favFilterActive)setTimeout(()=>_applyFavoritesFilter(),10);}
...
function _mobileFavLine(mk,isFav,count){var txt=(count>0?count:0)+' kişi takip ediyor';return'<div class="mobile-fav-line"><span class="fav-heart '+(isFav?'fav-active':'')+'" data-matchkey="'+mk.replace(/"/g,'&quot;')+'" onclick="event.stopPropagation(); toggleFavorite(this);">'+_heartSvg+'</span><span class="mobile-fav-count" data-matchkey="'+mk.replace(/"/g,'&quot;')+'">'+txt+'</span></div>';}
function renderMobileMoneywayCard(match,idx,d,volume,dateStr,starHtml){let oddsBlocks='';var tv=d.Volume;if(currentMarket.includes('1x2')){oddsBlocks=`
        ...
        `;}
const blockCount=currentMarket.includes('1x2')?'three':'two';const matchStatus=getMatchStatus(match.date);const liveCapsule=getMatchLiveCapsule(match.date,match.home_team,match.away_team,match.match_id);const _mk=_getMatchKey(match);const _isFav=_userFavorites.has(_mk);const _fc=_favCounts[_mk]||0;return`
...
var _liveMode=false;var _liveInterval=null;var _liveData=[];var _liveMarket='1x2';var _modalLiveData=null;var _modalLiveMarket='1x2';var _liveDetailData=null;var _liveDetailMarket='1x2';var _prevLiveScores={};var _finishedScores={};function proLockPanelHtml(){return'<div class="pro-lock-panel">'+'<div class="pro-lock-icon-wrap">'+'<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#2BFF88" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">'+'<rect x="3" y="11" width="18" height="11" rx="2.5"/>'+'<path d="M7 11V7a5 5 0 0 1 10 0v4"/>'+'</svg>'+'</div>'+'<div class="pro-lock-title">Canl\u0131 veriler i\u00e7in <span class="pro-lock-accent">Pro</span> \u00fcyelik gerekmektedir</div>'+'<a href="/pricing" class="pro-lock-btn">Pro\'ya Ge\u00e7 \u2192</a>'+'</div>';}
...
var nowMs=Date.now();var filtered=matches.filter(function(m){var min=(m.minute||'').trim().toUpperCase();if(min==='FT'||min==='AET'||min==='PEN')return false;if((m.status||'').toLowerCase()==='ft')return false;if(m.kickoff_utc){var ko=new Date(m.kickoff_utc);if(!isNaN(ko.getTime())&&(nowMs-ko.getTime())/60000>=120)return false;}
return true;});var rowsHtml='';var cardsHtml='';for(var i=0;i<filtered.length;i++){var m=filtered[i];var o=m.odds||{};var matchMin=_calcLiveMatchMin(m);var scoreStr=m.score?escLiveHtml(m.score):'';var _liveFavTd=_buildLiveFavCell(m);var capHtml='<div class="live-capsule live-capsule-desk">'+'<span class="lc-dot"></span>'+'<span class="lc-min">'+escLiveHtml(matchMin)+'</span>';if(scoreStr){var sp=scoreStr.split('-');if(sp.length===2){capHtml+='<span class="lc-divider"></span>'+'<span class="lc-score-h">'+sp[0].trim()+'</span>'+'<span class="lc-score-sep">-</span>'+'<span class="lc-score-a">'+sp[1].trim()+'</span>';}else{capHtml+='<span class="lc-divider"></span><span class="lc-score-val">'+escLiveHtml(scoreStr)+'</span>';}}
...
_liveFavTd+'<td class="match-date">'+capHtml+'</td>'+'<td class="match-league" title="'+escLiveHtml(m.league)+'">'+escLiveHtml(m.league)+'</td>'+'<td class="match-teams">'+escLiveHtml(m.home_team)+'<span class="vs">-</span>'+escLiveHtml(m.away_team)+'</td>'+'<td class="mw-outcomes-cell" colspan="3"><div class="mw-grid mw-grid-3">'+b1+bX+b2+'</div></td>'+'<td class="volume-cell">'+formatLiveVol(vol)+'</td></tr>';var _lmk=_getMatchKey(m);var _lmkIsFav=_userFavorites.has(_lmk);var _lmkFc=_favCounts[_lmk]||0;var cardScore=m.score?escLiveHtml(m.score):(matchMin==='MS'?'-':'');var mobCapHtml='';if(matchMin||cardScore){mobCapHtml='<div class="live-capsule">'+'<span class="lc-dot"></span>'+'<span class="lc-min">'+escLiveHtml(matchMin)+'</span>'+
...
_mobileFavLine(_lmk,_lmkIsFav,_lmkFc)+'<div class="odds-card-header">'+'<div class="odds-card-teams">'+escLiveHtml(m.home_team)+' \u2013 '+escLiveHtml(m.away_team)+'</div>'+'<div class="odds-card-volume">'+formatLiveVol(vol)+'</div>'+'</div>'+'<div class="odds-card-meta"><span>'+escLiveHtml(m.league)+'</span>'+mobCapHtml+'</div>'+'<div class="odds-card-row three">'+
...
if(tbody)tbody.innerHTML=rowsHtml;if(cardList)cardList.innerHTML=cardsHtml;var tc=document.querySelector('.table-container');if(tc){var oldOv=tc.querySelector('.live-pro-overlay');if(oldOv)oldOv.remove();}
if(tc&&table&&filtered.length>0){setTimeout(function(){var selTh=table.querySelector('th.col-selection');if(!selTh)return;var tcRect=tc.getBoundingClientRect();var selRect=selTh.getBoundingClientRect();var leftPx=selRect.left-tcRect.left+tc.scrollLeft;var ov=document.createElement('div');ov.className='live-pro-overlay desktop-only';ov.style.left=leftPx+'px';ov.style.height=tc.scrollHeight+'px';ov.innerHTML='<div class="live-pro-overlay-msg">'+'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="flex-shrink:0;vertical-align:middle;margin-right:6px"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>'+'Canl\u0131 veriler i\u00e7in <strong>Pro</strong> \u00fcyelik gerekmektedir '+'<a href="/pricing" class="live-pro-overlay-link">Pro\'ya Ge\u00e7 \u2192</a>'+'</div>';tc.appendChild(ov);},30);}
if(cardList&&filtered.length>0){var mobOvHtml='<div class="live-pro-overlay-mobile">'+'<div class="live-pro-overlay-msg">'+'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="flex-shrink:0;vertical-align:middle;margin-right:5px"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>'+'Canl\u0131 veriler i\u00e7in <strong>Pro</strong> \u00fcyelik gerekmektedir '+'<a href="/pricing" class="live-pro-overlay-link">Pro\'ya Ge\u00e7 \u2192</a>'+'</div></div>';cardList.querySelectorAll('.match-card').forEach(function(card){var odr=card.querySelector('.odds-card-row');if(odr){card.style.position='relative';odr.classList.add('live-card-blur-section');var volEl=card.querySelector('.odds-card-volume');if(volEl)volEl.classList.add('live-card-blur-section');}});var mobOvDiv=document.createElement('div');mobOvDiv.innerHTML=mobOvHtml;cardList.insertBefore(mobOvDiv.firstChild,cardList.firstChild);}
var mc=document.getElementById('matchCount');if(mc)mc.textContent=filtered.length;if(_favFilterActive)setTimeout(function(){_applyFavoritesFilter();},10);}
function switchToLive(){if(window.userPlan==='core'){_renderLiveProLock();return;}
...
var is1x2=(_liveMarket==='1x2');var ouLine=is1x2?'':_liveMarket.replace('ou','');if(is1x2){if(table)table.setAttribute('data-selection-count','3');if(colgroup)colgroup.innerHTML='<col class="col-fav"><col class="col-date"><col class="col-league"><col class="col-match">'+'<col class="col-selection"><col class="col-selection"><col class="col-selection"><col class="col-volume">';if(thead)thead.innerHTML='<tr>'+'<th class="col-fav"></th>'+'<th class="col-date">DK</th>'+'<th class="col-league">LİG</th>'+'<th class="col-match">MAÇ</th>'+'<th class="col-selection">1</th>'+'<th class="col-selection">X</th>'+'<th class="col-selection">2</th>'+'<th class="col-volume">HACİM</th>'+'</tr>';}else{if(table)table.setAttribute('data-selection-count','2');if(colgroup)colgroup.innerHTML='<col class="col-fav"><col class="col-date"><col class="col-league"><col class="col-match">'+'<col class="col-selection"><col class="col-selection"><col class="col-volume">';if(thead)thead.innerHTML='<tr>'+'<th class="col-fav"></th>'+'<th class="col-date">DK</th>'+'<th class="col-league">LİG</th>'+'<th class="col-match">MAÇ</th>'+'<th class="col-selection">ALT</th>'+'<th class="col-selection">ÜST</th>'+'<th class="col-volume">HACİM</th>'+'</tr>';}
