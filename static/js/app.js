...
async function loadChartForSource(home,away,market,league){if(chartOddsSource==='matchbook'){var cacheKey=`${home}|${away}|${league}|mb_chart`;if(_mbBulkCacheKey!==cacheKey){var params=`home=${encodeURIComponent(home)}&away=${encodeURIComponent(away)}&league=${encodeURIComponent(league || '')}`;try{var resp=await fetch('/api/match/history/matchbook?'+params);var data=await resp.json();_mbBulkCache=data.markets||{};_mbBulkCacheKey=cacheKey;}catch(e){console.error('[ChartSource] Matchbook fetch error:',e);return;}}
