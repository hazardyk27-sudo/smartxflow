...
async function refreshMatchData(){if(_loadMatchesLock){console.log('[AutoRefresh] Zaten yükleme var, atlanıyor');return;}
...
async function loadLiveMatches(){try{var resp=await fetch('/api/live/matches');var data=await resp.json();_liveData=data.matches||[];renderLiveMatches(_liveData);document.getElementById('liveMatchCount').textContent=_liveData.length;var now=new Date();document.getElementById('liveUpdateTime').textContent=String(now.getHours()).padStart(2,'0')+':'+
...
function _calcLiveMatchMin(m){if(!m.kickoff_utc)return escLiveHtml(m.minute||'');try{var ko=new Date(m.kickoff_utc);var now=new Date();var diff=Math.max(0,Math.floor((now-ko)/60000));return diff+"'";}catch(e){return escLiveHtml(m.minute||'');}}
function _liveMwBlock(label,d){if(!d)return'<div class="mw-outcome-block"><div class="mw-info-stack"><div class="mw-odds">-</div></div><div class="mw-donut">'+_liveDonut(0,52)+'</div></div>';var pct=d.share||0;var odds=d.odds?d.odds.toFixed(2):'-';var money=d.volume?formatLiveVol(d.volume):'';return'<div class="mw-outcome-block">'+'<div class="mw-info-stack">'+'<div class="mw-odds">'+odds+'</div>'+
(money?'<div class="mw-money">'+money+'</div>':'')+'</div>'+'<div class="mw-donut">'+_liveDonut(pct,52)+'</div>'+'</div>';}
...
html+='<div class="odds-card-volume">'+formatLiveVol(vol)+'</div>';html+='</div>';html+='<div class="odds-card-meta"><span>'+escLiveHtml(m.league)+'</span><span class="meta-sep">\u2022</span>';html+='<span class="live-min-badge" style="font-size:11px;">'+_calcLiveMatchMin(m)+'</span>';if(m.score)html+='<span class="meta-sep">\u2022</span><span style="color:#f59e0b;font-weight:600;">'+escLiveHtml(m.score)+'</span>';html+='</div>';if(is1x2){var o=m.odds||{};html+='<div class="odds-card-row three">';html+=_liveMobileBlock('1',o['1']);html+=_liveMobileBlock('X',o['X']);html+=_liveMobileBlock('2',o['2']);html+='</div>';}else{var ouD=_liveGetOu(m,marketOrLine);html+='<div class="odds-card-row two">';html+=_liveMobileBlock('Alt',ouD['U']);html+=_liveMobileBlock('Ust',ouD['O']);html+='</div>';}
...
var is1x2=(market==='1x2');var ouLine=is1x2?'':market.replace('ou','');var sels=is1x2?['1','X','2']:['U','O'];var selLabels=is1x2?['1','X','2']:['Alt '+escLiveHtml(ouLine),'Ust '+escLiveHtml(ouLine)];var html='<div class="live-detail-table-wrap"><table class="live-period-table">';html+='<thead>';html+='<tr class="lpt-group-row">';html+='<th colspan="2"></th>';for(var g=0;g<sels.length;g++){html+='<th colspan="4" class="lpt-group-th">'+selLabels[g]+'</th>';}
html+='<th></th>';html+='</tr>';html+='<tr>';html+='<th>Time</th><th>Score</th>';for(var h=0;h<sels.length;h++){html+='<th>Odd</th><th>Odd +/-</th><th>Volume</th><th>% of total</th>';}
html+='<th>Toplam Hacim</th>';html+='</tr>';html+='</thead><tbody>';var totals={};sels.forEach(function(sel){totals[sel]=0;});for(var i=0;i<snapshots.length;i++){var s=snapshots[i];var prevS=i>0?snapshots[i-1]:null;var minVal=s.minute;var minStr=(minVal!==''&&minVal!==null&&minVal!==undefined)?minVal+"'":'-';var scoreStr=s.score||'-';var rowVol=0;html+='<tr>';html+='<td class="period-time">'+escLiveHtml(minStr)+'</td>';html+='<td class="period-score">'+escLiveHtml(scoreStr)+'</td>';var dataGroup=is1x2?(s['1x2']||{}):((s.ou_lines||{})[ouLine]||{});var prevDataGroup=prevS?(is1x2?(prevS['1x2']||{}):((prevS.ou_lines||{})[ouLine]||{})):null;for(var j=0;j<sels.length;j++){var sel=sels[j];var sd=dataGroup[sel]||{};var odds=(sd.odds!==undefined&&sd.odds!==null)?parseFloat(sd.odds):null;var pct=sd.share||0;var vol=sd.volume||0;rowVol+=vol;totals[sel]+=vol;var oddsStr=odds!==null?odds.toFixed(2):'-';var pctStr=pct?pct.toFixed(0)+'%':'-';var hi=pct>=50;var deltaHtml='<span class="period-delta-neutral">-</span>';if(prevDataGroup&&odds!==null){var prevOdds=(prevDataGroup[sel]||{}).odds;if(prevOdds!==undefined&&prevOdds!==null){var delta=odds-parseFloat(prevOdds);if(Math.abs(delta)>=0.005){var dSign=delta>0?'+':'';var dCls=delta>0?'period-delta-up':'period-delta-down';deltaHtml='<span class="'+dCls+'">'+dSign+delta.toFixed(2)+'</span>';}}}
html+='<td class="period-odds">'+oddsStr+'</td>';html+='<td>'+deltaHtml+'</td>';html+='<td class="period-vol">'+(vol?formatLiveVol(vol):'-')+'</td>';html+='<td class="period-pct '+(hi?'pct-high':'pct-low')+'">'+pctStr+'</td>';}
html+='<td class="period-vol-total">'+formatLiveVol(rowVol)+'</td>';html+='</tr>';}
html+='<tr class="lpt-totals-row">';html+='<td colspan="2" class="lpt-totals-label">Toplam</td>';var grandTotal=0;for(var k=0;k<sels.length;k++){grandTotal+=totals[sels[k]];html+='<td colspan="3"></td>';html+='<td class="period-vol lpt-total-vol">'+formatLiveVol(totals[sels[k]])+'</td>';}
html+='<td class="period-vol-total lpt-grand-total">'+formatLiveVol(grandTotal)+'</td>';html+='</tr>';html+='</tbody></table></div>';body.innerHTML=html;}