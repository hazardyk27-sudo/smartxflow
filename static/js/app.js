...
function renderMobileMoneywayBlock(label,oddsValue,pctValue,totalVol){const odds=formatOdds(oddsValue);let pct='';let moneyHtml='';if(pctValue!==null&&pctValue!==undefined){const pctNum=parseFloat(String(pctValue).replace('%',''));if(!isNaN(pctNum)){pct=pctNum.toFixed(0)+'%';if(totalVol){var rawVol=String(totalVol).replace(/[£€$,\s]/g,'');var mult=1;if(rawVol.toUpperCase().includes('M')){mult=1000000;rawVol=rawVol.replace(/M/gi,'');}
else if(rawVol.toUpperCase().includes('K')){mult=1000;rawVol=rawVol.replace(/K/gi,'');}
var totalNum=parseFloat(rawVol)*mult;if(!isNaN(totalNum)&&totalNum>0){var blockMoney=totalNum*pctNum/100;var moneyStr=blockMoney>=1000000?'£'+(blockMoney/1000000).toFixed(1)+'M':blockMoney>=1000?'£'+(blockMoney/1000).toFixed(1)+'k':'£'+Math.round(blockMoney);moneyHtml='<span class="ob-money">'+moneyStr+'</span>';}}}}
var bottomRow='';if(moneyHtml||pct){bottomRow='<div class="odds-block-bottom">'+moneyHtml+(pct?'<span class="ob-pct">'+pct+'</span>':'')+'</div>';}
return`
...
var minCell='<td class="match-date">'+'<div class="dk-cell-inner"'+goalData+'>'+'<div class="date-line live-min-badge'+minExtra+'">'+escLiveHtml(matchMin)+'</div>'+
...
html+='</div>';if(is1x2){var o=m.odds||{};html+='<div class="odds-card-row three">';html+=_liveMobileBlock('1',o['1'],vol);html+=_liveMobileBlock('X',o['X'],vol);html+=_liveMobileBlock('2',o['2'],vol);html+='</div>';var lp1=(o['1']&&o['1'].share)?parseFloat(o['1'].share):0;var lpX=(o['X']&&o['X'].share)?parseFloat(o['X'].share):0;var lp2=(o['2']&&o['2'].share)?parseFloat(o['2'].share):0;if(lp1>0||lpX>0||lp2>0){var lBest=Math.max(lp1,lpX,lp2);var lLeader=lBest===lp1?'1':(lBest===lpX?'X':'2');var lSpread=(lBest-Math.min(lp1>0?lp1:999,lpX>0?lpX:999,lp2>0?lp2:999)).toFixed(0);var lTag=parseInt(lSpread)>30?'Güçlü favori':(parseInt(lSpread)>10?'Favori açık':'Dengeli');html+='<div class="odds-card-insight"><span class="insight-tag">Favori: <span class="insight-val">'+lLeader+'</span></span><span class="insight-tag">Pay: <span class="insight-val positive">'+lBest.toFixed(0)+'%</span></span><span class="insight-label">'+lTag+'</span></div>';}}else{var ouD=_liveGetOu(m,marketOrLine);var ouVol=_liveCalcVolOU(ouD);html+='<div class="odds-card-row two">';html+=_liveMobileBlock('Alt',ouD['U'],ouVol);html+=_liveMobileBlock('Ust',ouD['O'],ouVol);html+='</div>';var lpU=(ouD['U']&&ouD['U'].share)?parseFloat(ouD['U'].share):0;var lpO=(ouD['O']&&ouD['O'].share)?parseFloat(ouD['O'].share):0;if(lpU>0||lpO>0){var ouLdr=lpO>lpU?'Üst':'Alt';var ouPct=Math.max(lpU,lpO);html+='<div class="odds-card-insight"><span class="insight-tag">Lider: <span class="insight-val">'+ouLdr+'</span></span><span class="insight-tag">Pay: <span class="insight-val positive">'+ouPct.toFixed(0)+'%</span></span></div>';}}
html+='</div>';return html;}
function _liveMobileBlock(label,d,totalVol){var odds=(d&&d.odds)?d.odds.toFixed(2):'-';var pct='';var moneyHtml='';if(d&&d.share!==null&&d.share!==undefined){var pn=parseFloat(String(d.share));if(!isNaN(pn)){pct=pn.toFixed(0)+'%';if(totalVol&&totalVol>0){var bm=totalVol*pn/100;var ms=bm>=1000000?'£'+(bm/1000000).toFixed(1)+'M':bm>=1000?'£'+(bm/1000).toFixed(1)+'k':'£'+Math.round(bm);moneyHtml='<span class="ob-money">'+ms+'</span>';}}}
var bottom='';if(moneyHtml||pct){bottom='<div class="odds-block-bottom">'+moneyHtml+(pct?'<span class="ob-pct">'+pct+'</span>':'')+'</div>';}
return'<div class="odds-block mw-block">'+'<div class="odds-block-label">'+label+'</div>'+'<div class="odds-block-value">'+odds+'</div>'+
bottom+'</div>';}
