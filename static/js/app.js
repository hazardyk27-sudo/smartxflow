...
resolve();});});};let currentMarket='moneyway_1x2';let matches=[];let filteredMatches=[];let chart=null;let selectedMatch=null;let selectedChartMarket='moneyway_1x2';let autoScrapeRunning=false;let currentSortColumn='volume';let currentSortDirection='desc';let chartVisibleSeries={};let dateFilterMode='ALL';let chartTimeRange='10min';let currentChartHistoryData=[];let chartViewMode='percent';let isClientMode=true;let _modalRequestId=0;let _analysisMatchHashes=[];let _mobileHideEnded=false;let _mobileHideLive=false;let _mobileOnlyLive=false;let _analysisHashesFetched=false;let _licenseReadyResolve;const _licenseReady=new Promise(r=>{_licenseReadyResolve=r;});let _isLicensed=false;function getWebDeviceId(){let did=localStorage.getItem('smartxflow_device_id');if(!did||did.length>16){did='w-'+crypto.randomUUID().replace(/-/g,'').substring(0,14);localStorage.setItem('smartxflow_device_id',did);}
return did;}
...
return null;}
function _findFinishedScore(homeTeam,awayTeam){var key=((homeTeam||'')+'|'+(awayTeam||'')).toLowerCase();return _finishedScores[key]||null;}
async function loadFinishedScores(){try{var resp=await fetch('/api/finished-scores');var data=await resp.json();if(data&&data.scores){_finishedScores=data.scores;}}catch(e){}}
function getMatchLiveCapsule(dateStr,homeTeam,awayTeam){if(!dateStr)return'';const matchTime=toTurkeyTime(dateStr);if(!matchTime||!matchTime.isValid())return'';const now=nowTurkey();const diffMinutes=now.diff(matchTime,'minute');if(diffMinutes>=80){var lmExact=_findLiveMatchExact(homeTeam,awayTeam);if(_isFinishedLiveMatch(lmExact)){return _buildFinishedCapsule(lmExact,'');}
var ftScore=_findFinishedScore(homeTeam,awayTeam);if(ftScore){return _buildFinishedCapsule({score:ftScore.score,status:'ft'},'');}}
var lm=_findLiveMatch(homeTeam,awayTeam);if(diffMinutes<0||diffMinutes>113)return'';var minStr='';var scoreHtml='';if(lm){var rawMin=(lm.minute||'').trim();minStr=rawMin||diffMinutes+"'";var sc=lm.score||'';if(sc){var parts=sc.split('-');var sh=(parts[0]||'').trim();var sa=(parts[1]||'').trim();scoreHtml='<span class="lc-divider"></span>'+'<span class="lc-score-val"><span class="lc-score-h">'+sh+'</span>'+'<span class="lc-score-sep">-</span>'+'<span class="lc-score-a">'+sa+'</span></span>';}}else{if(diffMinutes<=45){minStr=diffMinutes+"'";}else if(diffMinutes<=60){minStr='HT';}else if(diffMinutes<=105){minStr=(diffMinutes-15)+"'";}else{minStr="90+'";}}
return'<div class="live-capsule">'+'<span class="lc-dot"></span>'+'<span class="lc-min">'+minStr+'</span>'+
scoreHtml+'</div>';}
function getMatchLiveCapsuleDT(dateStr,homeTeam,awayTeam){if(!dateStr)return'';const matchTime=toTurkeyTime(dateStr);if(!matchTime||!matchTime.isValid())return'';const now=nowTurkey();const diffMinutes=now.diff(matchTime,'minute');if(diffMinutes>=80){var lmExact=_findLiveMatchExact(homeTeam,awayTeam);if(_isFinishedLiveMatch(lmExact)){return _buildFinishedCapsule(lmExact,'live-capsule-desk');}
var ftScore=_findFinishedScore(homeTeam,awayTeam);if(ftScore){return _buildFinishedCapsule({score:ftScore.score,status:'ft'},'live-capsule-desk');}}
var lm=_findLiveMatch(homeTeam,awayTeam);if(diffMinutes<0||diffMinutes>113)return'';var minStr='';var scoreHtml='';if(lm){var rawMin=(lm.minute||'').trim();minStr=rawMin||diffMinutes+"'";var sc=lm.score||'';if(sc){var parts=sc.split('-');var sh=(parts[0]||'').trim();var sa=(parts[1]||'').trim();scoreHtml='<span class="lc-divider"></span>'+'<span class="lc-score-val"><span class="lc-score-h">'+sh+'</span>'+'<span class="lc-score-sep">-</span>'+'<span class="lc-score-a">'+sa+'</span></span>';}}else{if(diffMinutes<=45){minStr=diffMinutes+"'";}else if(diffMinutes<=60){minStr='HT';}else if(diffMinutes<=105){minStr=(diffMinutes-15)+"'";}else{minStr="90+'";}}
...
let apiUrl;if(dateFilterMode==='ALL'){apiUrl=`/api/matches?market=${requestMarket}&date_filter=today_future&bulk=1`;}else if(dateFilterMode==='YESTERDAY'){apiUrl=`/api/matches?market=${requestMarket}&date_filter=yesterday&bulk=1`;}else if(dateFilterMode==='TODAY'){apiUrl=`/api/matches?market=${requestMarket}&date_filter=today&bulk=1`;}else{apiUrl=`/api/matches?market=${requestMarket}&date_filter=today_future&bulk=1`;}
...
function _renderMatchRow(match,idx){const d=match.details||match.odds||{};const _star=getAnalysisStarHtml(match.match_id);const isDropping=currentMarket.startsWith('dropping');const isMoneyway=currentMarket.startsWith('moneyway');if(currentMarket.includes('1x2')){if(isMoneyway){const block1=renderMoneywayBlock('1',d.Pct1,d.Odds1||d['1'],d.Amt1);const blockX=renderMoneywayBlock('X',d.PctX,d.OddsX||d['X'],d.AmtX);const block2=renderMoneywayBlock('2',d.Pct2,d.Odds2||d['2'],d.Amt2);const matchStatus=getMatchStatus(match.date);const deskCapsule=getMatchLiveCapsuleDT(match.date,match.home_team,match.away_team);return`
            ...
            `;}}else if(currentMarket.includes('ou25')){if(isMoneyway){const blockUnder=renderMoneywayBlock('Alt',d.PctUnder,d.Under,d.AmtUnder);const blockOver=renderMoneywayBlock('Üst',d.PctOver,d.Over,d.AmtOver);const matchStatus=getMatchStatus(match.date);const deskCapsule=getMatchLiveCapsuleDT(match.date,match.home_team,match.away_team);return`
                <tr data-index="${idx}" onclick="openMatchModal(${idx})">
                    <td class="match-date">${deskCapsule || formatDateTwoLine(match.date)}</td>
                    <td class="match-league" title="${match.league || ''}">${match.league || '-'}</td>
                    <td class="match-teams">${match.home_team}<span class="vs">-</span>${match.away_team}${!deskCapsule ? matchStatus : ''}${_star}</td>
            ...
            `;}}else{if(isMoneyway){const blockYes=renderMoneywayBlock('Evet',d.PctYes,d.OddsYes||d.Yes,d.AmtYes);const blockNo=renderMoneywayBlock('Hayır',d.PctNo,d.OddsNo||d.No,d.AmtNo);const matchStatus=getMatchStatus(match.date);const deskCapsule=getMatchLiveCapsuleDT(match.date,match.home_team,match.away_team);return`
                <tr data-index="${idx}" onclick="openMatchModal(${idx})">
                    <td class="match-date">${deskCapsule || formatDateTwoLine(match.date)}</td>
                    <td class="match-league" title="${match.league || ''}">${match.league || '-'}</td>
                    <td class="match-teams">${match.home_team}<span class="vs">-</span>${match.away_team}${!deskCapsule ? matchStatus : ''}${_star}</td>
...
return'';}
function sortByColumn(column){if(currentSortColumn===column){currentSortDirection=currentSortDirection==='asc'?'desc':'asc';}else{currentSortColumn=column;currentSortDirection='desc';}
...
var nowMs=Date.now();var filtered=matches.filter(function(m){var min=(m.minute||'').trim().toUpperCase();if(min==='FT'||min==='MS'||min==='AET'||min==='PEN')return false;if((m.status||'').toLowerCase()==='ft')return false;if(!min&&!m.score){if(m.kickoff_utc){var ko=new Date(m.kickoff_utc);if(!isNaN(ko.getTime())){var diffM=(nowMs-ko.getTime())/60000;if(diffM<0||diffM>130)return false;}}else{return false;}}
