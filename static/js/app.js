...
function _updateFavCountsInDOM(){document.querySelectorAll('.fav-heart').forEach(function(el){var mk=el.getAttribute('data-matchkey');var countEl=el.parentElement.querySelector('.fav-count');if(countEl&&mk){var c=_favCounts[mk]||0;countEl.innerHTML=_favCountHtml(c);}});}
function toggleFavorite(el){var mk=el.getAttribute('data-matchkey');if(!mk)return;var did=getWebDeviceId();console.log('[Fav] toggle:',mk,'device:',did);fetch('/api/favorite/toggle',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({match_key:mk,device_id:did})}).then(function(r){console.log('[Fav] response status:',r.status);return r.json();}).then(function(data){console.log('[Fav] result:',data.favorited?'ADDED':'REMOVED','count:',data.total_count);if(data.favorited){_userFavorites.add(mk);}else{_userFavorites.delete(mk);}
