...
return true;});var bannerRowHtml='<tr class="live-pro-banner-row"><td colspan="8">'+_liveProBanner()+'</td></tr>';var rowsHtml=bannerRowHtml;var cardsHtml=_liveProBanner();for(var i=0;i<filtered.length;i++){var m=filtered[i];var o=m.odds||{};var matchMin=_calcLiveMatchMin(m);var scoreStr=m.score?escLiveHtml(m.score):'';var _liveFavTd=_buildLiveFavCell(m);var capHtml='<div class="live-capsule live-capsule-desk">'+'<span class="lc-dot"></span>'+'<span class="lc-min">'+escLiveHtml(matchMin)+'</span>';if(scoreStr){var sp=scoreStr.split('-');if(sp.length===2){capHtml+='<span class="lc-divider"></span>'+'<span class="lc-score-h">'+sp[0].trim()+'</span>'+'<span class="lc-score-sep">-</span>'+'<span class="lc-score-a">'+sp[1].trim()+'</span>';}else{capHtml+='<span class="lc-divider"></span><span class="lc-score-val">'+escLiveHtml(scoreStr)+'</span>';}}
...
_liveFavTd+'<td class="match-date">'+capHtml+'</td>'+'<td class="match-league" title="'+escLiveHtml(m.league)+'">'+escLiveHtml(m.league)+'</td>'+'<td class="match-teams">'+escLiveHtml(m.home_team)+'<span class="vs">-</span>'+escLiveHtml(m.away_team)+'</td>'+'<td class="mw-outcomes-cell live-blur-cell" colspan="3"><div class="mw-grid mw-grid-3">'+b1+bX+b2+'</div></td>'+'<td class="volume-cell live-blur-cell">'+formatLiveVol(vol)+'</td></tr>';var _lmk=_getMatchKey(m);var _lmkIsFav=_userFavorites.has(_lmk);var _lmkFc=_favCounts[_lmk]||0;var cardScore=m.score?escLiveHtml(m.score):(matchMin==='MS'?'-':'');var mobCapHtml='';if(matchMin||cardScore){mobCapHtml='<div class="live-capsule">'+'<span class="lc-dot"></span>'+'<span class="lc-min">'+escLiveHtml(matchMin)+'</span>'+
...
_mobileFavLine(_lmk,_lmkIsFav,_lmkFc)+'<div class="odds-card-header">'+'<div class="odds-card-teams">'+escLiveHtml(m.home_team)+' \u2013 '+escLiveHtml(m.away_team)+'</div>'+'<div class="odds-card-volume live-blur-cell">'+formatLiveVol(vol)+'</div>'+'</div>'+'<div class="odds-card-meta"><span>'+escLiveHtml(m.league)+'</span>'+mobCapHtml+'</div>'+'<div class="odds-card-row three live-blur-cell">'+
