...
+'</div>';document.body.appendChild(overlay);}}
function deviceKickedRelogin(){fetch('/api/auth/logout',{method:'POST'}).catch(()=>{});window.location.href='/login';}
let _licenseStatusInterval=null;function startLicenseStatusRefresh(){if(_licenseStatusInterval)return;async function checkAccountStatus(){try{const resp=await _originalFetch('/api/auth/session-status');const data=await resp.json();if(data.status==='login_required'){window.location.href='/login?next='+encodeURIComponent(window.location.pathname);}else if(data.status==='email_unverified'){window.location.href='/verify-email';}else if(data.status==='membership_required'){window.location.href='/membership-required';}else if(data.status==='ok'){if(data.subscription_expires_at){var exp=new Date(data.subscription_expires_at);var days=Math.ceil((exp-new Date())/(1000*60*60*24));updateLicenseDaysBadge(days);}
if(data.plan){window.userPlan=data.plan;if(typeof _isPro!=='undefined')_isPro=(window.userPlan==='pro');}}}catch(e){}}
checkAccountStatus();_licenseStatusInterval=setInterval(checkAccountStatus,5*60*1000);}
const _originalFetch=window.fetch;window.fetch=function(url,options={}){const urlStr=typeof url==='string'?url:url.url||'';if(urlStr.startsWith('/api/')){const legacyKey=localStorage.getItem('smartxflow_web_license');if(legacyKey){options=options||{};options.headers=options.headers||{};if(options.headers instanceof Headers)options.headers.set('X-License-Key',legacyKey);else options.headers['X-License-Key']=legacyKey;}}return _originalFetch.call(window,url,options).then(function(response){if(response.status===403&&urlStr.startsWith('/api/')){response.clone().json().then(function(data){if(data&&data.error==='LOGIN_REQUIRED'&&!localStorage.getItem('smartxflow_web_license')){window.location.href='/login?next='+encodeURIComponent(window.location.pathname);}else if(data&&data.error==='EMAIL_NOT_VERIFIED'){window.location.href='/verify-email';}else if(data&&data.error==='MEMBERSHIP_REQUIRED'){window.location.href='/membership-required';}}).catch(function(){});}
...
return gradient;}
const filteredHistory=filterHistoryByTimeRange(data.history);const config=getBucketConfig();const bucketMinutes=config.bucketMinutes;const timeBlocks={};const actualTimes={};filteredHistory.forEach(h=>{const ts=h.ScrapedAt||'';const dt=toTurkeyTime(ts);if(!dt||!dt.isValid())return;if(bucketMinutes<=10){const key=dt.valueOf();timeBlocks[key]=h;actualTimes[key]=dt.valueOf();}else{const epochMinutes=Math.floor(dt.valueOf()/60000);const roundedEpoch=Math.floor(epochMinutes/bucketMinutes)*bucketMinutes;const bucketKey=roundedEpoch*60000;timeBlocks[bucketKey]=h;actualTimes[bucketKey]=dt.valueOf();}});const sortedKeys=Object.keys(timeBlocks).map(Number).sort((a,b)=>a-b);const labelFormat=config.labelFormat;const labels=sortedKeys.map(k=>{const realTime=actualTimes[k];const dt=dayjs(realTime).tz(APP_TIMEZONE);if(!dt||!dt.isValid())return'';if(labelFormat==='date')return dt.format('DD.MM');if(labelFormat==='datetime')return dt.format('DD.MM • HH:mm');return dt.format('HH:mm');});const historyData=sortedKeys.map(k=>timeBlocks[k]);if(isDropping&&historyData.length>0){const firstEntry=historyData[0];let hasOpeningOdds=false;let openingPoint={ScrapedAt:''};const _parseOr=(v,...keys)=>{const n=parseFloat(v);if(!isNaN(n)&&n>0)return n;for(const k of keys){for(const entry of historyData){const fb=parseFloat(String(entry[k]||'').split('\n')[0]);if(!isNaN(fb)&&fb>0)return fb;}}
return null;};if(market.includes('1x2')){const op1=parseFloat(firstEntry.Odds1_prev);const opX=parseFloat(firstEntry.OddsX_prev);const op2=parseFloat(firstEntry.Odds2_prev);if(!isNaN(op1)&&op1>0){hasOpeningOdds=true;openingPoint.Odds1=op1;openingPoint['1']=op1;openingPoint.OddsX=_parseOr(opX,'OddsX','X');openingPoint['X']=openingPoint.OddsX;openingPoint.Odds2=_parseOr(op2,'Odds2','2');openingPoint['2']=openingPoint.Odds2;}}else if(market.includes('ou25')){const opU=parseFloat(firstEntry.Under_prev||firstEntry.OddsUnder_prev);const opO=parseFloat(firstEntry.Over_prev||firstEntry.OddsOver_prev);if(!isNaN(opU)&&opU>0){hasOpeningOdds=true;openingPoint.Under=opU;openingPoint.Over=_parseOr(opO,'Over','OddsOver');openingPoint.OddsUnder=opU;openingPoint.OddsOver=openingPoint.Over;}}else if(market.includes('btts')){const opY=parseFloat(firstEntry.Yes_prev||firstEntry.OddsYes_prev);const opN=parseFloat(firstEntry.No_prev||firstEntry.OddsNo_prev);if(!isNaN(opY)&&opY>0){hasOpeningOdds=true;openingPoint.Yes=opY;openingPoint.No=_parseOr(opN,'No','OddsNo');openingPoint.OddsYes=opY;openingPoint.OddsNo=openingPoint.No;}}
...
const sorted=[...allValues].sort((a,b)=>a-b);const len=sorted.length;const p5Idx=Math.floor(len*0.05);const p95Idx=Math.floor(len*0.95);const p5=sorted[p5Idx];const p95=sorted[p95Idx];const range=p95-p5;const padding=range*0.08;const actualMin=Math.min(...allValues);const actualMax=Math.max(...allValues);const clampedMin=actualMin<p5-range?p5-padding:actualMin-padding;const clampedMax=actualMax>p95+range?p95+padding:actualMax+padding;return{x:{display:false,grid:{display:false,drawBorder:false},ticks:{display:false}},y:{display:false,grid:{display:false,drawBorder:false},ticks:{display:false},suggestedMin:clampedMin,suggestedMax:clampedMax}};})():{x:{offset:false,bounds:'data',grid:{color:'rgba(255, 255, 255, 0.06)',drawBorder:false,offset:false},border:{display:false},ticks:{color:'#a1a1aa',font:{size:11}}},y:{grid:{color:'rgba(255, 255, 255, 0.06)',drawBorder:false},border:{display:false},ticks:{color:'#8e8e95',font:{size:11},callback:function(value){if(value>=1000000){return(value/1000000).toFixed(1)+'M';}else if(value>=1000){return(value/1000).toFixed(0)+'K';}
...
const odds=h.Odds1||h['1']||h.Over||h.Yes||h.odds1||0;const oddsNum=parseFloat(String(odds).replace(/[^0-9.]/g,''));if(!isNaN(oddsNum)&&oddsNum>0)raw.push(oddsNum);});if(raw.length<2)return;const MAX_PTS=80;let values=raw;if(raw.length>MAX_PTS){const step=raw.length/MAX_PTS;values=[];for(let i=0;i<MAX_PTS;i++){const start=Math.floor(i*step);const end=Math.min(Math.floor((i+1)*step),raw.length);const chunk=raw.slice(start,end);values.push(chunk.reduce((a,b)=>a+b,0)/chunk.length);}}
const minVal=Math.min(...values);const maxVal=Math.max(...values);const range=maxVal-minVal||1;const pad=4;const pts=values.map((val,i)=>({x:(i/(values.length-1))*width,y:height-pad-((val-minVal)/range)*(height-pad*2)}));const gradient=ctx.createLinearGradient(0,0,0,height);gradient.addColorStop(0,'rgba(59, 130, 246, 0.25)');gradient.addColorStop(1,'rgba(59, 130, 246, 0.02)');ctx.beginPath();ctx.moveTo(pts[0].x,height);ctx.lineTo(pts[0].x,pts[0].y);for(let i=1;i<pts.length;i++){const cp1x=pts[i-1].x+(pts[i].x-pts[i-1].x)*0.5;ctx.bezierCurveTo(cp1x,pts[i-1].y,cp1x,pts[i].y,pts[i].x,pts[i].y);}
ctx.lineTo(pts[pts.length-1].x,height);ctx.closePath();ctx.fillStyle=gradient;ctx.fill();ctx.beginPath();ctx.moveTo(pts[0].x,pts[0].y);for(let i=1;i<pts.length;i++){const cp1x=pts[i-1].x+(pts[i].x-pts[i-1].x)*0.5;ctx.bezierCurveTo(cp1x,pts[i-1].y,cp1x,pts[i].y,pts[i].x,pts[i].y);}
ctx.strokeStyle='rgba(99, 152, 255, 0.7)';ctx.lineWidth=1.5;ctx.stroke();}
...
var pd=document.getElementById('mfPastDropdown');if(pd&&pd.style.display!=='none'&&typeof _buildMfPastDropdown==='function'){_buildMfPastDropdown(pd);}});
...
async function initLicenseCheck(){try{const resp=await fetch('/api/auth/session-status');const data=await resp.json();if(data.status==='login_required'){window.location.href='/login?next='+encodeURIComponent(window.location.pathname);return;}
if(data.status==='email_unverified'){window.location.href='/verify-email';return;}
if(data.status==='membership_required'){window.location.href='/membership-required';return;}
const logoutBtn=document.getElementById('logoutBtn');if(logoutBtn)logoutBtn.style.display='flex';if(data.test_mode){window.userPlan='test';if(typeof _isPro!=='undefined')_isPro=false;_isLicensed=true;await _loadTestFreeHashes();_licenseReadyResolve();updateLicenseDaysBadge(-1);var daysBadge=document.getElementById('licenseDaysBadge');if(daysBadge){var daysText=document.getElementById('licenseDaysText');if(daysText)daysText.textContent='Test Modu';daysBadge.classList.add('warning');}
setTimeout(function(){_addTestLockIcons();_addMobileMktLockIcons();},100);return;}
window.userPlan=data.plan||'core';if(typeof _isPro!=='undefined')_isPro=(window.userPlan==='pro');if(data.subscription_expires_at){var exp=new Date(data.subscription_expires_at);var days=Math.ceil((exp-new Date())/(1000*60*60*24));updateLicenseDaysBadge(days);}
_isLicensed=true;_licenseReadyResolve();startLicenseStatusRefresh();}catch(e){window.location.href='/login';}}
function logoutAccount(){if(confirm('Cikis yapmak istediginize emin misiniz?')){fetch('/api/auth/logout',{method:'POST'}).catch(()=>{}).finally(()=>{window.location.href='/login';});}}
document.addEventListener('DOMContentLoaded',function(){initLicenseCheck();});var _bgLiveInterval=null;async function _fetchBackgroundLiveData(){if(_liveMode)return;try{var resp=await fetch('/api/live/matches');var data=await resp.json();_liveData=data.matches||[];_updateLiveCapsulesInDOM();}catch(e){}}
...
function renderModalLiveDetail(snapshots,market){var body=document.getElementById('modalLiveBody');if(!body)return;if(!snapshots||snapshots.length===0){body.innerHTML='<div class="live-loading">'+_t('app.live.bos_tablo','Canlı maç bulunamadı')+'</div>';return;}
...
var is1x2=(market==='1x2');var ouLine=is1x2?'':market.replace('ou','');var sels=is1x2?['1','X','2']:['U','O'];var selLabels=is1x2?['1','X','2']:[escLiveHtml(ouLine)+' '+_t('app.tbl.alt','ALT'),escLiveHtml(ouLine)+' '+_t('app.tbl.over','ÜST')];var colsPerSel=3;var html='<div class="live-detail-table-wrap"><table class="live-period-table">';html+='<thead>';html+='<tr class="lpt-group-row">';html+='<th rowspan="2" class="lpt-dkskor-th"><span class="lpt-th-main">'+_t('app.live.dk','DK')+'</span><span class="lpt-th-sub">'+_t('app.live.skor','SKOR')+'</span></th>';for(var g=0;g<sels.length;g++){html+='<th colspan="'+colsPerSel+'" class="lpt-group-th">'+selLabels[g]+'</th>';}
html+='</tr>';html+='<tr class="lpt-col-row">';for(var h=0;h<sels.length;h++){html+='<th class="lpt-dual-th'+(h>0?' lpt-sel-divider':'')+'"><span class="lpt-th-main">'+_t('app.live.oran','ORAN')+'</span><span class="lpt-th-sub">'+_t('app.live.oran_degisim','Oran değişim')+'</span></th>';html+='<th class="lpt-dual-th"><span class="lpt-th-main">VOLUME</span><span class="lpt-th-sub">'+_t('app.live.gelen_para','Gelen para')+'</span></th>';html+='<th class="lpt-dual-th"><span class="lpt-th-main">'+_t('app.live.yuzde','YÜZDE')+'</span><span class="lpt-th-sub">'+_t('app.live.pct_degisimi','%değişimi')+'</span></th>';}
...
html+='</tr>';}
html+='</tbody></table></div>';body.innerHTML=html;}
function _renderModalLiveMobile(body,snapshots,market){var is1x2=(market==='1x2');var ouLine=is1x2?'':market.replace('ou','');var sels=is1x2?['1','X','2']:['U','O'];var selLabels=is1x2?['1','X','2']:[_t('app.tbl.alt','ALT')+' '+ouLine,_t('app.tbl.over','ÜST')+' '+ouLine];var html='<div class="mlive-wrap">';for(var i=0;i<snapshots.length;i++){var s=snapshots[i];var prevS=(i<snapshots.length-1)?snapshots[i+1]:null;var minVal=s.minute;var minStr='-';if(minVal!==''&&minVal!==null&&minVal!==undefined){var mv=String(minVal);minStr=mv.endsWith("'")?mv:mv+"'";}
