...
async function loadChartWithData(home,away,market,league,dataOverride){return loadChart(home,away,market,league,dataOverride);}
async function loadChart(home,away,market,league='',dataOverride=null){try{if(typeof Chart==='undefined'){console.error('[loadChart] Chart.js not loaded, retrying...');try{await window.loadChartLibs();registerChartPlugins();}catch(e2){console.error('[loadChart] Chart.js retry failed:',e2);return;}}
let data={history:[]};if(dataOverride){data=dataOverride;console.log('[loadChart] Using direct data override for',market,':',(data.history||[]).length,'rows');}else{const cacheKey=`${home}|${away}|${league}|${currentSource}`;if(bulkHistoryCacheKey===cacheKey&&bulkHistoryCache[market]){data=bulkHistoryCache[market];console.log('[loadChart] Using bulk cache for',market);}else{try{const response=await fetch(`/api/match/history?home=${encodeURIComponent(home)}&away=${encodeURIComponent(away)}&market=${market}&league=${encodeURIComponent(league || '')}`);data=await response.json();}catch(e){console.log('No history data available');}}}
...
async function loadChartForSource(home,away,market,league){if(chartOddsSource==='matchbook'){var cacheKey=`${home}|${away}|${league}|mb`;if(_mbBulkCacheKey!==cacheKey){var params=`home=${encodeURIComponent(home)}&away=${encodeURIComponent(away)}&league=${encodeURIComponent(league || '')}`;if(selectedMatch&&selectedMatch.match_id){params+=`&arb_hash=${encodeURIComponent(selectedMatch.match_id)}`;}
...
var mktData=_mbBulkCache[market];var mbHistory=(mktData&&mktData.history)?mktData.history:[];console.log('[ChartSource] Matchbook data for',market,':',mbHistory.length,'rows');loadChartWithData(home,away,market,league,{history:mbHistory});}else{bulkHistoryCache={};bulkHistoryCacheKey='';var reqId=++_modalRequestId;var marketsPromise=loadAllMarketsAtOnce(home,away,league,selectedMatch?selectedMatch.match_id:'');await marketsPromise;loadChart(home,away,market,league);}}
