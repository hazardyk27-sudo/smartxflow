...
var minExtra='';if(/^HT$/i.test(matchMin)||/^MS$/i.test(matchMin))minExtra=' is-ht';else if(/\d+\+/.test(matchMin))minExtra=' is-extra';var goalBadgeHtml=goalSide?'<div class="goal-badge">GOL!</div>':'';var minCell='<td class="match-date">'+'<div class="dk-cell-inner">'+
goalBadgeHtml+'<div class="date-line live-min-badge'+minExtra+'">'+escLiveHtml(matchMin)+'</div>'+
...
html+='<div class="odds-card-volume">'+formatLiveVol(vol)+'</div>';html+='</div>';html+='<div class="odds-card-meta"><span>'+escLiveHtml(m.league)+'</span><span class="meta-sep">\u2022</span>';var cardMin=_calcLiveMatchMin(m);if(goalSide){html+='<span class="goal-badge">GOL!</span> ';}
html+='<span class="live-min-badge" style="font-size:11px;">'+cardMin+'</span>';var cardScore=m.score?escLiveHtml(m.score):(cardMin==='MS'?'-':'');if(cardScore){var csp=cardScore.split('-');if(csp.length===2&&goalSide){var chCls=goalSide==='home'||goalSide==='both'?' goal-pop':'';var caCls=goalSide==='away'||goalSide==='both'?' goal-pop':'';html+='<span class="meta-sep">\u2022</span><span class="score-h'+chCls+'" style="font-weight:600;">'+csp[0].trim()+'</span><span class="score-sep" style="font-weight:600;">-</span><span class="score-a'+caCls+'" style="font-weight:600;">'+csp[1].trim()+'</span>';}else{html+='<span class="meta-sep">\u2022</span><span style="color:#f59e0b;font-weight:600;">'+cardScore+'</span>';}}
