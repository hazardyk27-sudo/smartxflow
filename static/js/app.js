...
function _liveMobileCard(m,marketOrLine,onclickAttr,goalSide){var is1x2=(marketOrLine==='1x2');var cardCls='match-card odds-card moneyway-card'+(goalSide?' goal-flash-card':'');var _lmk=_getMatchKey(m);var _lmkIsFav=_userFavorites.has(_lmk);var _lmkFc=_favCounts[_lmk]||(_lmkIsFav?1:0);var html='<div class="'+cardCls+'" data-matchkey="'+_lmk.replace(/"/g,'&quot;')+'" '+onclickAttr+'>';html+=_mobileFavLine(_lmk,_lmkIsFav,_lmkFc);html+='<div class="odds-card-header">';var cardMin=_calcLiveMatchMin(m);var cardScore=m.score?escLiveHtml(m.score):(cardMin==='MS'?'-':'');var liveCapHtml='';if(cardMin||cardScore){var minDisp=goalSide?'GOL!':cardMin;var scoreDisp='';if(cardScore){var csp=cardScore.split('-');if(csp.length===2&&goalSide){var chCls=goalSide==='home'||goalSide==='both'?' lc-scored':'';var caCls=goalSide==='away'||goalSide==='both'?' lc-scored':'';scoreDisp='<span class="lc-score-h'+chCls+'">'+csp[0].trim()+'</span><span class="lc-score-sep">-</span><span class="lc-score-a'+caCls+'">'+csp[1].trim()+'</span>';}else{scoreDisp='<span class="lc-score-val">'+cardScore+'</span>';}}
