...
let previousOddsData=null;let modalOddsData=null;function _setupNormalModalAccordion(home,away){setAccordionState('chart',true);setAccordionState('alarms',true);var liveHash=_findLiveHash(home,away);if(liveHash){_liveDetailHash=liveHash;document.getElementById('accordionLive').style.display='';setAccordionState('live',false);}else{document.getElementById('accordionLive').style.display='none';}}
async function openMatchModalFromMatches(index){if(index>=0&&index<matches.length){const reqId=++_modalRequestId;resetModalState();selectedMatch=matches[index];selectedChartMarket=currentMarket;document.getElementById('modalMatchTitle').textContent=`${selectedMatch.home_team} vs ${selectedMatch.away_team}`;const headerDt=toTurkeyTime(selectedMatch.date);let headerDateText=selectedMatch.date||'';if(headerDt&&headerDt.isValid()){headerDateText=headerDt.format('DD.MM HH:mm');}
document.getElementById('modalLeague').textContent=`${selectedMatch.league || ''} • ${headerDateText}`;document.querySelectorAll('#modalChartTabs .chart-tab').forEach(t=>{t.classList.remove('active');if(t.dataset.market===currentMarket){t.classList.add('active');}});const home=selectedMatch.home_team;const away=selectedMatch.away_team;const league=selectedMatch.league||'';_setupNormalModalAccordion(home,away);_accordionChartLoaded=true;_accordionAlarmsLoaded=true;document.getElementById('modalOverlay').classList.add('active');const chartLibsPromise=window.loadChartLibs?window.loadChartLibs().then(()=>registerChartPlugins()):Promise.resolve();const marketsPromise=loadAllMarketsAtOnce(home,away,league);const chartPipeline=Promise.all([marketsPromise,chartLibsPromise]).then(()=>{if(reqId!==_modalRequestId)return;return loadChartWithTrends(home,away,selectedChartMarket,league,reqId);}).catch(e=>console.error('[Modal] Chart pipeline error:',e));const alarmsPipeline=renderMatchAlarmsSection(home,away,reqId);await Promise.all([chartPipeline,alarmsPipeline]);}}
async function openMatchModalFromAPI(homeTeam,awayTeam,league,kickoff){const reqId=++_modalRequestId;resetModalState();league=league||'';kickoff=kickoff||'';selectedMatch={home_team:homeTeam,away_team:awayTeam,league:league,date:kickoff,match_id:'',history:[],history_count:0,odds:null,details:null};selectedChartMarket=currentMarket;document.getElementById('modalMatchTitle').textContent=`${homeTeam} vs ${awayTeam}`;const headerDt=kickoff?toTurkeyTime(kickoff):null;let headerDateText='';if(headerDt&&headerDt.isValid()){headerDateText=headerDt.format('DD.MM HH:mm');}
document.getElementById('modalLeague').textContent=league?`${league}${headerDateText ? ' • ' + headerDateText : ''}`:(headerDateText||'');document.querySelectorAll('#modalChartTabs .chart-tab').forEach(t=>{t.classList.remove('active');if(t.dataset.market===currentMarket){t.classList.add('active');}});_setupNormalModalAccordion(homeTeam,awayTeam);_accordionChartLoaded=true;_accordionAlarmsLoaded=true;document.getElementById('modalOverlay').classList.add('active');const chartLibsPromise=window.loadChartLibs?window.loadChartLibs().then(()=>registerChartPlugins()):Promise.resolve();const marketsPromise=loadAllMarketsAtOnce(homeTeam,awayTeam,league);try{const chartPipeline=Promise.all([marketsPromise,chartLibsPromise]).then(()=>{if(reqId!==_modalRequestId)return;return loadChartWithTrends(homeTeam,awayTeam,selectedChartMarket,league,reqId);}).catch(e=>console.error('[Modal] Chart pipeline error:',e));const alarmsPipeline=renderMatchAlarmsSection(homeTeam,awayTeam,reqId);await Promise.all([chartPipeline,alarmsPipeline]);}catch(error){console.error('[openMatchModalFromAPI] Error:',error);showToast('Maç bilgisi alınırken hata oluştu','error');}}
...
currentChartHistoryData=[];modalOddsData=null;previousOddsData=null;bulkHistoryCache={};bulkHistoryCacheKey='';_accordionChartLoaded=false;_accordionAlarmsLoaded=false;const card=document.getElementById('matchInfoCard');if(card)card.innerHTML='';const grid=document.getElementById('smartMoneyGrid');if(grid){grid.innerHTML='';grid.style.display='none';}
const empty=document.getElementById('smartMoneyEmpty');if(empty)empty.style.display='none';const canvas=document.getElementById('oddsChart');if(canvas){const ctx=canvas.getContext('2d');ctx.clearRect(0,0,canvas.width,canvas.height);}
_liveDetailData=null;_liveDetailHash=null;var liveBody=document.getElementById('liveDetailBody');if(liveBody)liveBody.innerHTML='';var liveTabs=document.getElementById('liveDetailTabs');if(liveTabs)liveTabs.innerHTML='';var liveSection=document.getElementById('accordionLive');if(liveSection)liveSection.style.display='none';showSmartMoneyLoading();showChartLoading();}
...
var _accordionState={chart:true,alarms:true,live:false};var _accordionChartLoaded=false;var _accordionAlarmsLoaded=false;function setAccordionState(section,open){_accordionState[section]=open;var bodyId,chevronId;if(section==='chart'){bodyId='accordionChartBody';chevronId='accordionChartChevron';}
else if(section==='alarms'){bodyId='accordionAlarmsBody';chevronId='accordionAlarmsChevron';}
else if(section==='live'){bodyId='accordionLiveBody';chevronId='accordionLiveChevron';}
else return;var body=document.getElementById(bodyId);var chevron=document.getElementById(chevronId);if(body)body.style.display=open?'':'none';if(chevron)chevron.textContent=open?'\u25BC':'\u25B2';if(open)_lazyLoadAccordion(section);}
function toggleAccordion(section){setAccordionState(section,!_accordionState[section]);}
function _lazyLoadAccordion(section){if(!selectedMatch)return;var home=selectedMatch.home_team;var away=selectedMatch.away_team;var league=selectedMatch.league||'';var reqId=_modalRequestId;if(section==='chart'&&!_accordionChartLoaded){_accordionChartLoaded=true;var chartLibsPromise=window.loadChartLibs?window.loadChartLibs().then(function(){registerChartPlugins();}):Promise.resolve();var marketsPromise=loadAllMarketsAtOnce(home,away,league);Promise.all([marketsPromise,chartLibsPromise]).then(function(){if(reqId!==_modalRequestId)return;return loadChartWithTrends(home,away,selectedChartMarket||currentMarket,league,reqId);}).catch(function(e){console.error('[Modal] Chart pipeline error:',e);});}
if(section==='alarms'&&!_accordionAlarmsLoaded){_accordionAlarmsLoaded=true;renderMatchAlarmsSection(home,away,reqId);}
if(section==='live'&&_liveDetailHash&&!_liveDetailData){_liveDetailMarket=_liveMarket;document.getElementById('liveDetailBody').innerHTML='<div class="live-loading"><div class="loading-spinner"></div>Periyot verileri yukleniyor...</div>';fetch('/api/live/match/history?hash='+encodeURIComponent(_liveDetailHash)).then(function(resp){return resp.json();}).then(function(data){if(reqId!==_modalRequestId)return;_liveDetailData=data.snapshots||[];_buildDetailTabs(_liveDetailData);renderLiveDetail(_liveDetailData,_liveDetailMarket);}).catch(function(e){if(reqId!==_modalRequestId)return;document.getElementById('liveDetailBody').innerHTML='<div class="live-loading">Veri yuklenemedi</div>';});}}
function _findLiveHash(home,away){if(!_liveData||_liveData.length===0)return null;var h=(home||'').toLowerCase().trim();var a=(away||'').toLowerCase().trim();for(var i=0;i<_liveData.length;i++){var m=_liveData[i];var mh=(m.home_team||'').toLowerCase().trim();var ma=(m.away_team||'').toLowerCase().trim();if(mh===h&&ma===a)return m.match_id_hash;if(h.indexOf(mh)!==-1||mh.indexOf(h)!==-1){if(a.indexOf(ma)!==-1||ma.indexOf(a)!==-1){return m.match_id_hash;}}}
return null;}
...
document.addEventListener('click',function(e){const dropdown=document.querySelector('.mob-time-dropdown');const menu=document.getElementById('mobTimeMenu');if(dropdown&&menu&&!dropdown.contains(e.target)){menu.classList.remove('open');}});function closeModal(){++_modalRequestId;document.getElementById('modalOverlay').classList.remove('active');_liveDetailData=null;_liveDetailHash=null;_accordionChartLoaded=false;_accordionAlarmsLoaded=false;var liveBody=document.getElementById('liveDetailBody');if(liveBody)liveBody.innerHTML='';var liveTabs=document.getElementById('liveDetailTabs');if(liveTabs)liveTabs.innerHTML='';var liveSection=document.getElementById('accordionLive');if(liveSection)liveSection.style.display='none';setAccordionState('chart',true);setAccordionState('alarms',true);setAccordionState('live',false);const chartTooltip=document.getElementById('chartjs-tooltip');if(chartTooltip){chartTooltip.style.opacity=0;chartTooltip.style.visibility='hidden';chartTooltip.style.display='none';}
...
let cachedAllAlarms=null;let smartMoneySectionOpen=true;async function loadAllAlarmsOnce(forceRefresh=false){const data=await fetchAlarmsBatch(forceRefresh);cachedAllAlarms=getCachedAlarmsWithType();return cachedAllAlarms;}
...
async function renderMatchAlarmsSection(homeTeam,awayTeam,reqId){const section=document.getElementById('accordionAlarms');const grid=document.getElementById('smartMoneyGrid');const empty=document.getElementById('smartMoneyEmpty');if(!section){hideSmartMoneyLoading();return;}
if(!cachedAllAlarms||cachedAllAlarms.length===0){await loadAllAlarmsOnce(false);}
if(reqId&&reqId!==_modalRequestId)return;const matchAlarms=getMatchAlarms(homeTeam,awayTeam);hideSmartMoneyLoading();grid.style.display='grid';if(matchAlarms.length===0){grid.style.display='none';empty.style.display='block';return;}
...
document.addEventListener('DOMContentLoaded',function(){initLicenseCheck();});document.getElementById('licenseKeyInput')?.addEventListener('keypress',function(e){if(e.key==='Enter')validateWebLicense();});var _liveMode=false;var _liveInterval=null;var _liveData=[];var _liveMarket='1x2';var _liveDetailData=null;var _liveDetailMarket='1x2';var _liveDetailHash=null;function switchToLive(){_liveMode=true;document.querySelectorAll('.tab').forEach(function(t){t.classList.remove('active');});document.querySelectorAll('.mobile-tab-btn').forEach(function(t){t.classList.remove('active');});var liveTab=document.querySelector('.tab-live');if(liveTab)liveTab.classList.add('active');var mobileLive=document.querySelector('.mobile-live-btn');if(mobileLive)mobileLive.classList.add('active');var liveToolbar=document.getElementById('liveToolbar');if(liveToolbar)liveToolbar.style.display='flex';var marketRow=document.querySelector('.mobile-tab-row.market-row');if(marketRow)marketRow.style.display='none';loadLiveMatches();if(_liveInterval)clearInterval(_liveInterval);_liveInterval=setInterval(loadLiveMatches,30000);}
function switchFromLive(){_liveMode=false;if(_liveInterval){clearInterval(_liveInterval);_liveInterval=null;}
var liveToolbar=document.getElementById('liveToolbar');if(liveToolbar)liveToolbar.style.display='none';var marketRow=document.querySelector('.mobile-tab-row.market-row');if(marketRow)marketRow.style.display='';var thead=document.querySelector('.matches-table thead');if(thead&&!thead.querySelector('tr')){thead.innerHTML='<tr></tr>';}
...
String(now.getSeconds()).padStart(2,'0');}catch(e){console.error('Live matches error:',e);}}
...
var filtered=matches.filter(function(m){if(is1x2)return _liveCalcVol1x2(m)>0;return _liveCalcVolOU(_liveGetOu(m,ouLine))>0;});if(filtered.length===0){if(tbody)tbody.innerHTML='<tr><td colspan="7" style="text-align:center;padding:40px;">'+emptyHtml+'</td></tr>';if(cardList)cardList.innerHTML=emptyHtml;var mc0=document.getElementById('matchCount');if(mc0)mc0.textContent='0';var lmc0=document.getElementById('liveMatchCount');if(lmc0)lmc0.textContent='0';return;}
var sorted=filtered.slice();sorted.sort(function(a,b){var va=is1x2?_liveCalcVol1x2(a):_liveCalcVolOU(_liveGetOu(a,ouLine));var vb=is1x2?_liveCalcVol1x2(b):_liveCalcVolOU(_liveGetOu(b,ouLine));return vb-va;});var rowsHtml='';var cardsHtml='';for(var i=0;i<sorted.length;i++){var m=sorted[i];var o=m.odds||{};var matchMin=_calcLiveMatchMin(m);var scoreStr=m.score?escLiveHtml(m.score):'';var onclickAttr='onclick="openLiveDetail(\''+m.match_id_hash+'\',\''+escLiveHtml(m.home_team)+'\',\''+escLiveHtml(m.away_team)+'\')"';var showScore=(matchMin==='MS')?(scoreStr||'-'):scoreStr;var minCell='<td class="match-date">'+'<div class="date-line live-min-badge">'+escLiveHtml(matchMin)+'</div>'+
(showScore?'<div class="time-line live-score-badge">'+showScore+'</div>':'')+'</td>';if(is1x2){var vol=_liveCalcVol1x2(m);var b1=_liveMwBlock('1',o['1']);var bX=_liveMwBlock('X',o['X']);var b2=_liveMwBlock('2',o['2']);rowsHtml+='<tr '+onclickAttr+'>'+
minCell+'<td class="match-league" title="'+escLiveHtml(m.league)+'">'+escLiveHtml(m.league)+'</td>'+'<td class="match-teams">'+escLiveHtml(m.home_team)+'<span class="vs">-</span>'+escLiveHtml(m.away_team)+'</td>'+'<td class="mw-outcomes-cell" colspan="3"><div class="mw-grid mw-grid-3">'+b1+bX+b2+'</div></td>'+'<td class="volume-cell">'+formatLiveVol(vol)+'</td></tr>';cardsHtml+=_liveMobileCard(m,'1x2',onclickAttr);}else{var ou=_liveGetOu(m,ouLine);var volOU=_liveCalcVolOU(ou);var bU=_liveMwBlock('Alt',ou['U']);var bO=_liveMwBlock('Ust',ou['O']);rowsHtml+='<tr '+onclickAttr+'>'+
minCell+'<td class="match-league" title="'+escLiveHtml(m.league)+'">'+escLiveHtml(m.league)+'</td>'+'<td class="match-teams">'+escLiveHtml(m.home_team)+'<span class="vs">-</span>'+escLiveHtml(m.away_team)+'</td>'+'<td class="mw-outcomes-cell" colspan="2"><div class="mw-grid mw-grid-2">'+bU+bO+'</div></td>'+'<td class="volume-cell">'+formatLiveVol(volOU)+'</td></tr>';cardsHtml+=_liveMobileCard(m,ouLine,onclickAttr);}}
...
html+='<div class="odds-card-volume">'+formatLiveVol(vol)+'</div>';html+='</div>';html+='<div class="odds-card-meta"><span>'+escLiveHtml(m.league)+'</span><span class="meta-sep">\u2022</span>';var cardMin=_calcLiveMatchMin(m);html+='<span class="live-min-badge" style="font-size:11px;">'+cardMin+'</span>';var cardScore=m.score?escLiveHtml(m.score):(cardMin==='MS'?'-':'');if(cardScore)html+='<span class="meta-sep">\u2022</span><span style="color:#f59e0b;font-weight:600;">'+cardScore+'</span>';html+='</div>';if(is1x2){var o=m.odds||{};html+='<div class="odds-card-row three">';html+=_liveMobileBlock('1',o['1']);html+=_liveMobileBlock('X',o['X']);html+=_liveMobileBlock('2',o['2']);html+='</div>';}else{var ouD=_liveGetOu(m,marketOrLine);html+='<div class="odds-card-row two">';html+=_liveMobileBlock('Alt',ouD['U']);html+=_liveMobileBlock('Ust',ouD['O']);html+='</div>';}
...
async function openLiveDetail(hash,home,away){var reqId=++_modalRequestId;resetModalState();_liveDetailHash=hash;selectedMatch={home_team:home||'--',away_team:away||'--',league:'',date:'',match_id:hash,history:[],history_count:0,odds:null,details:null};selectedChartMarket=currentMarket;document.getElementById('modalMatchTitle').textContent=(home||'--')+' vs '+(away||'--');document.getElementById('modalLeague').textContent='Canl\u0131 Ma\u00e7';document.getElementById('accordionLive').style.display='';setAccordionState('live',true);setAccordionState('chart',false);setAccordionState('alarms',false);document.getElementById('liveDetailBody').innerHTML='<div class="live-loading"><div class="loading-spinner"></div>Periyot verileri yukleniyor...</div>';document.getElementById('modalOverlay').classList.add('active');_liveDetailMarket=_liveMarket;var livePromise=fetch('/api/live/match/history?hash='+encodeURIComponent(hash)).then(function(resp){return resp.json();}).then(function(data){if(reqId!==_modalRequestId)return;_liveDetailData=data.snapshots||[];_buildDetailTabs(_liveDetailData);renderLiveDetail(_liveDetailData,_liveDetailMarket);}).catch(function(e){if(reqId!==_modalRequestId)return;document.getElementById('liveDetailBody').innerHTML='<div class="live-loading">Veri yuklenemedi</div>';});await livePromise;}
...
var is1x2=(market==='1x2');var ouLine=is1x2?'':market.replace('ou','');var sels=is1x2?['1','X','2']:['U','O'];var selLabels=is1x2?['1','X','2']:[escLiveHtml(ouLine)+' ALT',escLiveHtml(ouLine)+' \u00dcST'];var colsPerSel=3;var html='<div class="live-detail-table-wrap"><table class="live-period-table">';html+='<thead>';html+='<tr class="lpt-group-row">';html+='<th rowspan="2" class="lpt-dual-th"><span class="lpt-th-main">DK</span><span class="lpt-th-sub">SKOR</span></th>';for(var g=0;g<sels.length;g++){html+='<th colspan="'+colsPerSel+'" class="lpt-group-th">'+selLabels[g]+'</th>';}
html+='</tr>';html+='<tr class="lpt-col-row">';for(var h=0;h<sels.length;h++){html+='<th class="lpt-dual-th"><span class="lpt-th-main">ORAN</span><span class="lpt-th-sub">Oran de\u011fi\u015fim</span></th>';html+='<th class="lpt-dual-th"><span class="lpt-th-main">VOLUME</span><span class="lpt-th-sub">Gelen para</span></th>';html+='<th class="lpt-dual-th"><span class="lpt-th-main">Y\u00dcZDE</span><span class="lpt-th-sub">%de\u011fi\u015fimi</span></th>';}
html+='</tr>';html+='</thead><tbody>';var totals={};sels.forEach(function(sel){totals[sel]=0;});for(var i=0;i<snapshots.length;i++){var s=snapshots[i];var prevS=i>0?snapshots[i-1]:null;var minVal=s.minute;var minStr=(minVal!==''&&minVal!==null&&minVal!==undefined)?minVal+"'":'-';var scoreStr=s.score||'-';html+='<tr>';html+='<td class="period-dk-skor"><span class="dk-val">'+escLiveHtml(minStr)+'</span><span class="skor-val">'+escLiveHtml(scoreStr)+'</span></td>';var dataGroup=is1x2?(s['1x2']||{}):((s.ou_lines||{})[ouLine]||{});var prevDataGroup=prevS?(is1x2?(prevS['1x2']||{}):((prevS.ou_lines||{})[ouLine]||{})):null;for(var j=0;j<sels.length;j++){var sel=sels[j];var sd=dataGroup[sel]||{};var odds=(sd.odds!==undefined&&sd.odds!==null)?parseFloat(sd.odds):null;var pct=sd.share||0;var vol=sd.volume||0;totals[sel]+=vol;var oddsStr=odds!==null?odds.toFixed(2):'-';var pctStr=pct?pct.toFixed(0)+'%':'-';var deltaHtml='<span class="period-delta-neutral">-</span>';if(prevDataGroup&&odds!==null){var prevOdds=(prevDataGroup[sel]||{}).odds;if(prevOdds!==undefined&&prevOdds!==null){var delta=odds-parseFloat(prevOdds);if(Math.abs(delta)>=0.005){var dSign=delta>0?'+':'';var dCls=delta>0?'period-delta-up':'period-delta-down';deltaHtml='<span class="'+dCls+'">'+dSign+delta.toFixed(2)+'</span>';}}}
