...
function renderLiveMatches(matches){var tbody=document.getElementById('liveTableBody');var thead=document.getElementById('liveTableHead');var cardList=document.getElementById('liveCardList');var emptyHtml='<div class="live-empty">'+'<div class="live-empty-icon">\u26BD</div>'+'<div class="live-empty-text">Su an canli mac bulunamadi</div>'+'<div class="live-empty-sub">Canli maclar basladiginda burada gorunecek</div>'+'</div>';if(!matches||matches.length===0){tbody.innerHTML='<tr><td colspan="7" style="text-align:center;padding:40px;">'+emptyHtml+'</td></tr>';thead.innerHTML='';cardList.innerHTML=emptyHtml;return;}
var is1x2=(_liveMarket==='1x2');var ouLine=is1x2?'':_liveMarket.replace('ou','');if(is1x2){thead.innerHTML='<tr><th class="col-live-min">DK</th><th class="col-league">LIG</th><th class="col-match">MAC</th>'+'<th class="col-selection">1</th><th class="col-selection">X</th><th class="col-selection">2</th>'+'<th class="col-volume">MIKTAR</th></tr>';}else{thead.innerHTML='<tr><th class="col-live-min">DK</th><th class="col-league">LIG</th><th class="col-match">MAC</th>'+'<th class="col-selection" colspan="2"><span style="opacity:0.6">A/U '+escLiveHtml(ouLine)+'</span></th>'+'<th class="col-volume">MIKTAR</th></tr>';}
var rowsHtml='';var cardsHtml='';for(var i=0;i<matches.length;i++){var m=matches[i];var o=m.odds||{};var matchMin=_calcLiveMatchMin(m);var onclickAttr='onclick="openLiveDetail(\''+m.match_id_hash+'\',\''+escLiveHtml(m.home_team)+'\',\''+escLiveHtml(m.away_team)+'\')"';if(is1x2){var vol=_liveCalcVol1x2(m);var b1=_liveMwBlock('1',o['1']);var bX=_liveMwBlock('X',o['X']);var b2=_liveMwBlock('2',o['2']);rowsHtml+='<tr '+onclickAttr+'>'+'<td class="live-min-cell"><span class="live-min-badge">'+matchMin+'</span>'+
(m.score?'<span class="live-score-badge">'+escLiveHtml(m.score)+'</span>':'')+'</td>'+'<td class="match-league" title="'+escLiveHtml(m.league)+'">'+escLiveHtml(m.league)+'</td>'+'<td class="match-teams">'+escLiveHtml(m.home_team)+'<span class="vs">-</span>'+escLiveHtml(m.away_team)+'</td>'+'<td class="mw-outcomes-cell" colspan="3"><div class="mw-grid mw-grid-3">'+b1+bX+b2+'</div></td>'+'<td class="volume-cell">'+formatLiveVol(vol)+'</td></tr>';cardsHtml+=_liveMobileCard(m,'1x2',onclickAttr);}else{var ou=_liveGetOu(m,ouLine);var volOU=_liveCalcVolOU(ou);var bU=_liveMwBlock('Alt',ou['U']);var bO=_liveMwBlock('Ust',ou['O']);rowsHtml+='<tr '+onclickAttr+'>'+'<td class="live-min-cell"><span class="live-min-badge">'+matchMin+'</span>'+
...
function _liveMwBlock(label,d){if(!d)return'<div class="mw-outcome-block"><div class="mw-info-stack"><div class="mw-odds">-</div></div><div class="mw-donut">'+_liveDonut(0,44)+'</div></div>';var pct=d.share||0;var odds=d.odds?d.odds.toFixed(2):'-';var money=d.volume?formatLiveVol(d.volume):'';return'<div class="mw-outcome-block">'+'<div class="mw-info-stack">'+'<div class="mw-odds">'+odds+'</div>'+
(money?'<div class="mw-money">'+money+'</div>':'')+'</div>'+'<div class="mw-donut">'+_liveDonut(pct,44)+'</div>'+'</div>';}
