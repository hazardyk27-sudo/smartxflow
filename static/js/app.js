...
function _updateModalFavBtn(){var btn=document.getElementById('modalFavBtn');if(!btn)return;if(!selectedMatch){btn.classList.remove('fav-active');return;}var mk=_getMatchKey(selectedMatch);var isFav=_userFavorites.has(mk);btn.setAttribute('data-matchkey',mk);if(isFav){btn.classList.add('fav-active');}else{btn.classList.remove('fav-active');}}
function toggleModalFavorite(){var btn=document.getElementById('modalFavBtn');if(!btn||!selectedMatch)return;var mk=_getMatchKey(selectedMatch);var did=getWebDeviceId();fetch('/api/favorite/toggle',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({match_key:mk,device_id:did})}).then(function(r){return r.json();}).then(function(data){if(data.error){console.warn('[Fav] error:',data.error);return;}if(data.favorited){_userFavorites.add(mk);}else{_userFavorites.delete(mk);}_updateModalFavBtn();document.querySelectorAll('.fav-heart').forEach(function(h){if(h.getAttribute('data-matchkey')===mk){if(data.favorited){h.classList.add('fav-active');}else{h.classList.remove('fav-active');}}}); _favCounts[mk]=data.total_count||0;}).catch(function(){});}
