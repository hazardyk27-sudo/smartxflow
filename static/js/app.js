let currentMarket='moneyway_1x2';let matches=[];let filteredMatches=[];let chart=null;let selectedMatch=null;let selectedChartMarket='moneyway_1x2';let autoScrapeRunning=false;let currentSortColumn='volume';let currentSortDirection='desc';let chartVisibleSeries={};let dateFilterMode='ALL';let chartTimeRange='10min';let currentChartHistoryData=[];let chartViewMode='percent';let isClientMode=true;let _modalRequestId=0;let _licenseReadyResolve;const _licenseReady=new Promise(r=>{_licenseReadyResolve=r;});let _isLicensed=false;function getWebDeviceId(){let did=localStorage.getItem('smartxflow_device_id');if(!did||did.length>16){did='w-'+crypto.randomUUID().replace(/-/g,'').substring(0,14);localStorage.setItem('smartxflow_device_id',did);}
return did;}
...
function renderMobileMatchCards(data){const cardList=document.getElementById('matchCardList');if(!cardList)return;if(data.length===0){cardList.innerHTML=`
        ...
        `;return;}
const isDropping=currentMarket.startsWith('dropping');const isMoneyway=currentMarket.startsWith('moneyway');const html=data.map((match,idx)=>{const d=match.details||match.odds||{};const volume=formatVolumeCompact(d.Volume);let dateStr='';try{const dt=dayjs(match.date).tz('Europe/Istanbul');const day=dt.format('D');const monthIdx=dt.month();const monthsTR=['Oca','Şub','Mar','Nis','May','Haz','Tem','Ağu','Eyl','Eki','Kas','Ara'];const time=dt.format('HH:mm');dateStr=`${day} ${monthsTR[monthIdx]} ${time}`;}catch(e){dateStr=match.date||'';}
if(isMoneyway){return renderMobileMoneywayCard(match,idx,d,volume,dateStr);}
if(isDropping){return renderMobileOddsCard(match,idx,d,volume,dateStr);}
return`
...
function renderMobileMoneywayCard(match,idx,d,volume,dateStr){let oddsBlocks='';if(currentMarket.includes('1x2')){oddsBlocks=`
...
document.getElementById('modalLeague').textContent=`${selectedMatch.league || ''} • ${headerDateText}`;document.querySelectorAll('#modalChartTabs .chart-tab').forEach(t=>{t.classList.remove('active');if(t.dataset.market===currentMarket){t.classList.add('active');}});document.getElementById('modalOverlay').classList.add('active');const home=selectedMatch.home_team;const away=selectedMatch.away_team;const league=selectedMatch.league||'';const chartLibsPromise=window.loadChartLibs?window.loadChartLibs().then(()=>registerChartPlugins()):Promise.resolve();const alarmsPromise=fetchAlarmsBatch();const marketsPromise=loadAllMarketsAtOnce(home,away,league);await Promise.all([marketsPromise,alarmsPromise,chartLibsPromise]);if(reqId!==_modalRequestId)return;await Promise.all([loadChartWithTrends(home,away,selectedChartMarket,league,reqId),renderMatchAlarmsSection(home,away,reqId)]);}}
...
updateMatchInfoCard();loadChart(home,away,market,league);hideChartLoading();}catch(e){console.error('Error loading chart with trends:',e);if(reqId&&reqId!==_modalRequestId)return;loadChart(home,away,market,league);hideChartLoading();}}
...
async function loadChart(home,away,market,league=''){try{let data={history:[]};try{const response=await fetch(`/api/match/history?home=${encodeURIComponent(home)}&away=${encodeURIComponent(away)}&market=${market}&league=${encodeURIComponent(league || '')}`);data=await response.json();}catch(e){console.log('No history data available');}
...
await loadAllAlarmsOnce(true);if(reqId&&reqId!==_modalRequestId)return;const matchAlarms=getMatchAlarms(homeTeam,awayTeam);hideSmartMoneyLoading();smartMoneySectionOpen=true;chevron.textContent='▼';grid.style.display='grid';if(matchAlarms.length===0){grid.style.display='none';empty.style.display='block';return;}
