...
const _originalFetch=window.fetch;window.fetch=function(url,options={}){const urlStr=typeof url==='string'?url:url.url||'';return _originalFetch.call(window,url,options).then(function(response){if(response.status===403&&urlStr.startsWith('/api/')){response.clone().json().then(function(data){if(data&&data.error==='LOGIN_REQUIRED'){window.location.href='/login?next='+encodeURIComponent(window.location.pathname);}else if(data&&data.error==='EMAIL_NOT_VERIFIED'){window.location.href='/verify-email';}else if(data&&data.error==='MEMBERSHIP_REQUIRED'){window.location.href='/membership-required';}}).catch(function(){});}
