...
function _updateFavCountsInDOM() {
    ...
    document.querySelectorAll('.mobile-fav-count').forEach(function(el) {
        ...
        if (mk) {
            var c = _favCounts[mk] || 0;
            el.textContent = c + ' kişi takip ediyor';
        }
    });
}
...
function _favCountHtml(count) {
    ...
    return '<span class="fav-count-num">' + count + ' kişi</span>takip ediyor';
}
...
function _mobileFavLine(mk, isFav, count) {
    var txt = (count > 0 ? count : 0) + ' kişi takip ediyor';
...
async function openMatchModalFromAPI(homeTeam, awayTeam, league, kickoff) {
    ...
    if (prematchFound) {
        ...
        if (!kickoff && prematchFound.date) apiKickoff = prematchFound.date;
...
function filterAlarmsByMatchDate(alarms) {
    if (currentAlarmDateFilter === 'all') {
        ...
        const yesterday = dayjs().tz('Europe/Istanbul').subtract(1, 'day').format('YYYY-MM-DD');
        return alarms.filter(alarm => {
            const matchDateStr = getMatchDateFromAlarm(alarm);
            if (!matchDateStr) return true; // Tarih yoksa göster
            return matchDateStr >= yesterday;
        });
    }
