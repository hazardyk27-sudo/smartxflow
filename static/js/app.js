...
return true;});var _blurStyle='filter:blur(6px);-webkit-filter:blur(6px);user-select:none;pointer-events:none';var bannerRowHtml='<tr class="live-pro-banner-row"><td colspan="8">'+_liveProBanner()+'</td></tr>';var rowsHtml=bannerRowHtml;var cardsHtml=_liveProBanner();for(var i=0;i<filtered.length;i++){var m=filtered[i];var o=m.odds||{};var matchMin=_calcLiveMatchMin(m);var scoreStr=m.score?escLiveHtml(m.score):'';var _liveFavTd=_buildLiveFavCell(m);var capHtml='<div class="live-capsule live-capsule-desk">'+'<span class="lc-dot"></span>'+'<span class="lc-min">'+escLiveHtml(matchMin)+'</span>';if(scoreStr){var sp=scoreStr.split('-');if(sp.length===2){capHtml+='<span class="lc-divider"></span>'+'<span class="lc-score-h">'+sp[0].trim()+'</span>'+'<span class="lc-score-sep">-</span>'+'<span class="lc-score-a">'+sp[1].trim()+'</span>';}else{capHtml+='<span class="lc-divider"></span><span class="lc-score-val">'+escLiveHtml(scoreStr)+'</span>';}}
