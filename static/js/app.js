...
if(tbody)tbody.innerHTML=rowsHtml;if(cardList)cardList.innerHTML=cardsHtml;var mc=document.getElementById('matchCount');if(mc)mc.textContent=sorted.length;}
...
var pctDelta=pct-prevPct;var pctDeltaHtml='<span class="period-delta-neutral">\u00b7</span>';if(prevS&&Math.abs(pctDelta)>0){var pdSign=pctDelta>0?'+':'';pctDeltaHtml='<span class="pct-delta">'+pdSign+pctDelta.toFixed(0)+'%</span>';}
...
snapshots=snapshots.slice().sort(function(a,b){var am=_minuteToNum(a.minute),bm=_minuteToNum(b.minute);if(am!==bm)return bm-am;return(a.snapshot_at||'')>(b.snapshot_at||'')?-1:1;});var is1x2=(market==='1x2');var ouLine=is1x2?'':market.replace('ou','');var sels=is1x2?['1','X','2']:['U','O'];var selLabels=is1x2?['1','X','2']:[escLiveHtml(ouLine)+' ALT',escLiveHtml(ouLine)+' \u00dcST'];var colsPerSel=3;var html='<div class="live-detail-table-wrap"><table class="live-period-table">';html+='<colgroup><col class="col-dk">';for(var cg=0;cg<sels.length;cg++){html+='<col class="col-odds"><col class="col-vol"><col class="col-pct">';}
html+='</colgroup>';html+='<thead>';html+='<tr class="lpt-group-row">';html+='<th rowspan="2" class="lpt-dual-th"><span class="lpt-th-main">DK</span><span class="lpt-th-sub">SKOR</span></th>';for(var g=0;g<sels.length;g++){html+='<th colspan="'+colsPerSel+'" class="lpt-group-th">'+selLabels[g]+'</th>';}
html+='</tr>';html+='<tr class="lpt-col-row">';for(var h=0;h<sels.length;h++){html+='<th class="lpt-dual-th"><span class="lpt-th-main">ORAN</span><span class="lpt-th-sub">Oran de\u011fi\u015fim</span></th>';html+='<th class="lpt-dual-th"><span class="lpt-th-main">VOLUME</span><span class="lpt-th-sub">Gelen para</span></th>';html+='<th class="lpt-dual-th"><span class="lpt-th-main">Y\u00dcZDE</span><span class="lpt-th-sub">%de\u011fi\u015fimi</span></th>';}
...
var scoreStr=s.score||'-';html+='<tr>';html+='<td class="period-dk-skor"><span class="dk-val">'+escLiveHtml(minStr)+'</span><span class="skor-val">'+escLiveHtml(scoreStr)+'</span></td>';var dataGroup=is1x2?(s['1x2']||{}):((s.ou_lines||{})[ouLine]||{});var prevDataGroup=prevS?(is1x2?(prevS['1x2']||{}):((prevS.ou_lines||{})[ouLine]||{})):null;for(var j=0;j<sels.length;j++){var sel=sels[j];var sd=dataGroup[sel]||{};var odds=(sd.odds!==undefined&&sd.odds!==null)?parseFloat(sd.odds):null;var pct=sd.share||0;var vol=sd.volume||0;var oddsStr=odds!==null?odds.toFixed(2):'-';var pctStr=pct?pct.toFixed(0)+'%':'-';var deltaHtml='<span class="period-delta-neutral">\u00b7</span>';if(prevDataGroup&&odds!==null){var prevOdds=(prevDataGroup[sel]||{}).odds;if(prevOdds!==undefined&&prevOdds!==null){var delta=odds-parseFloat(prevOdds);if(Math.abs(delta)>=0.005){var dSign=delta>0?'+':'';var dCls=delta>0?'period-delta-up':'period-delta-down';deltaHtml='<span class="'+dCls+'">'+dSign+delta.toFixed(2)+'</span>';}}}
var prevPct=0;if(prevDataGroup){prevPct=(prevDataGroup[sel]||{}).share||0;}
var pctDelta=pct-prevPct;var pctDeltaHtml='<span class="period-delta-neutral">\u00b7</span>';if(prevS&&Math.abs(pctDelta)>0){var pdSign=pctDelta>0?'+':'';pctDeltaHtml='<span class="pct-delta">'+pdSign+pctDelta.toFixed(0)+'%</span>';}
var prevVol=0;if(prevDataGroup){prevVol=(prevDataGroup[sel]||{}).volume||0;}
var volDiff=vol-prevVol;var volDiffStr='\u00b7';var volDiffHasVal=false;if(prevS&&volDiff!==0){volDiffStr=formatLiveVol(volDiff);volDiffHasVal=true;}
