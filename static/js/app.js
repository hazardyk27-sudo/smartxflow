...
var is1x2=(market==='1x2');var ouLine=is1x2?'':market.replace('ou','');var html='<div class="live-detail-table-wrap"><table class="live-period-table"><thead><tr>';html+='<th>#</th><th>Dakika</th>';if(is1x2){html+='<th>1</th><th>X</th><th>2</th>';}else{html+='<th>Alt '+escLiveHtml(ouLine)+'</th><th>Ust '+escLiveHtml(ouLine)+'</th>';}
html+='<th>Hacim</th><th>Degisim</th>';html+='</tr></thead><tbody>';var prevVol=0;for(var i=0;i<snapshots.length;i++){var s=snapshots[i];var minVal=s.minute;var minStr='';if(minVal!==''&&minVal!==null&&minVal!==undefined){minStr=minVal+"'";}
html+='<tr>';html+='<td class="period-num">'+(i+1)+'</td>';html+='<td class="period-time">'+minStr+'</td>';var vol=0;if(is1x2){var o=s['1x2']||{};['1','X','2'].forEach(function(sel){var sd=o[sel]||{};var pct=sd.share||0;var odds=sd.odds?sd.odds.toFixed(2):'-';var hi=pct>=50;html+='<td class="period-odds-cell">'+'<span class="period-odds">'+odds+'</span>'+'<span class="period-pct '+(hi?'pct-high':'pct-low')+'">'+(pct?pct.toFixed(0)+'%':'-')+'</span>'+'</td>';vol+=sd.volume||0;});}else{var ols=s.ou_lines||{};var ouD=ols[ouLine]||{};['U','O'].forEach(function(sel){var sd=ouD[sel]||{};var pct=sd.share||0;var odds=sd.odds?sd.odds.toFixed(2):'-';var hi=pct>=50;html+='<td class="period-odds-cell">'+'<span class="period-odds">'+odds+'</span>'+'<span class="period-pct '+(hi?'pct-high':'pct-low')+'">'+(pct?pct.toFixed(0)+'%':'-')+'</span>'+'</td>';vol+=sd.volume||0;});}
html+='<td class="period-vol">'+formatLiveVol(vol)+'</td>';if(i===0){html+='<td class="period-vol-change">-</td>';}else{var vc=vol-prevVol;var cls=vc>=0?'period-vol-up':'period-vol-down';var sign=vc>=0?'+':'';html+='<td class="period-vol-change '+cls+'">'+sign+formatLiveVol(Math.abs(vc))+'</td>';}
html+='</tr>';prevVol=vol;}
