...
var _t=window._t;if(typeof dayjs!=='undefined'){if(typeof dayjs_plugin_utc!=='undefined')dayjs.extend(dayjs_plugin_utc);if(typeof dayjs_plugin_timezone!=='undefined')dayjs.extend(dayjs_plugin_timezone);if(typeof dayjs_plugin_customParseFormat!=='undefined')dayjs.extend(dayjs_plugin_customParseFormat);}
...
function translateMarket(market){if(!market)return'';const m=String(market).toUpperCase();if(m==='OU25'||m==='O/U 2.5'||m==='OU 2.5')return'Ü/A 2.5';if(m==='BTTS')return _t('app.j.btts','KG');if(m==='1X2')return'1X2';return market;}
...
if(prematchFound){apiHome=prematchFound.home_team||homeTeam;apiAway=prematchFound.away_team||awayTeam;if(!league&&prematchFound.league)apiLeague=prematchFound.league;var _kHasTime=/T\d{2}:\d{2}|\s\d{2}:\d{2}/.test(String(kickoff||''));if((!kickoff||!_kHasTime)&&prematchFound.date)apiKickoff=prematchFound.date;console.log('[ModalFromAPI] Prematch found:',apiHome,'vs',apiAway,'league:',apiLeague);}else{console.log('[ModalFromAPI] No prematch match for',homeTeam,'vs',awayTeam,'in filtered:',fArr2.length,'all:',mArr2.length);}
