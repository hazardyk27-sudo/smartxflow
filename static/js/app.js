...
if(selectedMatch){showChartLoading();loadChartWithTrends(selectedMatch.home_team,selectedMatch.away_team,selectedChartMarket,selectedMatch.league||'');var arbH=selectedMatch.arbworld_hash||'';loadMatchbookChartForModal(selectedMatch.home_team,selectedMatch.away_team,selectedMatch.league||'',arbH,selectedChartMarket);}});});}
...
function setSource(src){if(currentSource===src)return;currentSource=src;document.querySelectorAll('.mf-chip[data-group="source"]').forEach(function(c){c.classList.toggle('active',c.getAttribute('data-value')===src);});document.querySelectorAll('.source-toggle-btn').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-source')===src);});_matchesMarketCache={};loadMatches();}
function applyMobileFilter(){var dateChip=document.querySelector('.mf-chip[data-group="date"].active');var sortChip=document.querySelector('.mf-chip[data-group="sort"].active');var dateVal=dateChip?dateChip.getAttribute('data-value'):'all';var sortVal=sortChip?sortChip.getAttribute('data-value'):'date';_mobileHideEnded=!!document.querySelector('.mf-sw[data-value="hideEnded"].active');_mobileHideLive=!!document.querySelector('.mf-sw[data-value="hideLive"].active');_mobileOnlyLive=!!document.querySelector('.mf-sw[data-value="onlyLive"].active');var sourceChip=document.querySelector('.mf-chip[data-group="source"].active');var newSource=sourceChip?sourceChip.getAttribute('data-value'):'betfair';if(newSource!==currentSource){currentSource=newSource;_matchesMarketCache={};}
var hasFilter=(dateVal!=='all')||_mobileHideEnded||_mobileHideLive||_mobileOnlyLive||(sortVal!=='date')||(currentSource!=='betfair');var btn=document.getElementById('mobileFilterBtn');if(btn)btn.classList.toggle('has-filter',hasFilter);var dBtn=document.getElementById('desktopFilterBtn');if(dBtn)dBtn.classList.toggle('has-filter',hasFilter);closeMobileFilterModal();if(sortVal==='volume'){currentSortColumn='volume';currentSortDirection='desc';}else{currentSortColumn='date';currentSortDirection='desc';}
...
function filterMatches(query){let filtered=[...matches];filtered=filtered.filter(m=>hasValidMarketData(m,currentMarket));if(query){filtered=filtered.filter(m=>m.home_team.toLowerCase().includes(query)||m.away_team.toLowerCase().includes(query)||(m.league&&m.league.toLowerCase().includes(query)));}
