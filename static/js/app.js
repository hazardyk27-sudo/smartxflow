...
function setMobileTimeRange(range){mobileTimeRange=range;const rangeMap={'10':'10min','30':'30min','60':'1hour','360':'6hour','720':'12hour','1440':'1day','all':'1day'};chartTimeRange=rangeMap[range]||'1day';const labels={'all':_t('app.dyn.tumu','Tümü'),'10':_t('app.dyn.10_dakika','10 dk'),'30':_t('app.dyn.30_dakika','30 dk'),'60':_t('app.dyn.1_saat','1 saat'),'360':_t('app.dyn.6_saat','6 saat'),'720':_t('app.dyn.12_saat','12 saat'),'1440':_t('app.dyn.1_gun','1 gün')};const mobTimeLabel=document.getElementById('mobTimeLabel');if(mobTimeLabel)mobTimeLabel.textContent=labels[range]||'Tümü';document.querySelectorAll('.mob-time-option').forEach(opt=>{opt.classList.toggle('active',opt.dataset.range===range);});const mobTimeMenu=document.getElementById('mobTimeMenu');if(mobTimeMenu)mobTimeMenu.classList.remove('open');document.querySelectorAll('.time-pill').forEach(pill=>{pill.classList.toggle('active',pill.dataset.range===range);});if(selectedMatch&&isMobile()){loadChartHistory(selectedMatch.MatchId,selectedChartMarket);}}
