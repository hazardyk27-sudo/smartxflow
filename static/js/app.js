...
var is1x2=(market==='1x2');var ouLine=is1x2?'':market.replace('ou','');var sels=is1x2?['1','X','2']:['U','O'];var selLabels=is1x2?['1','X','2']:[escLiveHtml(ouLine)+' ALT',escLiveHtml(ouLine)+' \u00dcST'];var html='<div class="live-detail-table-wrap"><table class="live-period-table">';html+='<thead>';html+='<tr class="lpt-group-row">';html+='<th colspan="2"></th>';for(var g=0;g<sels.length;g++){html+='<th colspan="3" class="lpt-group-th">'+selLabels[g]+'</th>';}
html+='</tr>';html+='<tr class="lpt-sub-row">';html+='<th>DK</th><th>SKOR</th>';for(var h=0;h<sels.length;h++){html+='<th>ORAN</th><th>Gelen Para</th><th>Y\u00fczde</th>';}
html+='</tr>';html+='</thead><tbody>';var totals={};sels.forEach(function(sel){totals[sel]=0;});for(var i=0;i<snapshots.length;i++){var s=snapshots[i];var prevS=i>0?snapshots[i-1]:null;var minVal=s.minute;var minStr=(minVal!==''&&minVal!==null&&minVal!==undefined)?minVal+"'":'-';var scoreStr=s.score||'-';html+='<tr>';html+='<td class="period-time">'+escLiveHtml(minStr)+'</td>';html+='<td class="period-score">'+escLiveHtml(scoreStr)+'</td>';var dataGroup=is1x2?(s['1x2']||{}):((s.ou_lines||{})[ouLine]||{});var prevDataGroup=prevS?(is1x2?(prevS['1x2']||{}):((prevS.ou_lines||{})[ouLine]||{})):null;for(var j=0;j<sels.length;j++){var sel=sels[j];var sd=dataGroup[sel]||{};var odds=(sd.odds!==undefined&&sd.odds!==null)?parseFloat(sd.odds):null;var pct=sd.share||0;var vol=sd.volume||0;totals[sel]+=vol;var oddsStr=odds!==null?odds.toFixed(2):'-';var pctStr=pct?pct.toFixed(0)+'%':'-';var deltaHtml='';if(prevDataGroup&&odds!==null){var prevOdds=(prevDataGroup[sel]||{}).odds;if(prevOdds!==undefined&&prevOdds!==null){var delta=odds-parseFloat(prevOdds);if(Math.abs(delta)>=0.005){var dSign=delta>0?'+':'';var dCls=delta>0?'period-delta-up':'period-delta-down';deltaHtml=' <span class="'+dCls+'">'+dSign+delta.toFixed(2)+'</span>';}}}
var hi=pct>=50;html+='<td class="period-odds">'+oddsStr+deltaHtml+'</td>';html+='<td class="period-vol">'+(vol?formatLiveVol(vol):'-')+'</td>';html+='<td class="period-pct '+(hi?'pct-high':'pct-low')+'">'+pctStr+'</td>';}
html+='</tr>';}
html+='<tr class="lpt-totals-row">';html+='<td colspan="2" class="lpt-totals-label">Toplam</td>';for(var k=0;k<sels.length;k++){html+='<td colspan="2"></td>';html+='<td class="period-vol lpt-total-vol">'+formatLiveVol(totals[sels[k]])+'</td>';}
