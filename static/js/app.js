...
document.addEventListener('DOMContentLoaded',function(){initLicenseCheck();});document.getElementById('licenseKeyInput')?.addEventListener('keypress',function(e){if(e.key==='Enter')validateWebLicense();});var _liveMode=false;var _liveInterval=null;var _liveData=[];var _liveMarket='1x2';var _liveDetailData=null;var _liveDetailMarket='1x2';function switchToLive(){_liveMode=true;document.querySelectorAll('.tab').forEach(function(t){t.classList.remove('active');});document.querySelectorAll('.mobile-tab-btn').forEach(function(t){t.classList.remove('active');});var liveTab=document.querySelector('.tab-live');if(liveTab)liveTab.classList.add('active');var mobileLive=document.querySelector('.mobile-live-btn');if(mobileLive)mobileLive.classList.add('active');document.querySelector('.table-container').style.display='none';var mobileCards=document.getElementById('matchCardList');if(mobileCards)mobileCards.style.display='none';document.getElementById('liveContainer').style.display='block';var marketRow=document.querySelector('.mobile-tab-row.market-row');if(marketRow)marketRow.style.display='none';loadLiveMatches();if(_liveInterval)clearInterval(_liveInterval);_liveInterval=setInterval(loadLiveMatches,30000);}
function switchFromLive(){_liveMode=false;if(_liveInterval){clearInterval(_liveInterval);_liveInterval=null;}
document.getElementById('liveContainer').style.display='none';document.querySelector('.table-container').style.display='';var mobileCards=document.getElementById('matchCardList');if(mobileCards)mobileCards.style.display='';var marketRow=document.querySelector('.mobile-tab-row.market-row');if(marketRow)marketRow.style.display='';}
...
function renderLiveMatches(matches){var tbody=document.getElementById('liveTableBody');var thead=document.getElementById('liveTableHead');var colgroup=document.getElementById('liveTableColgroup');var table=document.getElementById('liveTable');var cardList=document.getElementById('liveCardList');var emptyHtml='<div class="live-empty">'+'<div class="live-empty-icon">\u26BD</div>'+'<div class="live-empty-text">Su an canli mac bulunamadi</div>'+'<div class="live-empty-sub">Canli maclar basladiginda burada gorunecek</div>'+'</div>';if(!matches||matches.length===0){tbody.innerHTML='<tr><td colspan="7" style="text-align:center;padding:40px;">'+emptyHtml+'</td></tr>';thead.innerHTML='';cardList.innerHTML=emptyHtml;return;}
var is1x2=(_liveMarket==='1x2');var ouLine=is1x2?'':_liveMarket.replace('ou','');if(is1x2){table.setAttribute('data-selection-count','3');if(colgroup)colgroup.innerHTML='<col class="col-date"><col class="col-league"><col class="col-match">'+'<col class="col-selection"><col class="col-selection"><col class="col-selection"><col class="col-volume">';thead.innerHTML='<tr>'+'<th class="col-date">DK</th>'+'<th class="col-league">LİG</th>'+'<th class="col-match">MAÇ</th>'+'<th class="col-selection">1</th>'+'<th class="col-selection">X</th>'+'<th class="col-selection">2</th>'+'<th class="col-volume">HACİM</th>'+'</tr>';}else{table.setAttribute('data-selection-count','2');if(colgroup)colgroup.innerHTML='<col class="col-date"><col class="col-league"><col class="col-match">'+'<col class="col-selection"><col class="col-selection"><col class="col-volume">';thead.innerHTML='<tr>'+'<th class="col-date">DK</th>'+'<th class="col-league">LİG</th>'+'<th class="col-match">MAÇ</th>'+'<th class="col-selection">A/U '+escLiveHtml(ouLine)+'</th>'+'<th class="col-selection"></th>'+'<th class="col-volume">HACİM</th>'+'</tr>';}
var rowsHtml='';var cardsHtml='';for(var i=0;i<matches.length;i++){var m=matches[i];var o=m.odds||{};var matchMin=_calcLiveMatchMin(m);var scoreStr=m.score?escLiveHtml(m.score):'';var onclickAttr='onclick="openLiveDetail(\''+m.match_id_hash+'\',\''+escLiveHtml(m.home_team)+'\',\''+escLiveHtml(m.away_team)+'\')"';var minCell='<td class="match-date">'+'<div class="date-line live-min-badge">'+escLiveHtml(matchMin)+'</div>'+
...
tbody.innerHTML=rowsHtml;cardList.innerHTML=cardsHtml;}
