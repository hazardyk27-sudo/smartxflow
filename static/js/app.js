...
function _buildFavCell(match){var mk=_getMatchKey(match);var isFav=_userFavorites.has(mk);var count=_favCounts[mk]||0;if(count===0&&isFav)count=1;return'<td class="fav-cell"><span class="fav-heart '+(isFav?'fav-active':'')+'" data-matchkey="'+mk.replace(/"/g,'&quot;')+'" onclick="event.stopPropagation(); toggleFavorite(this);">\u2665</span><span class="fav-count">'+_favCountHtml(count)+'</span></td>';}
...
function _liveMobileCard(m,marketOrLine,onclickAttr,goalSide){var is1x2=(marketOrLine==='1x2');var cardCls='match-card odds-card moneyway-card'+(goalSide?' goal-flash-card':'');var _lmk=_getMatchKey(m);var html='<div class="'+cardCls+'" data-matchkey="'+_lmk.replace(/"/g,'&quot;')+'" '+onclickAttr+'>';html+='<div class="odds-card-header">';var cardMin=_calcLiveMatchMin(m);var cardScore=m.score?escLiveHtml(m.score):(cardMin==='MS'?'-':'');var liveCapHtml='';if(cardMin||cardScore){var minDisp=goalSide?'GOL!':cardMin;var scoreDisp='';if(cardScore){var csp=cardScore.split('-');if(csp.length===2&&goalSide){var chCls=goalSide==='home'||goalSide==='both'?' lc-scored':'';var caCls=goalSide==='away'||goalSide==='both'?' lc-scored':'';scoreDisp='<span class="lc-score-h'+chCls+'">'+csp[0].trim()+'</span><span class="lc-score-sep">-</span><span class="lc-score-a'+caCls+'">'+csp[1].trim()+'</span>';}else{scoreDisp='<span class="lc-score-val">'+cardScore+'</span>';}}
