...
function _getModalFinishedCapsule(match){if(!match||!match.date)return'';var mt=toTurkeyTime(match.date);if(!mt||!mt.isValid())return'';var diff=nowTurkey().diff(mt,'minute');if(diff<80)return'';var lmExact=_findLiveMatchExact(match.home_team,match.away_team);if(_isFinishedLiveMatch(lmExact)){return' <span class="lc-ended" style="margin-left:8px;font-size:13px;vertical-align:middle;">BİTTİ '+(lmExact.score||'')+'</span>';}
var ft=_findFinishedScore(match.home_team,match.away_team,match.match_id);if(ft){return' <span class="lc-ended" style="margin-left:8px;font-size:13px;vertical-align:middle;">BİTTİ '+(ft.score||'')+'</span>';}
return'';}
...
selectedMatch={home_team:apiHome,away_team:apiAway,league:apiLeague,date:apiKickoff,match_id:prematchFound?(prematchFound.match_id||''):'',history:[],history_count:0,odds:null,details:null};selectedChartMarket=currentMarket;var modalTitleHtml=escapeHtml(homeTeam)+' vs '+escapeHtml(awayTeam);var modalFtCapsule=_getModalFinishedCapsule(selectedMatch);document.getElementById('modalMatchTitle').innerHTML=modalTitleHtml+modalFtCapsule;const headerDt=apiKickoff?toTurkeyTime(apiKickoff):null;let headerDateText='';if(headerDt&&headerDt.isValid()){headerDateText=headerDt.format('DD.MM HH:mm');}
