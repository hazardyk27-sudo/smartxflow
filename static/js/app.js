...
async function openLiveDetail(hash,home,away){var reqId=++_modalRequestId;resetModalState();_liveDetailHash=hash;document.getElementById('modalMatchTitle').textContent=(home||'--')+' vs '+(away||'--');document.getElementById('modalLeague').textContent='Canl\u0131 Ma\u00e7';document.getElementById('mobileTabLiveBtn').style.display='';switchMobileTab('live');document.getElementById('liveDetailBody').innerHTML='<div class="live-loading"><div class="loading-spinner"></div>Periyot verileri yukleniyor...</div>';document.getElementById('modalOverlay').classList.add('active');_liveDetailMarket=_liveMarket;try{var resp=await fetch('/api/live/match/history?hash='+encodeURIComponent(hash));var data=await resp.json();if(reqId!==_modalRequestId)return;_liveDetailData=data.snapshots||[];_buildDetailTabs(_liveDetailData);renderLiveDetail(_liveDetailData,_liveDetailMarket);}catch(e){if(reqId!==_modalRequestId)return;document.getElementById('liveDetailBody').innerHTML='<div class="live-loading">Veri yuklenemedi</div>';}}
...
function closeLiveDetail(){closeModal();}
