...
setInterval(()=>{if(_isLicensed)loadAlertBand();},120000);document.addEventListener('DOMContentLoaded',async()=>{await _licenseReady;if(!_isLicensed)return;setTimeout(loadAlertBand,500);});let currentAlarmFilter='all';let allAlarmsData=[];let groupedAlarmsData=[];let alarmsDataByType={sharp:[],bigmoney:[],volumeshock:[],dropping:[],publicmove:[],volumeleader:[],mim:[]};let alarmSearchQuery='';let alarmsDisplayCount=10;let alarmsSidebarOpen=false;let openAlarmId=null;var _mobMarkets={moneyway:[{v:'1x2',l:'1X2'},{v:'ou25',l:'2.5'},{v:'btts',l:'KG'}],dropping:[{v:'1x2',l:'1X2'},{v:'ou25',l:'2.5'},{v:'btts',l:'KG'}],live:[{v:'1x2',l:'1X2'},{v:'ou1.5',l:'1.5'},{v:'ou2.5',l:'2.5'},{v:'ou3.5',l:'3.5'},{v:'ou4.5',l:'4.5'}]};var _mobCurCat='moneyway';var _mobCurMkt='1x2';function toggleMobDD(which){var wrap=document.getElementById(which==='cat'?'mobCatWrap':'mobMktWrap');var other=document.getElementById(which==='cat'?'mobMktWrap':'mobCatWrap');if(!wrap)return;if(other)other.classList.remove('open');wrap.classList.toggle('open');if(wrap.classList.contains('open')){setTimeout(function(){document.addEventListener('click',function _cl(e){if(!wrap.contains(e.target)){wrap.classList.remove('open');}
document.removeEventListener('click',_cl);});},10);}}
function selectMobCat(cat){_mobCurCat=cat;var valEl=document.getElementById('mobCatVal');var labels={moneyway:'Moneyway',dropping:'Oran',live:'Canlı'};if(valEl)valEl.textContent=labels[cat]||cat;document.querySelectorAll('#mobCatMenu .mob-dd-opt').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-cat')===cat);});document.getElementById('mobCatWrap').classList.remove('open');_updateMobMktOptions(cat);if(cat==='live'){switchToLive();}else{setMobileGroup(cat);}}
function _updateMobMktOptions(cat){var menu=document.getElementById('mobMktMenu');if(!menu)return;var opts=_mobMarkets[cat]||_mobMarkets.moneyway;var html='';opts.forEach(function(o,i){html+='<button class="mob-dd-opt'+(i===0?' active':'')+'" data-mkt="'+o.v+'" onclick="selectMobMkt(\''+o.v+'\')">'+o.l+'</button>';});menu.innerHTML=html;var valEl=document.getElementById('mobMktVal');if(valEl)valEl.textContent=opts[0].l;_mobCurMkt=opts[0].v;if(cat==='live'){setLiveMarket(opts[0].v);}else{setMobileMarket(opts[0].v);}}
function selectMobMkt(mkt){_mobCurMkt=mkt;var menu=document.getElementById('mobMktMenu');if(menu){menu.querySelectorAll('.mob-dd-opt').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-mkt')===mkt);});var sel=menu.querySelector('.mob-dd-opt[data-mkt="'+mkt+'"]');var valEl=document.getElementById('mobMktVal');if(sel&&valEl)valEl.textContent=sel.textContent.trim();}
document.getElementById('mobMktWrap').classList.remove('open');if(_mobCurCat==='live'){setLiveMarket(mkt);}else{setMobileMarket(mkt);}}
function openMobSearch(){var wrap=document.getElementById('mobSearchWrap');if(wrap){wrap.style.display='block';var inp=document.getElementById('mobSearchInput');if(inp){inp.value=document.getElementById('searchInput').value||'';inp.focus();}}}
function closeMobSearch(){var wrap=document.getElementById('mobSearchWrap');if(wrap)wrap.style.display='none';var inp=document.getElementById('mobSearchInput');if(inp){inp.value='';document.getElementById('searchInput').value='';document.getElementById('searchInput').dispatchEvent(new Event('input'));}}
