...
async function _openLiveInMainModal(hash,home,away){var match=null;var matchIdx=-1;var dataSource=(typeof filteredMatches!=='undefined'&&filteredMatches.length>0)?filteredMatches:((typeof matches!=='undefined')?matches:[]);var homeLow=(home||'').toLowerCase().trim();var awayLow=(away||'').toLowerCase().trim();for(var i=0;i<dataSource.length;i++){var mh=(dataSource[i].home_team||'').toLowerCase().trim();var ma=(dataSource[i].away_team||'').toLowerCase().trim();if(mh===homeLow&&ma===awayLow){match=dataSource[i];matchIdx=i;break;}
if((mh.includes(homeLow)||homeLow.includes(mh))&&(ma.includes(awayLow)||awayLow.includes(ma))){if(!match){match=dataSource[i];matchIdx=i;}}}
if(match&&matchIdx>=0){await openMatchModal(matchIdx);}else{await openMatchModalFromAPI(home,away,'','');}
