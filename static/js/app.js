if (typeof window._t !== 'function') {
    window._t = function (k, fb) {
        try {
            var v = (window.SXFI18n && window.SXFI18n.t) ? window.SXFI18n.t(k) : null;
            return (v && v !== k) ? v : fb;
        } catch (e) { return fb; }
    };
}
...
if (typeof dayjs !== 'undefined') {
    ...
    if (typeof dayjs_plugin_customParseFormat !== 'undefined') dayjs.extend(dayjs_plugin_customParseFormat);
}

window._chartLibsLoaded = false;
window._chartLibsLoading = false;
window.loadChartLibs = function() {
    return new Promise(function(resolve) {
        if (window._chartLibsLoaded && typeof Chart !== 'undefined') return resolve();
        if (window._chartLibsLoading && !window._chartLibsRetry) {
            var check = setInterval(function() {
                if (window._chartLibsLoaded) { clearInterval(check); resolve(); }
            }, 50);
            return;
        }
        window._chartLibsLoading = true;
        window._chartLibsRetry = false;
        var coreScripts = [
        ...
        var zoomScript = 'https://cdn.jsdelivr.net/npm/chartjs-plugin-zoom@2.0.1/dist/chartjs-plugin-zoom.min.js';
        var failed = [];
        function loadScript(src) {
            return new Promise(function(res) {
                ...
                s.onerror = function() { failed.push(src); res(false); };
                document.head.appendChild(s);
            });
        }
        loadScript(coreScripts[0]).then(function() {
            var parallel = pluginScripts.map(function(src) { return loadScript(src); });
            parallel.push(loadScript(zoomScript));
            return Promise.all(parallel);
        }).then(function() {
            if (typeof Chart !== 'undefined') {
                window._chartLibsLoaded = true;
            } else {
                console.error('[ChartLibs] Chart.js failed to load. Failed scripts:', failed);
                window._chartLibsLoaded = false;
                window._chartLibsLoading = false;
                window._chartLibsRetry = true;
            }
            resolve();
        });
    });
...
let _analysisMatchHashes = [];
let _mobileHideEnded = false;
let _mobileHideLive = false;
let _mobileOnlyLive = false;
let _analysisHashesFetched = false;
let currentSource = 'betfair';
...
function loadUserFavorites() {
    ...
    return fetch('/api/favorites?device_id=' + encodeURIComponent(did))
...
function loadFavoriteCounts() {
    return fetch('/api/favorite/all-counts')
    .then(function(r) {
        if (!r.ok) throw new Error('HTTP ' + r.status);
        return r.json();
    })
    .then(function(data) {
        console.log('[Fav] Loaded counts:', Object.keys(data.counts || {}).length, 'matches');
        _favCounts = data.counts || {};
        _updateFavCountsInDOM();
    })
    .catch(function(err) { console.error('[Fav] loadFavoriteCounts error:', err); });
}

function _updateFavCountsInDOM() {
    document.querySelectorAll('.fav-heart').forEach(function(el) {
        var mk = el.getAttribute('data-matchkey');
        var c = _favCounts[mk] || 0;
        var countEl = el.parentElement.querySelector('.fav-count');
        if (countEl && mk) {
            countEl.innerHTML = _favCountHtml(c);
        }
    });
    document.querySelectorAll('.mobile-fav-count').forEach(function(el) {
        var mk = el.getAttribute('data-matchkey');
        if (mk) {
            var c = _favCounts[mk] || 0;
            el.textContent = c + (window.SXFI18n ? window.SXFI18n.t('app.dyn.kisi') + ' ' + window.SXFI18n.t('app.dyn.takip_ediyor') : ' kişi takip ediyor');
        }
    });
}
...
function toggleFavorite(el) {
    ...
    if (!mk) return;
    var did = getWebDeviceId();
    console.log('[Fav] toggle:', mk, 'device:', did);
    fetch('/api/favorite/toggle', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({match_key: mk, device_id: did})
    })
    .then(function(r) { console.log('[Fav] response status:', r.status); return r.json(); })
    .then(function(data) {
        if (data.error) { console.warn('[Fav] error:', data.error); return; }
        console.log('[Fav] result:', data.favorited ? 'ADDED' : 'REMOVED', 'count:', data.total_count);
        if (data.favorited) {
        ...
        document.querySelectorAll('.fav-heart').forEach(function(h) {
            if (h.getAttribute('data-matchkey') === mk) {
                if (data.favorited) {
                    h.classList.add('fav-active');
                } else {
                    h.classList.remove('fav-active');
                }
            }
        });
        ...
        document.querySelectorAll('.mobile-fav-count').forEach(function(mc) {
            if (mc.getAttribute('data-matchkey') === mk) {
                mc.textContent = (displayCount > 0 ? displayCount : 0) + (window.SXFI18n ? window.SXFI18n.t('app.dyn.kisi') + ' ' + window.SXFI18n.t('app.dyn.takip_ediyor') : ' kişi takip ediyor');
            }
        });
        if (_favFilterActive && !data.favorited) {
...
function toggleFavoritesFilter() {
    ...
    var btnM = document.getElementById('favoritesBtnMobile');
    if (_favFilterActive) {
        if (btn) btn.classList.add('btn-favorites-active');
        if (btnM) btnM.classList.add('btn-favorites-active');
        _loadAndApplyFavoritesFilter();
    } else {
        if (btn) btn.classList.remove('btn-favorites-active');
        if (btnM) btnM.classList.remove('btn-favorites-active');
        _clearFavoritesFilter();
    }
}

async function _loadAndApplyFavoritesFilter() {
    if (_userFavorites.size === 0) {
        _applyFavoritesFilter();
        return;
    }

    var renderedKeys = new Set();
    document.querySelectorAll('#matchesTableBody tr').forEach(function(row) {
        var mk = row.getAttribute('data-matchkey');
        if (!mk) { var h = row.querySelector('.fav-heart'); if (h) mk = h.getAttribute('data-matchkey'); }
        if (mk) renderedKeys.add(mk);
    });
    document.querySelectorAll('#matchCardList .match-card').forEach(function(card) {
        var mk = card.getAttribute('data-matchkey');
        if (!mk) { var h = card.querySelector('.fav-heart'); if (h) mk = h.getAttribute('data-matchkey'); }
        if (mk) renderedKeys.add(mk);
    });

    var missingFavs = [];
    _userFavorites.forEach(function(mk) {
        if (!renderedKeys.has(mk)) missingFavs.push(mk);
    });
    ...
    try {
        var did = getWebDeviceId();
        var mkt = (typeof currentMarket !== 'undefined') ? currentMarket : 'moneyway_1x2';
        var srcParam = (typeof currentSource !== 'undefined' && currentSource !== 'betfair') ? '&source=' + encodeURIComponent(currentSource) : '';
        var url = '/api/favorites-matches?device_id=' + encodeURIComponent(did || '') + '&market=' + encodeURIComponent(mkt) + srcParam;
        var resp = await fetch(url);
        var data = await resp.json();
        var extraMatches = data.matches || [];

        if (extraMatches.length > 0) {
            var missingSet = new Set(missingFavs);
            var existingKeys = new Set();
            (matches || []).forEach(function(m) {
                existingKeys.add((m.home_team || '') + '|' + (m.away_team || '') + '|' + (m.league || ''));
            });
            var toAdd = extraMatches.filter(function(m) {
                var mk = (m.home_team || '') + '|' + (m.away_team || '') + '|' + (m.league || '');
                return missingSet.has(mk) && !existingKeys.has(mk);
            });
            if (toAdd.length > 0) {
                matches = (matches || []).concat(toAdd);
        ...
        console.error('[Fav] Error loading past favorites:', e);
    }

    _applyFavoritesFilter();
}

function _applyFavoritesFilter() {
    document.querySelectorAll('#matchesTableBody tr').forEach(function(row) {
        var mk = row.getAttribute('data-matchkey');
        if (!mk) {
            var heart = row.querySelector('.fav-heart');
            if (heart) mk = heart.getAttribute('data-matchkey');
        }
        if (mk) {
            row.style.display = _userFavorites.has(mk) ? '' : 'none';
        }
    });
    document.querySelectorAll('#matchCardList .match-card').forEach(function(card) {
        var mk = card.getAttribute('data-matchkey');
        if (!mk) {
            var heart = card.querySelector('.fav-heart');
            if (heart) mk = heart.getAttribute('data-matchkey');
        }
        if (mk) {
            card.style.display = _userFavorites.has(mk) ? '' : 'none';
        }
    });
}

function _clearFavoritesFilter() {
    document.querySelectorAll('#matchesTableBody tr').forEach(function(row) {
        row.style.display = '';
    });
    document.querySelectorAll('#matchCardList .match-card').forEach(function(card) {
...
function _favCountHtml(count) {
    if (!count || count <= 0) return '';
    return '<span class="fav-count-num">' + count + (window.SXFI18n ? window.SXFI18n.t('app.dyn.kisi') : ' kişi') + '</span>' + (window.SXFI18n ? window.SXFI18n.t('app.dyn.takip_ediyor') : 'takip ediyor');
}

var _heartSvg = '<svg viewBox="0 0 24 24" width="1em" height="1em" fill="currentColor"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>';

function _buildLiveFavCell(match) {
    var mk = _getMatchKey(match);
    var isFav = _userFavorites.has(mk);
    var count = _favCounts[mk] || 0;
    return '<td class="fav-cell"><span class="fav-heart ' + (isFav ? 'fav-active' : '') + '" data-matchkey="' + mk.replace(/"/g, '&quot;') + '" onclick="event.stopPropagation(); toggleFavorite(this);">' + _heartSvg + '</span><span class="fav-count">' + _favCountHtml(count) + '</span></td>';
}

function _buildFavCell(match) {
    var mk = _getMatchKey(match);
    var isFav = _userFavorites.has(mk);
    var count = _favCounts[mk] || 0;
    return '<td class="fav-cell"><span class="fav-heart ' + (isFav ? 'fav-active' : '') + '" data-matchkey="' + mk.replace(/"/g, '&quot;') + '" onclick="event.stopPropagation(); toggleFavorite(this);">' + _heartSvg + '</span><span class="fav-count">' + _favCountHtml(count) + '</span></td>';
...
function showLicenseExpiredOverlay() {
    ...
    if (overlay) {
        ...
        overlay.innerHTML = '<div style="background:#1c1f23;border:1px solid #2e3238;border-radius:16px;padding:48px 40px;max-width:440px;width:90%;text-align:center;">'
    ...
    return _originalFetch.call(window, url, options).then(function(response) {
        if (response.status === 403 && urlStr.startsWith('/api/')) {
            response.clone().json().then(function(data) {
                if (data && (data.error === 'LICENSE_EXPIRED' || data.error === 'LICENSE_REVOKED' || data.error === 'LICENSE_REQUIRED')) {
                    showLicenseExpiredOverlay();
                }
            }).catch(function() {});
        }
...
let _allFilteredMatches = [];
let _renderedCount = 0;
const _RENDER_BATCH = 20;
...
function translateMarket(market) {
    ...
    if (m === 'BTTS') return _t('app.j.btts', 'KG');
    if (m === '1X2') return '1X2';
    return market;
}
...
function _findLiveMatchByHash(matchId) {
    if (!matchId || !_liveData || !_liveData.length) return null;
    for (var i = 0; i < _liveData.length; i++) {
        if (_liveData[i].match_id_hash === matchId) return _liveData[i];
    }
    return null;
}

function _findLiveMatch(homeTeam, awayTeam) {
    if (!_liveData || !_liveData.length) return null;
    var h = (homeTeam || '').toLowerCase().trim();
    var a = (awayTeam || '').toLowerCase().trim();
    for (var i = 0; i < _liveData.length; i++) {
        var lm = _liveData[i];
        var lh = (lm.home_team || '').toLowerCase().trim();
        var la = (lm.away_team || '').toLowerCase().trim();
        if (lh === h && la === a) return lm;
        if (h.length > 3 && lh.includes(h)) return lm;
        if (a.length > 3 && la.includes(a)) return lm;
        if (lh.length > 3 && h.includes(lh)) return lm;
    }
    return null;
}

function _isFinishedLiveMatch(lm) {
    if (!lm) return false;
    var st = (lm.status || '').toLowerCase();
    var mn = (lm.minute || '').trim().toUpperCase();
    return st === 'ft' || mn === 'FT' || mn === 'MS' || mn === 'AET' || mn === 'PEN';
}

function _buildFinishedCapsule(lm, extraClass) {
    var scoreHtml = '';
    var sc = lm.score || '';
    if (sc) {
        var parts = sc.split('-');
        var sh = (parts[0] || '').trim();
        var sa = (parts[1] || '').trim();
        scoreHtml = '<span class="lc-divider"></span>' +
            '<span class="lc-score-val"><span class="lc-score-h">' + sh + '</span>' +
            '<span class="lc-score-sep">-</span>' +
            '<span class="lc-score-a">' + sa + '</span></span>';
    }
    return '<div class="live-capsule lc-ended' + (extraClass ? ' ' + extraClass : '') + '">' +
        '<span class="lc-min">BİTTİ</span>' +
        scoreHtml +
        '</div>';
}

function _findLiveMatchExact(homeTeam, awayTeam) {
    if (!_liveData || !_liveData.length) return null;
    var h = (homeTeam || '').toLowerCase().trim();
    var a = (awayTeam || '').toLowerCase().trim();
    for (var i = 0; i < _liveData.length; i++) {
...
function _findFinishedScore(homeTeam, awayTeam, matchId) {
    if (matchId && _finishedScores[matchId]) return _finishedScores[matchId];
    var key = ((homeTeam || '') + '|' + (awayTeam || '')).toLowerCase();
    return _finishedScores[key] || null;
}

function _getModalFinishedCapsule(match) {
    if (!match || !match.date) return '';
    var mt = toTurkeyTime(match.date);
    if (!mt || !mt.isValid()) return '';
    var diff = nowTurkey().diff(mt, 'minute');
    if (diff < 80) return '';
    var lmExact = _findLiveMatchExact(match.home_team, match.away_team);
    if (_isFinishedLiveMatch(lmExact)) {
        return ' ' + _buildFinishedCapsule(lmExact, 'lc-modal');
    }
    var ft = _findFinishedScore(match.home_team, match.away_team, match.match_id);
    if (ft) {
        return ' ' + _buildFinishedCapsule({score: ft.score, status: 'ft'}, 'lc-modal');
    }
    return '';
}

async function loadFinishedScores(reRender) {
    try {
        var resp = await fetch('/api/finished-scores');
        var data = await resp.json();
        if (data && data.scores) {
            _finishedScores = data.scores;
            if (reRender && filteredMatches && filteredMatches.length) {
                renderMatches(filteredMatches);
            }
        }
    } catch (e) {}
...
function getMatchLiveCapsule(dateStr, homeTeam, awayTeam, matchId) {
    if (!dateStr) return '';
    const matchTime = toTurkeyTime(dateStr);
    if (!matchTime || !matchTime.isValid()) return '';
    const now = nowTurkey();
    const diffMinutes = now.diff(matchTime, 'minute');
    if (diffMinutes >= 80) {
        var lmHash = _findLiveMatchByHash(matchId);
        if (_isFinishedLiveMatch(lmHash)) {
            return _buildFinishedCapsule(lmHash, '');
        }
        var lmExact = _findLiveMatchExact(homeTeam, awayTeam);
        if (_isFinishedLiveMatch(lmExact)) {
            return _buildFinishedCapsule(lmExact, '');
        }
        var ftScore = _findFinishedScore(homeTeam, awayTeam, matchId);
        if (ftScore) {
            return _buildFinishedCapsule({score: ftScore.score, status: 'ft'}, '');
        }
    }
    var lm = _findLiveMatchByHash(matchId) || _findLiveMatch(homeTeam, awayTeam);
    if (diffMinutes < 0 || diffMinutes > 113) return '';
    var minStr = '';
    var scoreHtml = '';
    if (lm) {
        var rawMin = (lm.minute || '').trim();
        minStr = rawMin || diffMinutes + "'";
        var sc = lm.score || '';
        if (sc) {
            var parts = sc.split('-');
            var sh = (parts[0] || '').trim();
        ...
        if (diffMinutes <= 45) {
            ...
            minStr = (diffMinutes - 15) + "'";
        } else {
            minStr = "90+'";
        }
    }
    return '<div class="live-capsule">' +
        '<span class="lc-dot"></span>' +
        '<span class="lc-min">' + minStr + '</span>' +
        scoreHtml +
        '</div>';
}

function getMatchLiveCapsuleDT(dateStr, homeTeam, awayTeam, matchId) {
    if (!dateStr) return '';
    const matchTime = toTurkeyTime(dateStr);
    if (!matchTime || !matchTime.isValid()) return '';
    const now = nowTurkey();
    ...
    if (diffMinutes >= 80) {
        var lmHash = _findLiveMatchByHash(matchId);
        if (_isFinishedLiveMatch(lmHash)) {
            return _buildFinishedCapsule(lmHash, 'live-capsule-desk');
        }
        var lmExact = _findLiveMatchExact(homeTeam, awayTeam);
        if (_isFinishedLiveMatch(lmExact)) {
            return _buildFinishedCapsule(lmExact, 'live-capsule-desk');
        }
        var ftScore = _findFinishedScore(homeTeam, awayTeam, matchId);
        if (ftScore) {
            return _buildFinishedCapsule({score: ftScore.score, status: 'ft'}, 'live-capsule-desk');
        }
    }
    var lm = _findLiveMatchByHash(matchId) || _findLiveMatch(homeTeam, awayTeam);
    if (diffMinutes < 0 || diffMinutes > 113) return '';
    ...
    return '<div class="live-capsule live-capsule-desk">' +
        '<span class="lc-dot"></span>' +
        '<span class="lc-min">' + minStr + '</span>' +
        scoreHtml +
        '</div>';
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    loadFinishedScores();
    await Promise.all([loadUserFavorites(), loadFavoriteCounts()]);
    loadMatches();
    _startBackgroundLiveFetch();
    checkStatus();
    window.statusInterval = window.setInterval(checkStatus, 60000);
    
    if (window.loadChartLibs) window.loadChartLibs().then(() => registerChartPlugins()).catch(() => {});
    
    // Dropping preload deferred — loaded on tab switch
...
let _matchesMarketCache = {};
const _MATCHES_CACHE_TTL = 90000;
...
async function loadMatches(appendMode = false) {
    ...
    const cacheKey = `${requestMarket}|${dateFilterMode}|${requestSource}`;
    ...
    const colspan = currentMarket.includes('1x2') ? 8 : 7;
    ...
    _loadMatchesPending = (async () => {
        try {
            ...
            let apiUrl;
            const srcParam = requestSource !== 'betfair' ? `&source=${requestSource}` : '';
            if (dateFilterMode === 'ALL') {
                apiUrl = `/api/matches?market=${requestMarket}&date_filter=today_future&bulk=1${srcParam}`;
            } else if (dateFilterMode === 'YESTERDAY') {
                apiUrl = `/api/matches?market=${requestMarket}&date_filter=yesterday&bulk=1${srcParam}`;
            } else if (dateFilterMode === 'TODAY') {
                apiUrl = `/api/matches?market=${requestMarket}&date_filter=today&bulk=1${srcParam}`;
            } else if (dateFilterMode && dateFilterMode.startsWith('PAST_')) {
                const daysBack = dateFilterMode.split('_')[1];
                apiUrl = `/api/matches?market=${requestMarket}&date_filter=d-${daysBack}&bulk=1${srcParam}`;
            } else {
                apiUrl = `/api/matches?market=${requestMarket}&date_filter=today_future&bulk=1${srcParam}`;
            }
            ...
            let response;
            try {
                response = await _fetchWithTimeout(apiUrl, 12000);
            } catch (fetchErr) {
                if (fetchErr.name === 'AbortError') {
                    console.warn('[Matches] Fetch timeout (12s), retrying...');
                    await new Promise(r => setTimeout(r, 1500));
                    response = await _fetchWithTimeout(apiUrl, 20000);
                } else {
                    throw fetchErr;
                }
            }
            if (response.status === 403) {
                console.warn('[Matches] 403 License error');
                return;
            }
            ...
            if (result.finished_scores) {
                _finishedScores = result.finished_scores;
            }
    ...
    while (hasMoreMatches && page < maxPages) {
        ...
        try {
            ...
            if (dateFilterMode === 'YESTERDAY') {
                ...
                apiUrl += `&date_filter=d-${dateFilterMode.split('_')[1]}`;
            }
...
function renderDonutSVG(percent, size = 48) {
    ...
    const trackColor = '#2a2e33';
    const isHigh = num >= 50;
    const fillColor = isHigh ? '#22c55e' : '#1a1d21';
...
async function fetchAnalysisMatchHashes() {
    try {
        const resp = await fetch('/api/analyses/match-hashes');
        if (resp.ok) {
            const data = await resp.json();
...
function getAnalysisStarHtml(matchId) {
    if (!matchId || !_analysisMatchHashes.length) return '';
    if (_analysisMatchHashes.indexOf(matchId) === -1) return '';
    return '<span class="analysis-star" title="Bu maç ile ilgili analiz mevcut" onclick="event.stopPropagation(); openTrendsModal(\'analysis\');">★</span>';
}

function _renderMatchRow(match, idx) {
...
function _renderMoreDesktop() {
    ...
    if (_renderedCount >= _allFilteredMatches.length) return;
    const tbody = document.getElementById('matchesTableBody');
    if (!tbody) return;
    const end = Math.min(_renderedCount + _RENDER_BATCH, _allFilteredMatches.length);
    let html = '';
    for (let i = _renderedCount; i < end; i++) {
        html += _renderMatchRow(_allFilteredMatches[i], i);
    }
    var _prevEnd = _renderedCount;
...
function _renderMoreMobile() {
    ...
    const isMoneyway = currentMarket.startsWith('moneyway');
    for (let i = _renderedCount; i < end; i++) {
        const match = _allFilteredMatches[i];
        const d = match.details || match.odds || {};
        const volume = formatVolumeCompact(d.Volume);
        const _mStar = getAnalysisStarHtml(match.match_id);
        let dateStr = '';
        try {
        ...
        if (isMoneyway) {
            html += renderMobileMoneywayCard(match, i, d, volume, dateStr, _mStar);
        } else if (isDropping) {
            html += renderMobileOddsCard(match, i, d, volume, dateStr, _mStar);
        } else {
            const matchStatus = getMatchStatus(match.date);
...
function _updateRenderedCount() {
    const countEl = document.getElementById('matchCount');
    const mobileCountEl = document.getElementById('mobileMatchCount');
    const total = _allFilteredMatches.length;
    const shown = Math.min(_renderedCount, total);
    const text = shown < total ? `${shown} / ${total}` : `${total}`;
...
function renderMatches(data) {
    ...
    loadFavoriteCounts();
    if (_favFilterActive) setTimeout(() => _applyFavoritesFilter(), 50);
}
...
function _mobileFavLine(mk, isFav, count) {
    var txt = (count > 0 ? count : 0) + (window.SXFI18n ? window.SXFI18n.t('app.dyn.kisi') + ' ' + window.SXFI18n.t('app.dyn.takip_ediyor') : ' kişi takip ediyor');
    return '<div class="mobile-fav-line"><span class="fav-heart ' + (isFav ? 'fav-active' : '') + '" data-matchkey="' + mk.replace(/"/g, '&quot;') + '" onclick="event.stopPropagation(); toggleFavorite(this);">' + _heartSvg + '</span><span class="mobile-fav-count" data-matchkey="' + mk.replace(/"/g, '&quot;') + '">' + txt + '</span></div>';
}
...
function renderMobileMoneywayCard(match, idx, d, volume, dateStr, starHtml) {
    ...
    const liveCapsule = getMatchLiveCapsule(match.date, match.home_team, match.away_team, match.match_id);
    const _mk = _getMatchKey(match);
    const _isFav = _userFavorites.has(_mk);
    const _fc = _favCounts[_mk] || 0;
...
function buildCardInsight(d, market) {
    var parts = [];
    if (market.includes('1x2')) {
        var p1 = parseFloat(String(d.PctHome || d.Pct1 || d.PctYes || '0').replace('%',''));
        var pX = parseFloat(String(d.PctDraw || d.PctX || '0').replace('%',''));
        var p2 = parseFloat(String(d.PctAway || d.Pct2 || d.PctNo || '0').replace('%',''));
        if (p1 > 0 || pX > 0 || p2 > 0) {
            var best = Math.max(p1, pX, p2);
            var leader = best === p1 ? '1' : (best === pX ? 'X' : '2');
            parts.push('<span class="insight-tag">Favori: <span class="insight-val">' + leader + '</span></span>');
            parts.push('<span class="insight-tag">Pay: <span class="insight-val positive">' + best.toFixed(0) + '%</span></span>');
            var spread = (best - Math.min(p1 > 0 ? p1 : 999, pX > 0 ? pX : 999, p2 > 0 ? p2 : 999)).toFixed(0);
            var label = parseInt(spread) > 30 ? 'Güçlü favori' : (parseInt(spread) > 10 ? 'Favori açık' : 'Dengeli');
            parts.push('<span class="insight-label">' + label + '</span>');
        }
    } else if (market.includes('2.5')) {
        var pOver = parseFloat(String(d.PctYes || d.PctOver || '0').replace('%',''));
        var pUnder = parseFloat(String(d.PctNo || d.PctUnder || '0').replace('%',''));
        if (pOver > 0 || pUnder > 0) {
            var lbl = pOver > pUnder ? 'Üst' : 'Alt';
            var pct = Math.max(pOver, pUnder);
            parts.push('<span class="insight-tag">Lider: <span class="insight-val">' + lbl + '</span></span>');
            parts.push('<span class="insight-tag">Pay: <span class="insight-val positive">' + pct.toFixed(0) + '%</span></span>');
        }
    } else if (market.includes('kg')) {
        var pY = parseFloat(String(d.PctYes || '0').replace('%',''));
        var pN = parseFloat(String(d.PctNo || '0').replace('%',''));
        if (pY > 0 || pN > 0) {
            var lbl = pY > pN ? 'Evet' : 'Hayır';
            var pct = Math.max(pY, pN);
            parts.push('<span class="insight-tag">KG: <span class="insight-val">' + lbl + '</span></span>');
            parts.push('<span class="insight-tag">Pay: <span class="insight-val positive">' + pct.toFixed(0) + '%</span></span>');
        }
    }
    if (parts.length === 0) return '';
    return '<div class="odds-card-insight">' + parts.join('') + '</div>';
}

function _calcBlockMoney(pctNum, totalVol) {
    if (!totalVol) return '';
    var rawVol = String(totalVol).replace(/[£€$,\s]/g, '');
    var mult = 1;
    if (rawVol.toUpperCase().includes('M')) { mult = 1000000; rawVol = rawVol.replace(/M/gi, ''); }
    else if (rawVol.toUpperCase().includes('K')) { mult = 1000; rawVol = rawVol.replace(/K/gi, ''); }
    var totalNum = parseFloat(rawVol) * mult;
    if (isNaN(totalNum) || totalNum <= 0) return '';
    var bm = totalNum * pctNum / 100;
    return bm >= 1000000 ? '£' + (bm/1000000).toFixed(1) + 'M' : bm >= 1000 ? '£' + (bm/1000).toFixed(1) + 'k' : '£' + Math.round(bm);
}

function renderMobileMoneywayBlock(label, oddsValue, pctValue, totalVol) {
    const odds = formatOdds(oddsValue);
    var pct = '';
    var moneyStr = '';
    
    if (pctValue !== null && pctValue !== undefined) {
        var pctNum = parseFloat(String(pctValue).replace('%', ''));
        if (!isNaN(pctNum)) {
            pct = pctNum.toFixed(0) + '%';
            moneyStr = _calcBlockMoney(pctNum, totalVol);
        }
    }
    
    return '<div class="odds-block mw-block">' +
        '<div class="odds-block-label">' + label + '</div>' +
        '<div class="ob-data-row">' +
            '<span class="ob-odds">' + odds + '</span>' +
            (moneyStr ? '<span class="ob-money">' + moneyStr + '</span>' : '') +
            (pct ? '<span class="ob-pct">' + pct + '</span>' : '') +
        '</div>' +
    '</div>';
}
...
function renderMobileOddsBlock(label, oddsValue, trendData) {
    ...
    if (trendData && trendData.pct_change !== null && trendData.pct_change !== undefined) {
        ...
        if (pctChange !== 0) {
            ...
            trendStr = '<span class="ob-trend" style="color:' + color + '">' + arrow + ' ' + Math.abs(pctChange).toFixed(1) + '%</span>';
        } else {
            trendStr = '<span class="ob-trend" style="color:#5c636b">— 0%</span>';
        }
    } else if (trendData && trendData.trend && (trendData.trend === 'down' || trendData.trend === 'up')) {
        const isDown = trendData.trend === 'down';
        const arrow = isDown ? '▼' : '▲';
        const color = isDown ? '#ef4444' : '#22c55e';
        trendStr = '<span class="ob-trend" style="color:' + color + '">' + arrow + '</span>';
    } else {
        trendStr = '<span class="ob-trend" style="color:#5c636b">— 0%</span>';
    }
...
function setDayFilter(filter) {
    ...
    if (labelEl) labelEl.textContent = _getDayFilterLabel(filter);
    ...
    if (filter === 'today') {
    ...
    } else if (filter === 'yesterday' || filter === 'd-1') {
        dateFilterMode = filter === 'yesterday' ? 'YESTERDAY' : 'PAST_1';
        ...
        if (todayBtn) todayBtn.classList.remove('active');
        if (yesterdayBtn) yesterdayBtn.classList.remove('active');
    } else if (filter && filter.startsWith('d-')) {
        dateFilterMode = 'PAST_' + filter.slice(2);
        if (todayBtn) todayBtn.classList.remove('active');
        if (yesterdayBtn) yesterdayBtn.classList.remove('active');
    } else {
        dateFilterMode = 'ALL';
...
function openMobileFilterModal() {
    ...
    var isPast = currentDayFilter && currentDayFilter.startsWith('d-');
    document.querySelectorAll('.mf-chip[data-group="date"]').forEach(function(c) {
        var val = c.getAttribute('data-value');
        if (val === 'past') {
            c.classList.toggle('active', isPast);
            if (isPast) c.textContent = _getDayFilterLabel(currentDayFilter) + ' ▾';
            else c.textContent = 'Geçmiş ▾';
        } else {
            c.classList.toggle('active', val === currentDayFilter);
        }
    });
    var pd = document.getElementById('mfPastDropdown');
    var pv = document.getElementById('mfPastSelectedValue');
    if (pd && pv) {
        if (isPast) {
            pv.value = currentDayFilter;
            _buildMfPastDropdown(pd);
    ...
    var sortVal = (currentSortColumn === 'volume') ? 'volume' : 'date';
    document.querySelectorAll('.mf-chip[data-group="sort"]').forEach(function(c) {
        c.classList.toggle('active', c.getAttribute('data-value') === sortVal);
    });
    
    document.querySelectorAll('.mf-sw[data-group="filter"]').forEach(function(c) {
        var v = c.getAttribute('data-value');
        if (v === 'hideEnded') c.classList.toggle('active', _mobileHideEnded);
        if (v === 'hideLive') c.classList.toggle('active', _mobileHideLive);
...
function mfSelect(el) {
    var group = el.getAttribute('data-group');
    el.closest('.mf-chips').querySelectorAll('.mf-chip[data-group="' + group + '"]').forEach(function(c) {
...
function mfSelectPast(el) {
    var chips = el.closest('.mf-chips').querySelectorAll('.mf-chip[data-group="date"]');
    chips.forEach(function(c) { c.classList.remove('active'); });
    el.classList.add('active');
    var pd = document.getElementById('mfPastDropdown');
    if (!pd) return;
    if (pd.style.display === 'flex') {
        pd.style.display = 'none';
    } else {
        _buildMfPastDropdown(pd);
        pd.style.display = 'flex';
...
function _buildMfPastDropdown(container) {
    var nowTR = nowTurkey ? nowTurkey() : dayjs();
    var selectedVal = (document.getElementById('mfPastSelectedValue') || {}).value || '';
    var months = ['Oca','Şub','Mar','Nis','May','Haz','Tem','Ağu','Eyl','Eki','Kas','Ara'];
    container.innerHTML = '';
    for (var i = 1; i <= 7; i++) {
        var d = nowTR.subtract(i, 'day');
        var label = i === 1 ? 'Dün' : (d.date() + ' ' + months[d.month()]);
        var val = 'd-' + i;
        var btn = document.createElement('button');
        btn.className = 'mf-past-day-btn' + (selectedVal === val ? ' active' : '');
        btn.textContent = label;
        btn.setAttribute('data-val', val);
        btn.setAttribute('data-label', label);
        btn.onclick = (function(v, lbl, b) {
            return function() {
                container.querySelectorAll('.mf-past-day-btn').forEach(function(x) { x.classList.remove('active'); });
                b.classList.add('active');
                var pv = document.getElementById('mfPastSelectedValue');
                if (pv) pv.value = v;
                var toggle = document.querySelector('.mf-past-toggle');
                if (toggle) toggle.textContent = lbl + ' ▾';
            };
        })(val, label, btn);
...
function mfToggle(el) {
    ...
    var list = el.closest('.mf-switch-list');
    
    if (val === 'onlyLive' && !el.classList.contains('active')) {
        var hl = list ? list.querySelector('.mf-sw[data-value="hideLive"]') : null;
        if (hl) hl.classList.remove('active');
    }
    if (val === 'hideLive' && !el.classList.contains('active')) {
        var ol = list ? list.querySelector('.mf-sw[data-value="onlyLive"]') : null;
...
function applyMobileFilter() {
    var dateChip = document.querySelector('.mf-chip[data-group="date"].active');
    var sortChip = document.querySelector('.mf-chip[data-group="sort"].active');
    
    var dateChipVal = dateChip ? dateChip.getAttribute('data-value') : 'all';
    var sortVal = sortChip ? sortChip.getAttribute('data-value') : 'date';
    
    var dateVal = dateChipVal;
    if (dateChipVal === 'past') {
        var pvEl = document.getElementById('mfPastSelectedValue');
        var pastVal = pvEl ? pvEl.value : '';
        if (pastVal && pastVal.startsWith('d-')) {
            dateVal = pastVal;
        } else {
            dateVal = 'd-1';
        }
    ...
    _mobileHideEnded = !!document.querySelector('.mf-sw[data-value="hideEnded"].active');
    _mobileHideLive = !!document.querySelector('.mf-sw[data-value="hideLive"].active');
    _mobileOnlyLive = !!document.querySelector('.mf-sw[data-value="onlyLive"].active');
    
    var hasFilter = (dateVal !== 'all') || _mobileHideEnded || _mobileHideLive || _mobileOnlyLive || (sortVal !== 'date');
    var btn = document.getElementById('mobileFilterBtn');
    if (btn) btn.classList.toggle('has-filter', hasFilter);
    var dBtn = document.getElementById('desktopFilterBtn');
    if (dBtn) dBtn.classList.toggle('has-filter', hasFilter);
    ...
    var modeMap = {all: 'ALL', today: 'TODAY', yesterday: 'YESTERDAY', future: 'FUTURE'};
    if (dateVal.startsWith('d-')) {
        dateFilterMode = 'PAST_' + dateVal.slice(2);
    } else {
        dateFilterMode = modeMap[dateVal] || 'ALL';
    }
    currentDayFilter = dateVal;
    
    var todayBtn = document.getElementById('todayBtn');
    var yesterdayBtn = document.getElementById('yesterdayBtn');
    if (todayBtn) todayBtn.classList.toggle('active', dateVal === 'today');
    if (yesterdayBtn) yesterdayBtn.classList.toggle('active', dateVal === 'd-1' || dateVal === 'yesterday');
    
    var labelEl = document.getElementById('dayFilterLabel');
    if (labelEl) labelEl.textContent = _getDayFilterLabel(dateVal);
    
    delete _matchesMarketCache[currentMarket + '|' + dateFilterMode + '|' + currentSource];
    loadMatches();
}

function _getDayFilterLabel(dateVal) {
    if (!dateVal || dateVal === 'all') return 'Tümü';
    if (dateVal === 'today') return 'Bugün';
    if (dateVal === 'yesterday' || dateVal === 'd-1') return 'Dün';
    if (dateVal === 'future') return 'Gelecek';
    if (dateVal.startsWith('d-')) {
        var n = parseInt(dateVal.slice(2));
        if (!isNaN(n)) {
            var nowTR = nowTurkey ? nowTurkey() : dayjs();
            var d = nowTR.subtract(n, 'day');
            var months = ['Oca','Şub','Mar','Nis','May','Haz','Tem','Ağu','Eyl','Eki','Kas','Ara'];
...
function filterMatches(query) {
    if (_liveMode) {
        if (!_liveData || !_liveData.length) return;
        if (query) {
            var lf = _liveData.filter(function(m) {
                return (m.home_team && m.home_team.toLowerCase().includes(query)) ||
                       (m.away_team && m.away_team.toLowerCase().includes(query)) ||
                       (m.league && m.league.toLowerCase().includes(query));
            });
            renderLiveMatches(lf);
        } else {
            renderLiveMatches(_liveData);
        }
        return;
    }
...
function applySorting(data) {
    ...
    if (dateFilterMode === 'YESTERDAY') {
        ...
        const daysBack = parseInt(dateFilterMode.split('_')[1]);
        const targetStr = nowTR.subtract(daysBack, 'day').format('YYYY-MM-DD');
        console.log('[Filter] PAST mode:', targetStr);
        sortedData = sortedData.filter(m => {
            const matchDateStr = getMatchDateTR(m.date);
            if (!matchDateStr) return false;
            return matchDateStr === targetStr;
        });
        console.log('[Filter] PAST filtered count:', sortedData.length);
    } else if (dateFilterMode === 'TODAY') {
        ...
        console.log('[Filter] FUTURE mode: >', todayStr);
        sortedData = sortedData.filter(m => {
            const dt = toTurkeyTime(m.date);
            if (!dt || !dt.isValid()) return false;
            return dt.isAfter(nowTR);
        });
        console.log('[Filter] FUTURE filtered count:', sortedData.length);
    } else {
    ...
    if (_mobileHideEnded || _mobileHideLive || _mobileOnlyLive) {
        sortedData = sortedData.filter(m => {
            const statusHtml = getMatchStatus(m.date);
            const isLive = statusHtml.includes('CANLI');
            const isEnded = statusHtml.includes('BİTTİ');
            if (_mobileOnlyLive && !isLive) return false;
            if (_mobileHideEnded && isEnded) return false;
            if (_mobileHideLive && isLive) return false;
            return true;
        });
    }
...
function sortByColumn(column) {
    if (currentSortColumn === column) {
        ...
        currentSortDirection = (column === 'date' || column === 'league' || column === 'match') ? 'asc' : 'desc';
    }
...
async function openMatchModalFromMatches(index) {
    if (index >= 0 && index < matches.length) {
        ...
        var modalTitleHtml = escapeHtml(selectedMatch.home_team) + ' vs ' + escapeHtml(selectedMatch.away_team);
        document.getElementById('modalMatchTitle').innerHTML = modalTitleHtml;
        
        var modalFtCapsule = _getModalFinishedCapsule(selectedMatch);
        var isFinishedMatch = modalFtCapsule !== '';
        const headerDt = toTurkeyTime(selectedMatch.date);
        let headerDateText = '';
        if (headerDt && headerDt.isValid()) {
            headerDateText = headerDt.format('DD.MM HH:mm');
        }
        var leagueHtml = escapeHtml(selectedMatch.league || '');
        if (isFinishedMatch) leagueHtml += modalFtCapsule;
        if (headerDateText) leagueHtml += '<span class="modal-sub-sep"> \u2022 </span><span class="modal-sub-date">' + headerDateText + '</span>';
        document.getElementById('modalLeague').innerHTML = leagueHtml;
        ...
        const matchIdHash = selectedMatch.match_id || '';
        const chartLibsPromise = window.loadChartLibs ? window.loadChartLibs().then(() => registerChartPlugins()) : Promise.resolve();
        const marketsPromise = loadAllMarketsAtOnce(home, away, league, matchIdHash);
        
        const chartPipeline = Promise.all([marketsPromise, chartLibsPromise]).then(() => {
            if (reqId !== _modalRequestId) return;
            return loadChartWithTrends(home, away, selectedChartMarket, league, reqId);
        }).catch(e => console.error('[Modal] Chart pipeline error:', e));
        
        const alarmsPipeline = renderMatchAlarmsSection(home, away, reqId);
        
        const livePipeline = _checkModalLiveData(home, away, reqId);
        
        await Promise.all([chartPipeline, alarmsPipeline, livePipeline]);
    }
}
...
async function openMatchModalFromAPI(homeTeam, awayTeam, league, kickoff) {
    ...
    var apiHome = homeTeam;
    var apiAway = awayTeam;
    var apiLeague = league;
    var apiKickoff = kickoff;
    var prematchFound = null;
    var hLow = (homeTeam || '').toLowerCase().trim();
    var aLow = (awayTeam || '').toLowerCase().trim();
    function _pmSearch(arr) {
        for (var si = 0; si < arr.length; si++) {
            var pmH = (arr[si].home_team || '').toLowerCase().trim();
            var pmA = (arr[si].away_team || '').toLowerCase().trim();
            if (pmH === hLow && pmA === aLow) return arr[si];
            if ((pmH.includes(hLow) || hLow.includes(pmH)) && (pmA.includes(aLow) || aLow.includes(pmA)) && pmH.length > 2 && pmA.length > 2) {
                return arr[si];
            }
        }
        return null;
    }
    var fArr2 = (typeof filteredMatches !== 'undefined' && filteredMatches.length > 0) ? filteredMatches : [];
    var mArr2 = (typeof matches !== 'undefined' && matches.length > 0) ? matches : [];
    prematchFound = _pmSearch(fArr2);
    if (!prematchFound && mArr2.length > fArr2.length) {
        prematchFound = _pmSearch(mArr2);
    }
    if (prematchFound) {
        apiHome = prematchFound.home_team || homeTeam;
        apiAway = prematchFound.away_team || awayTeam;
        if (!league && prematchFound.league) apiLeague = prematchFound.league;
        var _kHasTime = /T\d{2}:\d{2}|\s\d{2}:\d{2}/.test(String(kickoff || ''));
        if ((!kickoff || !_kHasTime) && prematchFound.date) apiKickoff = prematchFound.date;
        console.log('[ModalFromAPI] Prematch found:', apiHome, 'vs', apiAway, 'league:', apiLeague);
    } else {
        console.log('[ModalFromAPI] No prematch match for', homeTeam, 'vs', awayTeam, 'in filtered:', fArr2.length, 'all:', mArr2.length);
    }
    ...
    selectedMatch = {
        ...
        league: apiLeague,
        date: apiKickoff,
        match_id: prematchFound ? (prematchFound.match_id || '') : '',
        history: [],
        history_count: 0,
        odds: null,
        details: null
    ...
    var modalTitleHtml = escapeHtml(homeTeam) + ' vs ' + escapeHtml(awayTeam);
    document.getElementById('modalMatchTitle').innerHTML = modalTitleHtml;
    
    var modalFtCapsule = _getModalFinishedCapsule(selectedMatch);
    var isFinishedMatch = modalFtCapsule !== '';
    const headerDt = apiKickoff ? toTurkeyTime(apiKickoff) : null;
    let headerDateText = '';
    if (headerDt && headerDt.isValid()) {
        headerDateText = headerDt.format('DD.MM HH:mm');
    }
    var leagueHtml = escapeHtml(apiLeague || '');
    if (isFinishedMatch) leagueHtml += modalFtCapsule;
    if (headerDateText) leagueHtml += '<span class="modal-sub-sep"> \u2022 </span><span class="modal-sub-date">' + headerDateText + '</span>';
    document.getElementById('modalLeague').innerHTML = leagueHtml || '';
    ...
    const chartLibsPromise = window.loadChartLibs ? window.loadChartLibs().then(() => registerChartPlugins()) : Promise.resolve();
    const marketsPromise = loadAllMarketsAtOnce(apiHome, apiAway, apiLeague, matchIdHash2);

    try {
        const chartPipeline = Promise.all([marketsPromise, chartLibsPromise]).then(() => {
            if (reqId !== _modalRequestId) return;
            return loadChartWithTrends(apiHome, apiAway, selectedChartMarket, apiLeague, reqId);
        }).catch(e => console.error('[Modal] Chart pipeline error:', e));
        
        const alarmsPipeline = renderMatchAlarmsSection(apiHome, apiAway, reqId);
        
        const livePipeline = _checkModalLiveData(homeTeam, awayTeam, reqId);
...
async function openMatchModal(index) {
    ...
    if (index >= 0 && index < dataSource.length) {
        ...
        const matchIdHash3 = selectedMatch.match_id || '';
        const chartLibsPromise = window.loadChartLibs ? window.loadChartLibs().then(() => registerChartPlugins()) : Promise.resolve();
        const marketsPromise = loadAllMarketsAtOnce(home, away, league, matchIdHash3);
...
function resetModalState() {
    ...
    var liveTab = document.getElementById('modalLiveTab');
    if (liveTab) liveTab.style.display = 'none';
    var liveBody = document.getElementById('modalLiveBody');
    if (liveBody) liveBody.innerHTML = '';
    var liveTabs = document.getElementById('modalLiveMarketTabs');
    if (liveTabs) liveTabs.innerHTML = '';
    if (window.userPlan === 'core') {
        if (liveTab) liveTab.style.display = '';
        if (liveBody) liveBody.innerHTML = proLockPanelHtml();
    }
    if (window.userPlan === 'test' && selectedMatch && !_isTestFreeMatch(selectedMatch.match_id)) {
        if (liveTab) liveTab.style.display = '';
        if (liveBody) liveBody.innerHTML = proLockPanelHtml();
    }
...
async function loadAllMarketsAtOnce(home, away, league = '', matchIdHash = '') {
    const cacheKey = `${home}|${away}|${league}|${currentSource}`;
    ...
    try {
        console.log('[Bulk] Fetching all markets for', home, 'vs', away, 'league:', league, 'source:', currentSource);
        const startTime = performance.now();
        var url = `/api/match/history/bulk?home=${encodeURIComponent(home)}&away=${encodeURIComponent(away)}&league=${encodeURIComponent(league || '')}`;
...
async function loadChartWithTrends(home, away, market, league = '', reqId) {
    try {
        ...
        if (modalOddsData && modalOddsData.Date) {
            const leagueEl = document.getElementById('modalLeague');
            if (leagueEl && leagueEl.innerHTML.indexOf(' • ') === -1) {
                const dt = toTurkeyTime(modalOddsData.Date);
                if (dt && dt.isValid()) {
                    leagueEl.innerHTML += ' • ' + dt.format('DD.MM HH:mm');
                }
            }
        }
...
function switchMobileTab(tabName) {
    if (tabName === 'live' && (window.userPlan === 'core' || window.userPlan === 'test')) {
        ...
        var _tc = document.getElementById('mobileTabLive');
        if (_tc) _tc.classList.add('active');
        var _lb = document.getElementById('modalLiveBody');
        if (_lb) _lb.innerHTML = proLockPanelHtml();
        var _lt = document.getElementById('modalLiveMarketTabs');
        if (_lt) _lt.innerHTML = '';
        return;
    }
    document.querySelectorAll('.mobile-tab-bar .mob-tab').forEach(t => {
        t.classList.remove('active');
...
function closeModal() {
    ...
    desktopChartSectionOpen = true;
    const chartBody = document.getElementById('desktopChartBody');
    if (chartBody) chartBody.style.display = '';
    const chartChevron = document.getElementById('desktopChartChevron');
    if (chartChevron) chartChevron.textContent = '▼';
...
async function loadChart(home, away, market, league = '') {
    try {
        if (typeof Chart === 'undefined') {
            console.error('[loadChart] Chart.js not loaded, retrying...');
            try {
                await window.loadChartLibs();
                registerChartPlugins();
            } catch(e2) {
                console.error('[loadChart] Chart.js retry failed:', e2);
                return;
            }
        }
        let data = { history: [] };
        ...
        if (bulkHistoryCacheKey === cacheKey && bulkHistoryCache[market]) {
            data = bulkHistoryCache[market];
            console.log('[loadChart] Using bulk cache for', market);
        } else {
            try {
                const response = await fetch(
                    `/api/match/history?home=${encodeURIComponent(home)}&away=${encodeURIComponent(away)}&market=${market}&league=${encodeURIComponent(league || '')}`
        ...
        if (data.history.length === 0) {
            ...
            var chartEl = document.getElementById('oddsChart');
            if (chartEl) {
                var wrap = chartEl.closest('.chart-container') || chartEl.parentElement;
                if (wrap) {
                    var emptyMsg = wrap.querySelector('.chart-empty-msg');
                    if (!emptyMsg) {
                        emptyMsg = document.createElement('div');
                        emptyMsg.className = 'chart-empty-msg';
                        emptyMsg.style.cssText = 'text-align:center;padding:40px 20px;color:rgba(203,213,225,0.4);font-size:13px;';
                        emptyMsg.textContent = 'Bu maç için grafik verisi bulunamadı';
        ...
        var chartWrap = document.getElementById('oddsChart');
        if (chartWrap) {
            var emptyEl = (chartWrap.closest('.chart-container') || chartWrap.parentElement);
            if (emptyEl) { var em = emptyEl.querySelector('.chart-empty-msg'); if (em) em.remove(); }
        }
...
async function exportFullMatchTXT() {
    ...
    const away = selectedMatch.away_team;
    const league = selectedMatch.league || '';
    const matchDate = selectedMatch.date || selectedMatch.start_time || '';
    const matchId = selectedMatch.match_id || '';

    const cacheKey = `${home}|${away}|${league}|${currentSource}`;
    let marketsData = {};
    if (bulkHistoryCacheKey === cacheKey && Object.keys(bulkHistoryCache).length > 0) {
        marketsData = bulkHistoryCache;
    } else {
        try {
            const resp = await fetch(`/api/match/history/bulk?home=${encodeURIComponent(home)}&away=${encodeURIComponent(away)}&league=${encodeURIComponent(league)}`);
            if (!resp.ok) { alert('Veri yüklenemedi (HTTP ' + resp.status + '). Lütfen tekrar deneyin.'); return; }
            const data = await resp.json();
            marketsData = data.markets || {};
        } catch (e) {
            console.error('[FullTXT] Bulk fetch error:', e);
    ...
    Object.values(marketsData).forEach(function(m) { if (m && m.history) totalSnapshots += m.history.length; });
    if (totalSnapshots === 0) { alert('Bu maç için henüz snapshot verisi bulunamadı.'); return; }

    let alarmsData = {};
    if (matchId) {
        try {
            const resp = await fetch(`/api/match/${matchId}/snapshot?include=alarms`);
            if (resp.ok) {
                const data = await resp.json();
                alarmsData = data.alarms || {};
            }
        } catch (e) {
            console.error('[FullTXT] Alarms fetch error:', e);
        }
    }
    ...
    var marketDefs = [
        {key: 'moneyway_1x2', label: 'Moneyway 1X2', cols: ['ScrapedAt','Odds1','OddsX','Odds2','Pct1','PctX','Pct2','Amt1','AmtX','Amt2','Volume','Odds1_prev','OddsX_prev','Odds2_prev','Trend1','TrendX','Trend2','DropPct1','DropPctX','DropPct2']},
        {key: 'moneyway_ou25', label: 'Moneyway O/U 2.5', cols: ['ScrapedAt','Over','Under','PctOver','PctUnder','AmtOver','AmtUnder','Volume','Line','Over_prev','Under_prev','TrendOver','TrendUnder','DropPctOver','DropPctUnder']},
        {key: 'moneyway_btts', label: 'Moneyway BTTS', cols: ['ScrapedAt','OddsYes','OddsNo','PctYes','PctNo','AmtYes','AmtNo','Volume','OddsYes_prev','OddsNo_prev','TrendYes','TrendNo','DropPctYes','DropPctNo']},
        {key: 'dropping_1x2', label: 'Dropping Odds 1X2', cols: ['ScrapedAt','Odds1','OddsX','Odds2','Odds1_prev','OddsX_prev','Odds2_prev','Trend1','TrendX','Trend2','DropPct1','DropPctX','DropPct2','Volume']},
        {key: 'dropping_ou25', label: 'Dropping Odds O/U 2.5', cols: ['ScrapedAt','Over','Under','Over_prev','Under_prev','TrendOver','TrendUnder','DropPctOver','DropPctUnder','Volume','Line']},
        {key: 'dropping_btts', label: 'Dropping Odds BTTS', cols: ['ScrapedAt','OddsYes','OddsNo','OddsYes_prev','OddsNo_prev','TrendYes','TrendNo','DropPctYes','DropPctNo','Volume']}
    ];

    txt += '===== SNAPSHOT VERILERI =====\n\n';

    marketDefs.forEach(function(md) {
        var mdata = marketsData[md.key];
        var rows = mdata ? (mdata.history || []) : [];
        txt += '--- ' + md.label + ' (' + rows.length + ' snapshot) ---\n';
        if (rows.length > 0) {
            rows.forEach(function(row) {
                var ts = row['ScrapedAt'] || '';
                var parts = [];
    ...
    txt += '===== ALARM VERILERI =====\n\n';
    var alarmTypes = ['sharp','bigmoney','volumeshock','dropping','volumeleader','mim'];
    var hasAlarms = false;
    alarmTypes.forEach(function(atype) {
        var alarms = alarmsData[atype] || [];
        if (alarms.length > 0) hasAlarms = true;
    });
    ...
    if (hasAlarms) {
        alarmTypes.forEach(function(atype) {
            ...
            txt += '--- ' + atype.toUpperCase() + ' (' + alarms.length + ' alarm) ---\n';
            alarms.forEach(function(a) {
                var ts = a.trigger_at || a.triggered_at || a.created_at || a.scraped_at || '';
                var market = a.market || '';
                var side = a.selection || a.side || '';
                var odds = a.odds || a.current_odds || '';
                var vol = a.volume || a.total_volume || a.amount || '';
                var details = '';
                if (atype === 'sharp') details = 'score=' + (a.sharp_score || a.score || '');
                else if (atype === 'bigmoney') details = 'amount=' + (a.incoming_amount || a.amount || '');
                else if (atype === 'volumeshock') details = 'shock=' + (a.shock_multiplier || a.shock || '') + 'x';
                else if (atype === 'dropping') details = 'drop=' + (a.drop_pct || a.drop_percent || '') + '%';
                else if (atype === 'volumeleader') details = 'from=' + (a.from_side || '') + ' to=' + (a.to_side || '') + ' pct=' + (a.to_pct || '');
                else if (atype === 'mim') details = 'impact=' + (a.impact_score || a.impact || '');
                txt += '  ' + ts + ' | market=' + market + ' | side=' + side + ' | odds=' + odds + ' | vol=' + vol + ' | ' + details + '\n';
            });
            txt += '\n';
        });
    } else {
        txt += '  Bu mac icin alarm verisi bulunamadi.\n';
    }

    var blob = new Blob([txt], { type: 'text/plain;charset=utf-8;' });
    var link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    var safeName = (home + '_vs_' + away).replace(/[^a-zA-Z0-9_\-]/g, '_');
    link.download = safeName + '_full_data.txt';
    link.click();
    URL.revokeObjectURL(link.href);
}
...
async function preloadDropMarkets() {
    ...
    console.log('[Preload] Starting parallel load for drop markets...');
    
    await Promise.all(dropMarkets.map(async (market) => {
...
function highlightNewAlarm(alarm) {
    ...
    if (alarmType === 'sharp') {
        ...
        value = `£${Math.round(Number(alarm.incoming_money || alarm.stake || 0)).toLocaleString('en-GB')}`;
...
function renderAlertBand() {
    ...
    const pillsHtml = top.map((alarm, idx) => {
        ...
        if (alarm._type === 'sharp') {
            ...
            value = incomingMoney > 0 ? `+£${Math.round(Number(incomingMoney)).toLocaleString('en-GB')}` : `${(impact * 100).toFixed(0)}%`;
        }
        ...
        var _alarmFree = _isTestFreeAlarm(home, away);
        var _blurClass = (window.userPlan === 'test' && !_alarmFree) ? ' test-blur-alarm' : '';
...
setInterval(() => { if (_isLicensed) loadAlertBand(); }, 120000);
document.addEventListener('DOMContentLoaded', async () => {
    await _licenseReady;
    if (!_isLicensed) return;
...
let alarmsDisplayCount = 10;
let alarmsSidebarOpen = false;
let openAlarmId = null;

var _mobMarkets = {
    moneyway: [{v:'1x2',l:'1X2'},{v:'ou25',l:'2.5'},{v:'btts',l:'KG'}],
    dropping: [{v:'1x2',l:'1X2'},{v:'ou25',l:'2.5'},{v:'btts',l:'KG'}],
    live: [{v:'1x2',l:'1X2'},{v:'ou1.5',l:'1.5'},{v:'ou2.5',l:'2.5'},{v:'ou3.5',l:'3.5'},{v:'ou4.5',l:'4.5'}]
};
var _mobCurCat = 'moneyway';
var _mobCurMkt = '1x2';
...
function _updateMobMktRow(cat) {
    var row = document.getElementById('mobMktRow');
    if (!row) return;
    var opts = _mobMarkets[cat] || _mobMarkets.moneyway;
    var html = '';
    opts.forEach(function(o, i) {
        html += '<button class="mob-tab' + (i === 0 ? ' active' : '') + '" data-mkt="' + o.v + '" onclick="selectMobMkt(\'' + o.v + '\')">' + o.l + '</button>';
    });
    row.innerHTML = html;
    _mobCurMkt = opts[0].v;
...
function toggleMobileOverflow(e) {
    ...
    if (menu.classList.contains('show')) {
        setTimeout(function() {
            document.addEventListener('click', closeMobileOverflow, { once: true });
        }, 10);
    }
}
...
function selectAlarmFilter(type) {
    ...
    if (type === 'all') {
        ...
        dotDisplay.className = 'filter-dot-display show';
        dotDisplay.style.background = alarmFilterColors[type] || '#7d848c';
        dotDisplay.innerHTML = '';
    }
...
function renderAlarmsList(filterType) {
    ...
    let html = displayGroups.map((group, idx) => {
        ...
        if (type === 'sharp') {
            ...
            mainValue = `<span class="value-money">£${Math.round(Number(money)).toLocaleString('en-GB')}</span>`;
        ...
        if (type === 'sharp') {
            ...
            metricValue = `£${Math.round(Number(alarm.incoming_money || alarm.stake || 0)).toLocaleString('en-GB')}`;
        } else if (type === 'dropping') {
            ...
            metricValue = incomingMoney > 0 ? `+£${Math.round(Number(incomingMoney)).toLocaleString('en-GB')}` : `${impactPct}%`;
        }
        ...
        if (historyCount > 0 && isOpen) {
            const historyItems = group.history.map(h => {
                ...
                if (type === 'sharp') {
                    ...
                    hValue = `£${Math.round(Number(h.incoming_money || h.stake || 0)).toLocaleString('en-GB')}`;
                } else if (type === 'dropping') {
                    ...
                    hValue = hIncoming > 0 ? `+£${Math.round(Number(hIncoming)).toLocaleString('en-GB')}` : `${(hImpact * 100).toFixed(0)}%`;
                }
...
function goToMatchFromAlarm(homeTeam, awayTeam, alarmType, alarmMarket, league, kickoff) {
    if (window.userPlan === 'test') {
        var _testFake = { home_team: homeTeam, away_team: awayTeam };
        if (!_isTestFreeMatch(_testFake)) return;
    }
    openAlarmId = null;
    closeAlarmsSidebar();
...
let desktopChartSectionOpen = true;
...
async function renderMatchAlarmsSection(homeTeam, awayTeam, reqId) {
    ...
    function renderCardForGroup(type, alarms, config) {
        ...
        if (type === 'sharp') {
            ...
            row3Left = `<span class="sm-mim-money">+£${Math.round(Number(incomingVol)).toLocaleString('en-GB')}</span> <span class="sm-money-label">gelen para</span>`;
            row3Right = `<span class="sm-mim-total">Toplam: £${Math.round(Number(totalMarketVol)).toLocaleString('en-GB')}</span>`;
        ...
        if (count > 1) {
            ...
            const tooltipTheme = {
                bigmoney: { title: 'Geçmiş Big Money Girişleri', pillLabel: (a) => `£${Math.round(Number(a.incoming_money || a.stake || 0)).toLocaleString('en-GB')}` },
            ...
            const tooltipItems = tooltipAlarms.map(a => {
                ...
                if (type === 'bigmoney') {
                    const money = Math.round(Number(a.incoming_money || a.stake || 0)).toLocaleString('en-GB');
                    const sel = a.selection || a.side || '-';
                    const totalVal = a.selection_total || a.total_selection || a.balance_after || a.volume || a.total_volume || 0;
                    const totalText = totalVal > 0 ? `Sonrası: £${Math.round(Number(totalVal)).toLocaleString('en-GB')}` : '';
...
function toggleDesktopChartSection() {
    ...
    const chevron = document.getElementById('desktopChartChevron');
    if (!body || !chevron) return;
    desktopChartSectionOpen = !desktopChartSectionOpen;
    if (desktopChartSectionOpen) {
        chevron.textContent = '▼';
        body.style.display = '';
    } else {
        chevron.textContent = '▲';
...
async function loadAdminDroppingData() {
    ...
    try {
        ...
        if (alarms.length === 0) {
            ...
            html += '</tbody></table>';
            if (alarms.length > 30) {
                html += `<div style="text-align:center; padding:12px; color:#5c636b; font-size:12px;">+${alarms.length - 30} daha fazla alarm...</div>`;
            }
        }
...
async function loadAdminMimData() {
    ...
    try {
        ...
        if (alarms.length === 0) {
            ...
            html += '</tbody></table>';
            if (alarms.length > 50) {
                html += `<div style="text-align:center; padding:12px; color:#5c636b; font-size:12px;">+${alarms.length - 50} daha fazla alarm...</div>`;
            }
        }
...
window._testFreeHashes = [];
window._testFreeTeams = [];

function checkWebLicense() {
    if (localStorage.getItem('license_plan') === 'test') {
        window.userPlan = 'test';
        window.userLicenseKey = '';
        if (typeof _isPro !== 'undefined') _isPro = false;
        return true;
    }
    ...
    if (savedKey && validUntil) {
        ...
        if (validDate > new Date()) {
            ...
            if (typeof _isPro !== 'undefined') _isPro = (window.userPlan === 'pro');
            return true;
        }
    }
    return false;
...
function updateLicenseDaysBadge(days) {
    ...
    if (days >= 36500) {
        ...
        text.textContent = window.SXFI18n ? window.SXFI18n.t('app.dyn.suresi_doldu') : 'Süresi doldu';
        badge.classList.add('danger');
    } else {
        text.textContent = (window.SXFI18n ? window.SXFI18n.t('app.dyn.kalan_pre') : 'Kalan ') + days + (window.SXFI18n ? window.SXFI18n.t('app.dyn.kalan_suf') : ' gün');
        if (days <= 3) {
            badge.classList.add('danger');
        } else if (days <= 7) {
...
async function validateWebLicense() {
    ...
    try {
        ...
        if (data.valid) {
            ...
            successDiv.innerHTML = '<div style="font-size:24px;margin-bottom:8px;">✓</div>' + (window.SXFI18n ? window.SXFI18n.t('app.dyn.lisans_aktif_kalan_gun') : 'Lisans aktif! Kalan gün: ') + daysRemaining;
            successDiv.style.display = 'block';
            progressBar.style.display = 'none';
            
            setTimeout(() => {
                hideLicenseGate();
                const _np = new URLSearchParams(window.location.search);
                const _next = _np.get('next');
...
async function activateTestMode() {
    try {
        var res = await fetch('/api/test/activate', { method: 'POST' });
        var data = await res.json();
        if (data.success) {
            localStorage.setItem('license_plan', 'test');
        ...
        console.error('Test mode activate error:', e);
    }
}

async function _loadTestFreeHashes() {
    if (window.userPlan !== 'test') return;
    try {
        var res = await fetch('/api/test/free-matches');
        var data = await res.json();
        window._testFreeHashes = data.hashes || [];
        window._testFreeTeams = data.teams || [];
    } catch(e) {
        window._testFreeHashes = [];
        window._testFreeTeams = [];
    }
...
function _isTestFreeAlarm(home, away) {
    if (window.userPlan !== 'test') return true;
    var h = (home || '').toLowerCase().trim();
    var a = (away || '').toLowerCase().trim();
    for (var ti = 0; ti < window._testFreeTeams.length; ti++) {
        var t = window._testFreeTeams[ti];
        var th = (t.home || '').toLowerCase().trim();
        var ta = (t.away || '').toLowerCase().trim();
        if (th && ta && th === h && ta === a) return true;
    }
    return false;
}

function _isTestFreeMatch(matchId) {
    if (window.userPlan !== 'test') return true;
...
function _addTestLockIcons() {
    ...
    document.querySelectorAll('.market-tabs .tab').forEach(function(tab) {
        ...
        if (m && m !== 'moneyway_1x2' && m !== 'dropping_1x2' && m !== 'live') {
...
async function initLicenseCheck() {
    if (!checkWebLicense()) {
        ...
        if (window.userPlan === 'test') {
            ...
            if (daysBadge) {
                var daysText = document.getElementById('licenseDaysText');
...
// ── Background Live Data Fetch (MW/Oran sayfalarında dk/skor için) ──
var _bgLiveInterval = null;

async function _fetchBackgroundLiveData() {
    if (_liveMode) return;
    try {
        var resp = await fetch('/api/live/matches');
        var data = await resp.json();
        _liveData = data.matches || [];
        _updateLiveCapsulesInDOM();
    } catch (e) {}
}

function _updateLiveCapsulesInDOM() {
    if (_liveMode) return;
    document.querySelectorAll('#matchesTableBody tr').forEach(function(row) {
        var teamsCell = row.querySelector('.match-teams');
        var dateCell = row.querySelector('.match-date');
        if (!teamsCell || !dateCell) return;
        var capsuleEl = teamsCell.querySelector('.live-capsule');
        var deskCapsuleEl = dateCell.querySelector('.live-capsule-desk');
        var matchId = row.getAttribute('data-match-id') || '';
        var teamsText = teamsCell.textContent || '';
        var parts = teamsText.split(/\s*[-–]\s*/);
        if (parts.length < 2) return;
        var home = parts[0].trim();
        var away = parts[parts.length - 1].trim();
        var lm = _findLiveMatchByHash(matchId) || _findLiveMatch(home, away);
        if (!lm) return;
        ...
        if (capsuleEl) {
            ...
            if (rawMin || sc) {
                ...
                if (sc) {
                    var sp = sc.split('-');
                    if (sp.length === 2) {
                        scoreDisp = '<span class="lc-divider"></span><span class="lc-score-val"><span class="lc-score-h">' + sp[0].trim() + '</span><span class="lc-score-sep">-</span><span class="lc-score-a">' + sp[1].trim() + '</span></span>';
                    }
                }
        ...
        if (deskCapsuleEl) {
            ...
            if (rawMin2 || sc2) {
                ...
                if (sc2) {
                    var sp2 = sc2.split('-');
                    if (sp2.length === 2) {
                        scoreDisp2 = '<span class="lc-divider"></span><span class="lc-score-h">' + sp2[0].trim() + '</span><span class="lc-score-sep">-</span><span class="lc-score-a">' + sp2[1].trim() + '</span>';
                    }
                }
...
var _liveData = [];
var _liveMarket = '1x2';
var _modalLiveData = null;
var _modalLiveMarket = '1x2';
var _liveDetailData = null;
var _liveDetailMarket = '1x2';
var _prevLiveScores = {};
var _finishedScores = {};

function proLockPanelHtml() {
    return '<div class="smart-money-empty" style="display:block">Canl\u0131 periyot verileri i\u00e7in Pro \u00fcyelik gerektirir.</div>';
}

function _liveProBanner() {
    return '<div class="live-pro-banner">' +
        ...
        ' Canl\u0131 veriler i\u00e7in <strong>Pro</strong> \u00fcyelik gerekmektedir' +
        ' <a href="/pricing" class="live-pro-banner-btn">Pro\'ya Ge\u00e7 \u2192</a>' +
        '</div>';
}

async function _renderLiveProLock() {
    _liveMode = true;
    document.querySelectorAll('.tab').forEach(function(t) { t.classList.remove('active'); });
    document.querySelectorAll('.mobile-tab-btn').forEach(function(t) { t.classList.remove('active'); });
    var liveTab = document.querySelector('.tab-live');
    if (liveTab) liveTab.classList.add('active');
    var mobileLive = document.querySelector('.mobile-live-btn');
    if (mobileLive) mobileLive.classList.add('active');
    var mcLabel = document.getElementById('matchCountLabel');
    if (mcLabel) mcLabel.textContent = 'Canl\u0131 Ma\u00e7lar ';
    var mcSuffix = document.getElementById('matchCountSuffix');
    if (mcSuffix) mcSuffix.textContent = '';
    var mcWrap = document.querySelector('.match-count');
    if (mcWrap) mcWrap.classList.add('mc-live');
    var lmt = document.getElementById('liveMarketTabs');
    if (lmt) lmt.style.display = 'none';
    var marketRow = document.querySelector('.mobile-tab-row.market-row');
    if (marketRow) marketRow.style.display = 'none';

    var table = document.querySelector('.matches-table');
    ...
    if (table) table.setAttribute('data-selection-count', '3');
    if (colgroup) colgroup.innerHTML =
        '<col class="col-fav"><col class="col-date"><col class="col-league"><col class="col-match">' +
        '<col class="col-selection"><col class="col-selection"><col class="col-selection"><col class="col-volume">';
    if (thead) thead.innerHTML = '<tr>' +
        '<th class="col-fav"></th>' +
        '<th class="col-date">DK</th>' +
        '<th class="col-league">L\u0130G</th>' +
        '<th class="col-match">MA\u00c7</th>' +
        '<th class="col-selection">1</th>' +
        '<th class="col-selection">X</th>' +
        '<th class="col-selection">2</th>' +
        '<th class="col-volume">HAC\u0130M</th>' +
        '</tr>';

    if (tbody) tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:30px;"><div class="loading-spinner"></div></td></tr>';
    ...
    var filtered = matches.filter(function(m) {
        var min = (m.minute || '').trim().toUpperCase();
        if (min === 'FT' || min === 'MS' || min === 'AET' || min === 'PEN') return false;
        if ((m.status || '').toLowerCase() === 'ft') return false;
        if (m.kickoff_utc) {
            var ko = new Date(m.kickoff_utc);
            if (!isNaN(ko.getTime())) {
                var diffM = (nowMs - ko.getTime()) / 60000;
                if (diffM >= 120) return false;
                if (diffM < 0 || diffM > 130) {
                    if (!min && !m.score) return false;
                }
            }
        } else if (!min && !m.score) {
            return false;
        }
        if (_liveCalcVol1x2(m) <= 0) return false;
        return true;
    });

    var _blurStyle = 'filter:blur(6px);-webkit-filter:blur(6px);user-select:none;pointer-events:none';
    var rowsHtml = '';
    var cardsHtml = '';
    ...
    for (var i = 0; i < filtered.length; i++) {
        var m = filtered[i];
        var o = m.odds || {};
        var matchMin = _calcLiveMatchMin(m);
        var scoreStr = m.score ? escLiveHtml(m.score) : '';
        var _liveFavTd = _buildLiveFavCell(m);

        var capHtml = '<div class="live-capsule live-capsule-desk">' +
            '<span class="lc-dot"></span>' +
            '<span class="lc-min">' + escLiveHtml(matchMin) + '</span>';
        if (scoreStr) {
            var sp = scoreStr.split('-');
            if (sp.length === 2) {
                capHtml += '<span class="lc-divider"></span>' +
                    '<span class="lc-score-h">' + sp[0].trim() + '</span>' +
                    '<span class="lc-score-sep">-</span>' +
                    '<span class="lc-score-a">' + sp[1].trim() + '</span>';
            } else {
                capHtml += '<span class="lc-divider"></span><span class="lc-score-val">' + escLiveHtml(scoreStr) + '</span>';
            }
        }
        capHtml += '</div>';

        var vol = _liveCalcVol1x2(m);
        var b1 = _liveMwBlock('1', o['1']);
        var bX = _liveMwBlock('X', o['X']);
        var b2 = _liveMwBlock('2', o['2']);

        rowsHtml += '<tr class="live-row">' +
            _liveFavTd +
            '<td class="match-date">' + capHtml + '</td>' +
            ...
            '<td class="mw-outcomes-cell" colspan="3" style="' + _blurStyle + '"><div class="mw-grid mw-grid-3">' + b1 + bX + b2 + '</div></td>' +
            '<td class="volume-cell" style="' + _blurStyle + '">' + formatLiveVol(vol) + '</td></tr>';

        var _lmk = _getMatchKey(m);
        var _lmkIsFav = _userFavorites.has(_lmk);
        var _lmkFc = _favCounts[_lmk] || 0;
        var cardScore = m.score ? escLiveHtml(m.score) : (matchMin === 'MS' ? '-' : '');
        var mobCapHtml = '';
        if (matchMin || cardScore) {
            mobCapHtml = '<div class="live-capsule">' +
                '<span class="lc-dot"></span>' +
                '<span class="lc-min">' + escLiveHtml(matchMin) + '</span>' +
                (cardScore ? '<span class="lc-divider"></span><span class="lc-score-val">' + cardScore + '</span>' : '') +
                '</div>';
        }
        cardsHtml += '<div class="match-card odds-card moneyway-card" data-matchkey="' + _lmk.replace(/"/g, '&quot;') + '">' +
            _mobileFavLine(_lmk, _lmkIsFav, _lmkFc) +
            '<div class="odds-card-header">' +
            '<div class="odds-card-teams">' + escLiveHtml(m.home_team) + ' \u2013 ' + escLiveHtml(m.away_team) + '</div>' +
            '<div class="odds-card-volume" style="' + _blurStyle + '">' + formatLiveVol(vol) + '</div>' +
            '</div>' +
            '<div class="odds-card-meta"><span>' + escLiveHtml(m.league) + '</span>' + mobCapHtml + '</div>' +
            '<div class="odds-card-row three" style="' + _blurStyle + '">' +
    ...
    if (filtered.length === 0) {
        rowsHtml += '<tr><td colspan="8" style="text-align:center;padding:30px;color:#6b7280;">\u015eu an canl\u0131 ma\u00e7 bulunamad\u0131</td></tr>';
    }
    ...
    if (mc) mc.textContent = filtered.length;
    if (_favFilterActive) setTimeout(function() { _applyFavoritesFilter(); }, 10);
}
...
function switchToLive() {
    ...
    if (mcWrap) mcWrap.classList.add('mc-live');
    var lmt = document.getElementById('liveMarketTabs');
    if (lmt) lmt.style.display = 'flex';
    var marketRow = document.querySelector('.mobile-tab-row.market-row');
    if (marketRow) marketRow.style.display = 'none';
    var tbody = document.getElementById('matchesTableBody');
    if (tbody) tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:40px;"><div class="loading-spinner"></div> Canli maclar yukleniyor...</td></tr>';
    var cardList = document.getElementById('matchCardList');
    if (cardList) cardList.innerHTML = '<div style="text-align:center;padding:40px;"><div class="loading-spinner"></div> Canli maclar yukleniyor...</div>';
    loadLiveMatches();
    if (_liveInterval) clearInterval(_liveInterval);
    _liveInterval = setInterval(loadLiveMatches, 30000);
}

function switchFromLive() {
    _liveMode = false;
    if (_liveInterval) { clearInterval(_liveInterval); _liveInterval = null; }
    _prevLiveScores = {};
    var mcLabel = document.getElementById('matchCountLabel');
    if (mcLabel) mcLabel.textContent = '';
    var mcSuffix = document.getElementById('matchCountSuffix');
    if (mcSuffix) mcSuffix.textContent = ' Ma\u00e7';
    var mcWrap = document.querySelector('.match-count');
    if (mcWrap) mcWrap.classList.remove('mc-live');
    var lmt = document.getElementById('liveMarketTabs');
    if (lmt) lmt.style.display = 'none';
    var marketRow = document.querySelector('.mobile-tab-row.market-row');
    if (marketRow) marketRow.style.display = '';
    var thead = document.querySelector('.matches-table thead');
...
if (typeof window.setTab === 'function') {
    var _realSetTab = window.setTab;
    window.setTab = function(market) {
        if (_liveMode) switchFromLive();
        _realSetTab(market);
    };
}

if (typeof window.setMobileGroup === 'function') {
    var _realSetMobileGroup = window.setMobileGroup;
    window.setMobileGroup = function(group) {
...
document.addEventListener('click', function(e) {
    var tab = e.target.closest('.tab[data-market]');
    if (tab && tab.dataset.market !== 'live' && _liveMode) {
        switchFromLive();
    }
});

function setLiveMarket(market) {
    _liveMarket = market;
    document.querySelectorAll('.live-market-btn').forEach(function(b) {
        b.classList.toggle('active', b.getAttribute('data-lmarket') === market);
    });
    document.querySelectorAll('.live-market-dropdown-item').forEach(function(b) {
        b.classList.toggle('active', b.getAttribute('data-lmarket') === market);
    });
    renderLiveMatches(_liveData);
}

function toggleLiveMarketDropdown(e) {
    e.stopPropagation();
    var dd = document.getElementById('liveMarketTabs');
    if (dd) dd.classList.toggle('open');
}

function setLiveMarketFromDropdown(market, label) {
    setLiveMarket(market);
    var lbl = document.getElementById('liveMarketDropdownLabel');
...
async function loadLiveMatches() {
    try {
        ...
        _liveData = data.matches || [];
        var _goalHashes = {};
        var _prevCount = Object.keys(_prevLiveScores).length;
        for (var gi = 0; gi < _liveData.length; gi++) {
            var gm = _liveData[gi];
            var gh = gm.match_id_hash;
            var gs = gm.score || '';
            if (gh && gs && _prevLiveScores[gh] && _prevLiveScores[gh] !== gs) {
                var oldParts = _prevLiveScores[gh].split('-');
                var newParts = gs.split('-');
                var oldH = parseInt(oldParts[0]) || 0;
                var oldA = parseInt(oldParts[1]) || 0;
                var newH = parseInt(newParts[0]) || 0;
                var newA = parseInt(newParts[1]) || 0;
                var side = 'both';
                if (newH > oldH && newA === oldA) side = 'home';
                else if (newA > oldA && newH === oldH) side = 'away';
                _goalHashes[gh] = { side: side, oldScore: _prevLiveScores[gh] };
                console.log('[GOL] ' + gm.home_team + ' vs ' + gm.away_team + ': ' + _prevLiveScores[gh] + ' → ' + gs + ' (' + side + ')');
            }
            if (gh && gs) _prevLiveScores[gh] = gs;
        }
        if (_prevCount === 0) console.log('[GOL] İlk yükleme - ' + Object.keys(_prevLiveScores).length + ' maç kaydedildi');
        renderLiveMatches(_liveData, _goalHashes);
        var now = new Date();
        var lut = document.getElementById('liveUpdateTime');
        if (lut) lut.textContent =
            String(now.getHours()).padStart(2, '0') + ':' +
            String(now.getMinutes()).padStart(2, '0') + ':' +
            String(now.getSeconds()).padStart(2, '0');
    } catch (e) {
        console.error('Live matches error:', e && e.message ? e.message : e);
    }
}

function formatLiveVol(v) {
    if (!v && v !== 0) return '-';
...
function escLiveHtml(s) {
    if (!s) return '';
    return s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;');
}

function _liveDonut(pct, sz) {
    sz = sz || 44;
    var num = parseFloat(String(pct).replace(/[^0-9.]/g, '')) || 0;
    var sw = 4;
    var r = (sz - sw * 2) / 2;
    var c = 2 * Math.PI * r;
    var off = c - (num / 100) * c;
    var hi = num >= 50;
    var fc = hi ? '#22c55e' : '#1a1d21';
    var tc = hi ? '#ffffff' : '#9ca3af';
    return '<svg width="' + sz + '" height="' + sz + '" viewBox="0 0 ' + sz + ' ' + sz + '">' +
        '<circle cx="' + sz/2 + '" cy="' + sz/2 + '" r="' + r + '" fill="none" stroke="#2a2e33" stroke-width="' + sw + '"/>' +
        '<circle cx="' + sz/2 + '" cy="' + sz/2 + '" r="' + r + '" fill="none" stroke="' + fc + '" stroke-width="' + sw + '"' +
        ' stroke-dasharray="' + c + '" stroke-dashoffset="' + off + '" stroke-linecap="round" transform="rotate(-90 ' + sz/2 + ' ' + sz/2 + ')"/>' +
        '<text x="' + sz/2 + '" y="' + sz/2 + '" text-anchor="middle" dominant-baseline="central" fill="' + tc + '" font-size="9" font-weight="600">' + num.toFixed(0) + '%</text>' +
        '</svg>';
}

function _liveGetOu(m, line) {
    var ols = m.ou_lines || {};
    return ols[line] || {};
}

function _liveCalcVol1x2(m) {
    var o = m.odds || {};
    var v = 0;
    if (o['1']) v += o['1'].volume || 0;
    if (o['X']) v += o['X'].volume || 0;
    if (o['2']) v += o['2'].volume || 0;
    return v;
}

function _liveCalcVolOU(ou) {
    var v = 0;
    if (ou['U']) v += ou['U'].volume || 0;
    if (ou['O']) v += ou['O'].volume || 0;
    return v;
}

function renderLiveMatches(matches, goalHashes) {
    if (!goalHashes) goalHashes = {};
    var table = document.querySelector('.matches-table');
    var tbody = document.getElementById('matchesTableBody');
    var thead = table ? table.querySelector('thead') : null;
    var colgroup = table ? table.querySelector('colgroup') : null;
    var cardList = document.getElementById('matchCardList');
    var emptyHtml = '<div class="live-empty">' +
        '<div class="live-empty-icon">\u26BD</div>' +
        '<div class="live-empty-text">Su an canli mac bulunamadi</div>' +
        '<div class="live-empty-sub">Canli maclar basladiginda burada gorunecek</div>' +
        '</div>';
    if (!matches || matches.length === 0) {
        if (tbody) tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:40px;">' + emptyHtml + '</td></tr>';
        if (thead) thead.innerHTML = '<tr><th colspan="8" style="text-align:center;">Canli mac bulunamadi</th></tr>';
        if (cardList) cardList.innerHTML = emptyHtml;
        var mc = document.getElementById('matchCount');
        if (mc) mc.textContent = '0';
        return;
    }
    var is1x2 = (_liveMarket === '1x2');
    var ouLine = is1x2 ? '' : _liveMarket.replace('ou', '');
    if (is1x2) {
        if (table) table.setAttribute('data-selection-count', '3');
        if (colgroup) colgroup.innerHTML =
            '<col class="col-fav"><col class="col-date"><col class="col-league"><col class="col-match">' +
        ...
        if (table) table.setAttribute('data-selection-count', '2');
        if (colgroup) colgroup.innerHTML =
            '<col class="col-fav"><col class="col-date"><col class="col-league"><col class="col-match">' +
            '<col class="col-selection"><col class="col-selection"><col class="col-volume">';
        if (thead) thead.innerHTML = '<tr>' +
            '<th class="col-fav"></th>' +
            '<th class="col-date">DK</th>' +
            '<th class="col-league">LİG</th>' +
            '<th class="col-match">MAÇ</th>' +
            '<th class="col-selection">ALT</th>' +
            '<th class="col-selection">ÜST</th>' +
            '<th class="col-volume">HACİM</th>' +
            '</tr>';
    }
    var nowMs = Date.now();
    var filtered = matches.filter(function(m) {
        ...
        if (is1x2) return _liveCalcVol1x2(m) > 0;
        return _liveCalcVolOU(_liveGetOu(m, ouLine)) > 0;
    });
    if (filtered.length === 0) {
        if (tbody) tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:40px;">' + emptyHtml + '</td></tr>';
        if (cardList) cardList.innerHTML = emptyHtml;
        var mc0 = document.getElementById('matchCount');
        if (mc0) mc0.textContent = '0';
        return;
    }
    var sorted = filtered.slice();
    sorted.sort(function(a, b) {
        var va = is1x2 ? _liveCalcVol1x2(a) : _liveCalcVolOU(_liveGetOu(a, ouLine));
        var vb = is1x2 ? _liveCalcVol1x2(b) : _liveCalcVolOU(_liveGetOu(b, ouLine));
        return vb - va;
    });
    var rowsHtml = '';
    var cardsHtml = '';
    ...
    for (var i = 0; i < sorted.length; i++) {
        ...
        var matchMin = _calcLiveMatchMin(m);
        var scoreStr = m.score ? escLiveHtml(m.score) : '';
        var onclickAttr = 'onclick="openLiveDetail(\'' + m.match_id_hash + '\',\'' + escLiveHtml(m.home_team) + '\',\'' + escLiveHtml(m.away_team) + '\')"';
        var _liveMk = _getMatchKey(m);
        var _liveMkAttr = ' data-matchkey="' + _liveMk.replace(/"/g, '&quot;') + '"';
        var showScore = (matchMin === 'MS') ? (scoreStr || '-') : scoreStr;
        var goalInfo = m.match_id_hash ? goalHashes[m.match_id_hash] : null;
        var goalSide = goalInfo ? goalInfo.side : null;
        var goalOldScore = goalInfo ? goalInfo.oldScore : '';
        var scoreHtml = '';
        if (showScore) {
            var sp = showScore.split('-');
            if (sp.length === 2) {
                var hCls = goalSide === 'home' || goalSide === 'both' ? ' goal-pop' : '';
                var aCls = goalSide === 'away' || goalSide === 'both' ? ' goal-pop' : '';
                scoreHtml = '<div class="time-line live-score-badge">' +
                    '<span class="score-h' + hCls + '">' + sp[0].trim() + '</span>' +
                    '<span class="score-sep">-</span>' +
                    '<span class="score-a' + aCls + '">' + sp[1].trim() + '</span>' +
                    '</div>';
            } else {
                scoreHtml = '<div class="time-line live-score-badge">' + showScore + '</div>';
            }
        }
        var minExtra = '';
        if (/^HT$/i.test(matchMin) || /^MS$/i.test(matchMin)) minExtra = ' is-ht';
        else if (/\d+\+/.test(matchMin)) minExtra = ' is-extra';
        var goalData = '';
        if (goalSide) {
            goalData = ' data-goal-side="' + goalSide + '" data-old-score="' + escLiveHtml(goalOldScore) + '" data-new-score="' + escLiveHtml(showScore) + '" data-real-min="' + escLiveHtml(matchMin) + '"';
        }
        var minCell = '<td class="match-date">' +
            '<div class="dk-cell-inner"' + goalData + '>' +
            '<div class="date-line live-min-badge' + minExtra + '" style="color:#e5484d;">' + escLiveHtml(matchMin) + '</div>' +
            scoreHtml +
            '</div></td>';
        if (is1x2) {
            var vol = _liveCalcVol1x2(m);
            ...
            var bX = _liveMwBlock('X', o['X']);
            var b2 = _liveMwBlock('2', o['2']);
            var capGoalCls = goalSide ? ' lc-goal' : '';
            var capMinText = goalSide ? 'GOL!' : escLiveHtml(matchMin);
            var capScoreHtml = '';
            if (showScore) {
                var csp = showScore.split('-');
                if (csp.length === 2 && goalSide) {
                    var hGoal = (goalSide === 'home' || goalSide === 'both');
                    var aGoal = (goalSide === 'away' || goalSide === 'both');
                    capScoreHtml = '<span class="lc-divider"></span>' +
                        '<span class="lc-score-h' + (hGoal ? ' lc-scored' : '') + '">' + csp[0].trim() + '</span>' +
                        '<span class="lc-score-sep">-</span>' +
                        '<span class="lc-score-a' + (aGoal ? ' lc-scored' : '') + '">' + csp[1].trim() + '</span>';
                } else {
                    capScoreHtml = '<span class="lc-divider"></span><span class="lc-score-val">' + escLiveHtml(showScore) + '</span>';
                }
            }
            var deskLiveCap = '<div class="live-capsule live-capsule-desk' + capGoalCls + '">' +
                '<span class="lc-dot"></span>' +
                '<span class="lc-min">' + capMinText + '</span>' +
                capScoreHtml +
                '</div>';
            var _liveFavTd = _buildLiveFavCell(m);
            rowsHtml += '<tr class="live-row"' + _liveMkAttr + ' ' + onclickAttr + '>' +
                _liveFavTd +
                '<td class="match-date">' + deskLiveCap + '</td>' +
                '<td class="match-league" title="' + escLiveHtml(m.league) + '">' + escLiveHtml(m.league) + '</td>' +
                '<td class="match-teams">' + escLiveHtml(m.home_team) + '<span class="vs">-</span>' + escLiveHtml(m.away_team) + '</td>' +
            ...
            var volOU = _liveCalcVolOU(ou);
            var bU = _liveMwBlock('Alt', ou['U']);
            var bO = _liveMwBlock('Ust', ou['O']);
            var capGoalClsOU = goalSide ? ' lc-goal' : '';
            var capMinTextOU = goalSide ? 'GOL!' : escLiveHtml(matchMin);
            var capScoreHtmlOU = '';
            if (showScore) {
                var cspOU = showScore.split('-');
                if (cspOU.length === 2 && goalSide) {
                    var hGoalOU = (goalSide === 'home' || goalSide === 'both');
                    var aGoalOU = (goalSide === 'away' || goalSide === 'both');
                    capScoreHtmlOU = '<span class="lc-divider"></span>' +
                        '<span class="lc-score-h' + (hGoalOU ? ' lc-scored' : '') + '">' + cspOU[0].trim() + '</span>' +
                        '<span class="lc-score-sep">-</span>' +
                        '<span class="lc-score-a' + (aGoalOU ? ' lc-scored' : '') + '">' + cspOU[1].trim() + '</span>';
                } else {
                    capScoreHtmlOU = '<span class="lc-divider"></span><span class="lc-score-val">' + escLiveHtml(showScore) + '</span>';
                }
            }
            var deskLiveCapOU = '<div class="live-capsule live-capsule-desk' + capGoalClsOU + '">' +
                '<span class="lc-dot"></span>' +
                '<span class="lc-min">' + capMinTextOU + '</span>' +
                capScoreHtmlOU +
                '</div>';
            var _liveFavTdOU = _buildLiveFavCell(m);
            rowsHtml += '<tr class="live-row"' + _liveMkAttr + ' ' + onclickAttr + '>' +
                _liveFavTdOU +
    ...
    if (tbody) tbody.innerHTML = rowsHtml;
    if (cardList) cardList.innerHTML = cardsHtml;
    document.querySelectorAll('.goal-pop').forEach(function(el) {
        el.addEventListener('animationend', function() { el.classList.remove('goal-pop'); }, { once: true });
    });
    document.querySelectorAll('.goal-flash-card').forEach(function(el) {
        el.addEventListener('animationend', function() { el.classList.remove('goal-flash-card'); }, { once: true });
    });
    document.querySelectorAll('.dk-cell-inner[data-goal-side]').forEach(function(cell) {
        _startGoalTakeover(cell);
    });
    document.querySelectorAll('.live-min-badge[data-goal-min]').forEach(function(el) {
        var realMin = el.getAttribute('data-goal-min');
        setTimeout(function() {
            if (el.parentNode) {
                el.textContent = realMin;
                el.removeAttribute('data-goal-min');
            }
        }, 12000);
    });
    var mc = document.getElementById('matchCount');
    if (mc) mc.textContent = sorted.length;
    if (_favFilterActive) setTimeout(function() { _applyFavoritesFilter(); }, 10);
}

function _startGoalTakeover(cell) {
    var goalSide = cell.getAttribute('data-goal-side');
    var oldScore = cell.getAttribute('data-old-score') || '0-0';
    var newScore = cell.getAttribute('data-new-score') || '0-0';
    var realMin = cell.getAttribute('data-real-min') || '';
    cell.classList.add('goal-takeover');
    var golEl = document.createElement('div');
    golEl.className = 'goal-takeover-text';
    golEl.textContent = 'GOL!';
    cell.appendChild(golEl);
    setTimeout(function() {
        if (!cell.parentNode) return;
        if (golEl.parentNode) golEl.remove();
        var oldParts = oldScore.split('-');
        var newParts = newScore.split('-');
        var oh = oldParts.length === 2 ? oldParts[0].trim() : '0';
        var oa = oldParts.length === 2 ? oldParts[1].trim() : '0';
        var nh = newParts.length === 2 ? newParts[0].trim() : '0';
        var na = newParts.length === 2 ? newParts[1].trim() : '0';
        var scoreEl = document.createElement('div');
        scoreEl.className = 'goal-takeover-score';
        var hClass = (oh !== nh) ? ' slide-up' : '';
        var aClass = (oa !== na) ? ' slide-up' : '';
        scoreEl.innerHTML = '<span class="score-digit' + hClass + '">' + nh + '</span>' +
            '<span style="margin:0 2px;">-</span>' +
            '<span class="score-digit' + aClass + '">' + na + '</span>';
        cell.appendChild(scoreEl);
        setTimeout(function() {
            if (!cell.parentNode) return;
            cell.classList.remove('goal-takeover');
            if (scoreEl.parentNode) scoreEl.remove();
            cell.removeAttribute('data-goal-side');
            cell.removeAttribute('data-old-score');
            cell.removeAttribute('data-new-score');
            cell.removeAttribute('data-real-min');
        }, 6000);
    }, 6000);
}

function _calcLiveMatchMin(m) {
    if (m.status === 'ft') return 'MS';
    var rawMin = (m.minute || '').trim();
    if (!rawMin) {
        if (m.kickoff_utc) {
            var ko = new Date(m.kickoff_utc);
            if (!isNaN(ko.getTime())) {
                var diff = Math.floor((Date.now() - ko.getTime()) / 60000);
                if (diff >= 0 && diff <= 130) return diff + "'";
            }
        }
        return '';
    }
    var upper = rawMin.toUpperCase();
    if (upper === 'FT') return 'MS';
    var markers = ['HT', '1H', '2H', 'ET', 'PEN'];
    for (var k = 0; k < markers.length; k++) {
        if (upper === markers[k]) return markers[k];
    }
    var stoppage = rawMin.match(/^(\d{1,3})\+(\d{1,2})'?$/);
    if (stoppage) {
        return stoppage[1] + '+' + stoppage[2] + "'";
    }
    if (/^\d{1,3}'?$/.test(rawMin)) {
        var n = parseInt(rawMin, 10);
        if (n >= 0 && n <= 130) {
            return n + "'";
        }
    }
    if (/\d{1,2}:\d{2}/.test(rawMin)) return '';
    return escLiveHtml(rawMin);
}

function _liveMwBlock(label, d) {
    if (!d) return renderMoneywayBlock(label, 0, '-', '');
    return renderMoneywayBlock(label, d.share || 0, d.odds || '-', d.volume || '');
}

function _liveMobileCard(m, marketOrLine, onclickAttr, goalSide) {
    var is1x2 = (marketOrLine === '1x2');
    var cardCls = 'match-card odds-card moneyway-card' + (goalSide ? ' goal-flash-card' : '');
    var _lmk = _getMatchKey(m);
    var _lmkIsFav = _userFavorites.has(_lmk);
    var _lmkFc = _favCounts[_lmk] || 0;
    var html = '<div class="' + cardCls + '" data-matchkey="' + _lmk.replace(/"/g, '&quot;') + '" ' + onclickAttr + '>';
    html += _mobileFavLine(_lmk, _lmkIsFav, _lmkFc);
    html += '<div class="odds-card-header">';
    var cardMin = _calcLiveMatchMin(m);
    var cardScore = m.score ? escLiveHtml(m.score) : (cardMin === 'MS' ? '-' : '');
    var liveCapHtml = '';
    if (cardMin || cardScore) {
        var minDisp = goalSide ? 'GOL!' : cardMin;
        var scoreDisp = '';
        if (cardScore) {
            var csp = cardScore.split('-');
            if (csp.length === 2 && goalSide) {
                var chCls = goalSide === 'home' || goalSide === 'both' ? ' lc-scored' : '';
                var caCls = goalSide === 'away' || goalSide === 'both' ? ' lc-scored' : '';
                scoreDisp = '<span class="lc-score-h' + chCls + '">' + csp[0].trim() + '</span><span class="lc-score-sep">-</span><span class="lc-score-a' + caCls + '">' + csp[1].trim() + '</span>';
            } else {
                scoreDisp = '<span class="lc-score-val">' + cardScore + '</span>';
            }
        }
        var mobCapGoalCls = goalSide ? ' lc-goal' : '';
        liveCapHtml = '<div class="live-capsule' + mobCapGoalCls + '">' +
            '<span class="lc-dot"></span>' +
            '<span class="lc-min"' + (goalSide ? ' data-goal-min="' + cardMin + '"' : '') + '>' + minDisp + '</span>' +
            (scoreDisp ? '<span class="lc-divider"></span>' + scoreDisp : '') +
            '</div>';
    }
    html += '<div class="odds-card-teams">' + escLiveHtml(m.home_team) + ' \u2013 ' + escLiveHtml(m.away_team) + '</div>';
    var vol = 0;
    if (is1x2) {
        vol = _liveCalcVol1x2(m);
    } else {
        var ou = _liveGetOu(m, marketOrLine);
    ...
    if (is1x2) {
        ...
        if (_isTestLc) {
            ...
            html += _liveMobileBlock('2', o['2'], vol);
        }
        html += '</div>';
    } else {
        var ouD = _liveGetOu(m, marketOrLine);
...
function _liveMobileBlock(label, d, totalVol) {
    var odds = '-';
    if (d && d.odds) {
        odds = d.odds >= 100 ? d.odds.toFixed(0) : d.odds.toFixed(2);
    }
    var pct = '';
    var moneyStr = '';
    if (d && d.share !== null && d.share !== undefined) {
        var pn = parseFloat(String(d.share));
        if (!isNaN(pn)) {
            pct = pn.toFixed(0) + '%';
            if (totalVol && totalVol > 0) {
                var bm = totalVol * pn / 100;
                moneyStr = bm >= 1000000 ? '£' + (bm/1000000).toFixed(1) + 'M' : bm >= 1000 ? '£' + (bm/1000).toFixed(1) + 'k' : '£' + Math.round(bm);
            }
        }
    }
    return '<div class="odds-block mw-block">' +
...
async function openLiveDetail(hash, home, away) {
    _openLiveInMainModal(hash, home, away);
}

async function _openLiveInMainModal(hash, home, away) {
    var match = null;
    var matchIdx = -1;
    var usedSource = 'filtered';
    var homeLow = (home || '').toLowerCase().trim();
    var awayLow = (away || '').toLowerCase().trim();
    function _fuzzySearch(arr, label) {
        for (var i = 0; i < arr.length; i++) {
            var mh = (arr[i].home_team || '').toLowerCase().trim();
            var ma = (arr[i].away_team || '').toLowerCase().trim();
            if (mh === homeLow && ma === awayLow) { return { m: arr[i], idx: i }; }
            if ((mh.includes(homeLow) || homeLow.includes(mh)) && (ma.includes(awayLow) || awayLow.includes(ma))) {
                console.log('[LiveModal] Fuzzy matched in ' + label + ':', mh, 'vs', ma, 'at index', i);
                return { m: arr[i], idx: i };
            }
        }
        return null;
    }
    var fArr = (typeof filteredMatches !== 'undefined' && filteredMatches.length > 0) ? filteredMatches : [];
    var mArr = (typeof matches !== 'undefined' && matches.length > 0) ? matches : [];
    console.log('[LiveModal] Searching', homeLow, 'vs', awayLow, 'in filtered:', fArr.length, 'all:', mArr.length);
    var res = _fuzzySearch(fArr, 'filteredMatches');
    if (res) { match = res.m; matchIdx = res.idx; usedSource = 'filtered'; }
    if (!match && mArr.length > fArr.length) {
        res = _fuzzySearch(mArr, 'matches');
        if (res) { match = res.m; matchIdx = res.idx; usedSource = 'all'; }
    }
    if (match && matchIdx >= 0) {
        if (usedSource === 'all' && typeof matches !== 'undefined') {
            console.log('[LiveModal] Found in all matches (not in filtered), injecting into filteredMatches');
            filteredMatches.push(match);
            matchIdx = filteredMatches.length - 1;
        }
        console.log('[LiveModal] Found prematch match (' + usedSource + '), opening with openMatchModal idx=', matchIdx);
        await openMatchModal(matchIdx);
    } else {
        console.log('[LiveModal] No prematch match found in either list, using openMatchModalFromAPI');
        await openMatchModalFromAPI(home, away, '', '');
    }
    setTimeout(function() {
        var liveTab = document.getElementById('modalLiveTab');
        if (liveTab && liveTab.style.display !== 'none') {
            switchMobileTab('live');
        } else {
            var waitCount = 0;
            var waitInt = setInterval(function() {
                waitCount++;
                var lt = document.getElementById('modalLiveTab');
...
function _buildDetailTabs(snapshots) {
    var tabsEl = document.getElementById('liveDetailTabs');
    var lines = {};
    for (var i = 0; i < snapshots.length; i++) {
        var ols = snapshots[i].ou_lines || {};
        for (var k in ols) { lines[k] = true; }
    }
    var sortedLines = Object.keys(lines).sort(function(a, b) { return parseFloat(a) - parseFloat(b); });
    var html = '<button class="live-dtab' + (_liveDetailMarket === '1x2' ? ' active' : '') + '" onclick="setLiveDetailMarket(\'1x2\')">1X2</button>';
    for (var j = 0; j < sortedLines.length; j++) {
        var ln = sortedLines[j];
        var mk = 'ou' + ln;
        html += '<button class="live-dtab' + (_liveDetailMarket === mk ? ' active' : '') + '" onclick="setLiveDetailMarket(\'' + mk + '\')">' + escLiveHtml(ln) + '</button>';
    }
    tabsEl.innerHTML = html;
}

function setLiveDetailMarket(market) {
    _liveDetailMarket = market;
    document.querySelectorAll('.live-dtab').forEach(function(b) {
        b.classList.toggle('active', false);
    });
    var label = market === '1x2' ? '1X2' : market.replace('ou', '');
    document.querySelectorAll('.live-dtab').forEach(function(b) {
        if ((market === '1x2' && b.textContent === '1X2') || (market !== '1x2' && b.textContent === label)) {
            b.classList.add('active');
        }
    });
    if (_liveDetailData) renderLiveDetail(_liveDetailData, market);
}

function closeLiveDetail() {
    document.getElementById('liveDetailModal').style.display = 'none';
    _liveDetailData = null;
}

function _minuteToNum(m) {
    if (m === null || m === undefined || m === '') return -1;
    var s = String(m).replace(/'/g, '').trim();
    if (s === 'HT') return 45.5;
    if (s === 'FT') return 120;
    if (s === 'ET') return 105;
    if (s === 'PEN') return 115;
    var stop = s.match(/^(\d+)\+(\d+)$/);
    if (stop) return parseInt(stop[1]) + parseInt(stop[2]) * 0.01;
    var n = parseInt(s);
    return isNaN(n) ? -1 : n;
}

function renderLiveDetail(snapshots, market) {
    var body = document.getElementById('liveDetailBody');
    if (!snapshots || snapshots.length === 0) {
        body.innerHTML = '<div class="live-loading">Henuz periyot verisi yok</div>';
        return;
    }

    snapshots = snapshots.slice().sort(function(a, b) {
        var am = _minuteToNum(a.minute), bm = _minuteToNum(b.minute);
        if (am !== bm) return bm - am;
        return (a.snapshot_at || '') > (b.snapshot_at || '') ? -1 : 1;
    });

    var is1x2 = (market === '1x2');
    var ouLine = is1x2 ? '' : market.replace('ou', '');
    var sels = is1x2 ? ['1', 'X', '2'] : ['U', 'O'];
    var selLabels = is1x2 ? ['1', 'X', '2'] : [escLiveHtml(ouLine) + ' ALT', escLiveHtml(ouLine) + ' \u00dcST'];

    var colsPerSel = 3;
    var html = '<div class="live-detail-table-wrap"><table class="live-period-table">';
    html += '<thead>';
    html += '<tr class="lpt-group-row">';
    html += '<th rowspan="2" class="lpt-dkskor-th"><span class="lpt-th-main">DK</span><span class="lpt-th-sub">SKOR</span></th>';
    for (var g = 0; g < sels.length; g++) {
        html += '<th colspan="' + colsPerSel + '" class="lpt-group-th">' + selLabels[g] + '</th>';
    }
    html += '</tr>';
    html += '<tr class="lpt-col-row">';
    for (var h = 0; h < sels.length; h++) {
        html += '<th class="lpt-dual-th' + (h > 0 ? ' lpt-sel-divider' : '') + '"><span class="lpt-th-main">ORAN</span><span class="lpt-th-sub">Oran de\u011fi\u015fim</span></th>';
        html += '<th class="lpt-dual-th"><span class="lpt-th-main">VOLUME</span><span class="lpt-th-sub">Gelen para</span></th>';
        html += '<th class="lpt-dual-th"><span class="lpt-th-main">Y\u00dcZDE</span><span class="lpt-th-sub">%de\u011fi\u015fimi</span></th>';
    }
    html += '</tr>';
    html += '</thead><tbody>';

    var totals = {};
    sels.forEach(function(sel) { totals[sel] = 0; });

    for (var i = 0; i < snapshots.length; i++) {
        var s = snapshots[i];
        var prevS = (i < snapshots.length - 1) ? snapshots[i + 1] : null;

        var minVal = s.minute;
        var minStr = '-';
        if (minVal !== '' && minVal !== null && minVal !== undefined) {
            var mv = String(minVal);
            minStr = mv.endsWith("'") ? mv : mv + "'";
        }
        var scoreStr = s.score || '-';

        html += '<tr>';
        html += '<td class="period-dk-skor"><span class="dk-val">' + escLiveHtml(minStr) + '</span><span class="skor-val">' + escLiveHtml(scoreStr) + '</span></td>';

        var dataGroup = is1x2 ? (s['1x2'] || {}) : ((s.ou_lines || {})[ouLine] || {});
        var prevDataGroup = prevS ? (is1x2 ? (prevS['1x2'] || {}) : ((prevS.ou_lines || {})[ouLine] || {})) : null;

        for (var j = 0; j < sels.length; j++) {
            var sel = sels[j];
            var sd = dataGroup[sel] || {};
            var odds = (sd.odds !== undefined && sd.odds !== null) ? parseFloat(sd.odds) : null;
            var pct = sd.share || 0;
            var vol = sd.volume || 0;
            totals[sel] += vol;

            var oddsStr = odds !== null ? odds.toFixed(2) : '-';
            var pctStr = pct ? pct.toFixed(1) + '%' : '-';

            var deltaHtml = '<span class="period-delta-neutral">-</span>';
            if (prevDataGroup && odds !== null) {
                var prevOdds = (prevDataGroup[sel] || {}).odds;
                if (prevOdds !== undefined && prevOdds !== null) {
                    var delta = odds - parseFloat(prevOdds);
                    if (Math.abs(delta) >= 0.005) {
                        var dSign = delta > 0 ? '+' : '';
                        var dCls = delta > 0 ? 'period-delta-up' : 'period-delta-down';
                        deltaHtml = '<span class="' + dCls + '">' + dSign + delta.toFixed(2) + '</span>';
                    }
                }
            }
            ...
            var prevPct = 0;
            if (prevDataGroup) {
                prevPct = (prevDataGroup[sel] || {}).share || 0;
            }
            var pctDelta = pct - prevPct;
            var pctDeltaHtml = '<span class="period-delta-neutral">-</span>';
            if (prevS && Math.abs(pctDelta) >= 0.05) {
                var pdSign = pctDelta > 0 ? '+' : '';
                pctDeltaHtml = '<span class="pct-delta">' + pdSign + pctDelta.toFixed(1) + '%</span>';
            }

            var prevVol = 0;
            if (prevDataGroup) {
                prevVol = (prevDataGroup[sel] || {}).volume || 0;
            }
            var volDiff = vol - prevVol;
            var volDiffStr = '-';
            var volDiffHasVal = false;
            if (prevS && volDiff !== 0) {
                volDiffStr = formatLiveVol(volDiff);
                volDiffHasVal = true;
            }

            var hi = pct >= 50;
            var divCls = j > 0 ? ' lpt-sel-divider' : '';

            html += '<td class="period-dual' + divCls + '"><span class="dual-main period-odds">' + oddsStr + '</span><span class="dual-sub">' + deltaHtml + '</span></td>';
            html += '<td class="period-dual"><span class="dual-main period-vol">' + (vol ? formatLiveVol(vol) : '-') + '</span><span class="dual-sub vol-diff' + (volDiffHasVal ? ' has-val' : '') + '">' + volDiffStr + '</span></td>';
            html += '<td class="period-dual"><span class="dual-main period-pct ' + (hi ? 'pct-high' : 'pct-low') + '">' + pctStr + '</span><span class="dual-sub">' + pctDeltaHtml + '</span></td>';
        }
...
async function _checkModalLiveData(home, away, reqId) {
    try {
        var resp = await fetch('/api/live/match/history-by-teams?home=' + encodeURIComponent(home) + '&away=' + encodeURIComponent(away));
        var data = await resp.json();
        if (reqId !== _modalRequestId) return;
        if (data.has_live && data.snapshots && data.snapshots.length > 0 && window.userPlan !== 'core') {
            _modalLiveData = data.snapshots;
            _modalLiveMarket = '1x2';
            var liveTab = document.getElementById('modalLiveTab');
            if (liveTab) liveTab.style.display = '';
            _buildModalLiveTabs(_modalLiveData);
            renderModalLiveDetail(_modalLiveData, _modalLiveMarket);

            if (!modalOddsData) {
                var sorted = data.snapshots.slice().sort(function(a, b) {
                    return (a.snapshot_at || '') > (b.snapshot_at || '') ? 1 : -1;
                });
                ...
                var sX = d1x2['X'] || {};
                var s2 = d1x2['2'] || {};
                var vol1 = s1.volume || 0;
                var volX = sX.volume || 0;
                var vol2 = s2.volume || 0;
                var totalVol = vol1 + volX + vol2;
                modalOddsData = {
                    Odds1: s1.odds, OddsX: sX.odds, Odds2: s2.odds,
                    Amt1: vol1, AmtX: volX, Amt2: vol2,
                    Pct1: s1.share || 0, PctX: sX.share || 0, Pct2: s2.share || 0,
                    Volume: totalVol
                };
                if (prev) {
                    var p1x2 = prev['1x2'] || {};
                    previousOddsData = {
                        Odds1: (p1x2['1'] || {}).odds, OddsX: (p1x2['X'] || {}).odds, Odds2: (p1x2['2'] || {}).odds
                    };
                }
                if (selectedMatch && !selectedMatch.odds) {
                    selectedMatch.odds = modalOddsData;
                }
                updateMatchInfoCard();
            }
        }
    } catch (e) {
        console.error('[Modal] Live data check error:', e);
    }
}

function _buildModalLiveTabs(snapshots) {
    var tabsEl = document.getElementById('modalLiveMarketTabs');
    if (!tabsEl) return;
    var lines = {};
    for (var i = 0; i < snapshots.length; i++) {
        var ols = snapshots[i].ou_lines || {};
    ...
    var sortedLines = Object.keys(lines).sort(function(a, b) { return parseFloat(a) - parseFloat(b); });
    var html = '<button class="live-dtab' + (_modalLiveMarket === '1x2' ? ' active' : '') + '" onclick="setModalLiveMarket(\'1x2\')">1X2</button>';
    for (var j = 0; j < sortedLines.length; j++) {
        var ln = sortedLines[j];
        var mk = 'ou' + ln;
        html += '<button class="live-dtab' + (_modalLiveMarket === mk ? ' active' : '') + '" onclick="setModalLiveMarket(\'' + mk + '\')">' + escLiveHtml(ln) + '</button>';
    }
    tabsEl.innerHTML = html;
}

function setModalLiveMarket(market) {
    _modalLiveMarket = market;
    var tabsEl = document.getElementById('modalLiveMarketTabs');
    if (tabsEl) {
        tabsEl.querySelectorAll('.live-dtab').forEach(function(b) { b.classList.remove('active'); });
        var label = market === '1x2' ? '1X2' : market.replace('ou', '');
        tabsEl.querySelectorAll('.live-dtab').forEach(function(b) {
            if ((market === '1x2' && b.textContent === '1X2') || (market !== '1x2' && b.textContent === label)) {
                b.classList.add('active');
            }
        });
...
function renderModalLiveDetail(snapshots, market) {
    var body = document.getElementById('modalLiveBody');
    if (!body) return;
    if (!snapshots || snapshots.length === 0) {
        body.innerHTML = '<div class="live-loading">Canl\u0131 veri bulunamad\u0131</div>';
        return;
    }
    ...
    if (window.innerWidth <= 768) {
        _renderModalLiveMobile(body, snapshots, market);
        return;
    }
...
function _renderModalLiveMobile(body, snapshots, market) {
    var is1x2 = (market === '1x2');
    var ouLine = is1x2 ? '' : market.replace('ou', '');
    var sels = is1x2 ? ['1', 'X', '2'] : ['U', 'O'];
    var selLabels = is1x2 ? ['1', 'X', '2'] : ['Alt ' + ouLine, '\u00dcst ' + ouLine];

    var html = '<div class="mlive-wrap">';
    ...
    for (var i = 0; i < snapshots.length; i++) {
        ...
        html += '<div class="mlive-period">';
        html += '<div class="mlive-status"><span class="mlive-min">' + escLiveHtml(minStr) + '</span><span class="mlive-score">' + escLiveHtml(scoreStr) + '</span></div>';
        html += '<div class="mlive-cards">';
        ...
        for (var j = 0; j < sels.length; j++) {
            ...
            if (prevDataGroup && odds !== null) {
                ...
                if (prevOdds !== undefined && prevOdds !== null) {
                    ...
                    if (Math.abs(delta) >= 0.005) {
                        var dSign = delta > 0 ? '+' : '';
                        var dCls = delta > 0 ? 'ch-up' : 'ch-down';
                        oddsDelta = '<span class="mc-d ' + dCls + '">' + dSign + delta.toFixed(2) + '</span>';
                    }
                }
            }
            var prevPct = 0;
            if (prevDataGroup) { prevPct = (prevDataGroup[sel] || {}).share || 0; }
            var pctDelta = pct - prevPct;
            var pctDeltaHtml = '';
            if (prevS && Math.abs(pctDelta) >= 0.05) {
                var pdSign = pctDelta > 0 ? '+' : '';
                var pCls = pctDelta > 0 ? 'ch-up' : 'ch-down';
                pctDeltaHtml = '<span class="mc-d ' + pCls + '">' + pdSign + pctDelta.toFixed(1) + '%</span>';
            }
            var prevVol = 0;
            if (prevDataGroup) { prevVol = (prevDataGroup[sel] || {}).volume || 0; }
            var volDiff = vol - prevVol;
            var volDeltaHtml = '';
            if (prevS && volDiff !== 0) {
                var vCls = volDiff > 0 ? 'ch-up' : 'ch-down';
                volDeltaHtml = '<span class="mc-d ' + vCls + '">' + formatLiveVol(volDiff) + '</span>';
            }

            var gd = '<span class="mc-dash">-</span>';
            html += '<div class="mlive-card">';
            html += '<div class="mlive-card-lbl">' + selLabels[j] + '</div>';
            html += '<div class="mc-line mc-odds"><span class="mc-v">' + (odds !== null ? oddsStr : gd) + '</span>' + (oddsDelta || gd) + '</div>';
            html += '<div class="mc-line mc-vol"><span class="mc-v">' + (vol ? formatLiveVol(vol) : gd) + '</span>' + (volDeltaHtml || gd) + '</div>';
            html += '<div class="mc-line mc-pct"><span class="mc-v">' + (pct ? pctStr : gd) + '</span>' + (pctDeltaHtml || gd) + '</div>';
            html += '</div>';
        }
