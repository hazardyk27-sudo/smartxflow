...
resolve();});});};let currentMarket='moneyway_1x2';let matches=[];let filteredMatches=[];let chart=null;let selectedMatch=null;let selectedChartMarket='moneyway_1x2';let autoScrapeRunning=false;let currentSortColumn='volume';let currentSortDirection='desc';let chartVisibleSeries={};let dateFilterMode='ALL';let chartTimeRange='10min';let currentChartHistoryData=[];let chartViewMode='percent';let isClientMode=true;let _modalRequestId=0;let _analysisMatchHashes=[];let _mobileHideEnded=false;let _mobileHideLive=false;let _mobileOnlyLive=false;let _analysisHashesFetched=false;let currentSource='betfair';let chartOddsSource='betfair';let _licenseReadyResolve;const _licenseReady=new Promise(r=>{_licenseReadyResolve=r;});let _isLicensed=false;function getWebDeviceId(){let did=localStorage.getItem('smartxflow_device_id');if(!did||did.length>16){did='w-'+crypto.randomUUID().replace(/-/g,'').substring(0,14);localStorage.setItem('smartxflow_device_id',did);}
return did;}
...
if(selectedMatch){showChartLoading();if(selectedChartMarket.startsWith('dropping')&&chartOddsSource==='matchbook'){loadChartForSource(selectedMatch.home_team,selectedMatch.away_team,selectedChartMarket,selectedMatch.league||'');}else{if(selectedChartMarket.startsWith('dropping'))chartOddsSource='betfair';loadChartWithTrends(selectedMatch.home_team,selectedMatch.away_team,selectedChartMarket,selectedMatch.league||'');}
var arbH=selectedMatch.arbworld_hash||'';loadMatchbookChartForModal(selectedMatch.home_team,selectedMatch.away_team,selectedMatch.league||'',arbH,selectedChartMarket);}});});}
...
let apiUrl;const srcParam=requestSource!=='betfair'?`&source=${requestSource}`:'';if(dateFilterMode==='ALL'){apiUrl=`/api/matches?market=${requestMarket}&date_filter=today_future&bulk=1${srcParam}`;}else if(dateFilterMode==='YESTERDAY'){apiUrl=`/api/matches?market=${requestMarket}&date_filter=yesterday&bulk=1${srcParam}`;}else if(dateFilterMode==='TODAY'){apiUrl=`/api/matches?market=${requestMarket}&date_filter=today&bulk=1${srcParam}`;}else{apiUrl=`/api/matches?market=${requestMarket}&date_filter=today_future&bulk=1${srcParam}`;}
...
function mfSelect(el){var group=el.getAttribute('data-group');el.closest('.mf-chips').querySelectorAll('.mf-chip[data-group="'+group+'"]').forEach(function(c){c.classList.remove('active');});el.classList.add('active');}
...
selectedMatch={home_team:apiHome,away_team:apiAway,league:apiLeague,date:apiKickoff,match_id:prematchFound?(prematchFound.match_id||''):'',history:[],history_count:0,odds:null,details:null};selectedChartMarket=currentMarket;var modalTitleHtml=escapeHtml(homeTeam)+' vs '+escapeHtml(awayTeam);document.getElementById('modalMatchTitle').innerHTML=modalTitleHtml;var modalFtCapsule=_getModalFinishedCapsule(selectedMatch);var isFinishedMatch=modalFtCapsule!=='';const headerDt=apiKickoff?toTurkeyTime(apiKickoff):null;let headerDateText='';if(!isFinishedMatch&&headerDt&&headerDt.isValid()){headerDateText=headerDt.format('DD.MM HH:mm');}
var leagueHtml=escapeHtml(apiLeague||'');if(isFinishedMatch)leagueHtml+=modalFtCapsule;else if(headerDateText)leagueHtml+=' • '+headerDateText;document.getElementById('modalLeague').innerHTML=leagueHtml||'';document.querySelectorAll('#modalChartTabs .chart-tab').forEach(t=>{t.classList.remove('active');if(t.dataset.market===currentMarket){t.classList.add('active');}});document.getElementById('modalOverlay').classList.add('active');const matchIdHash2=selectedMatch.match_id||'';const chartLibsPromise=window.loadChartLibs?window.loadChartLibs().then(()=>registerChartPlugins()):Promise.resolve();const marketsPromise=loadAllMarketsAtOnce(apiHome,apiAway,apiLeague,matchIdHash2);try{const chartPipeline=Promise.all([marketsPromise,chartLibsPromise]).then(()=>{if(reqId!==_modalRequestId)return;return loadChartWithTrends(apiHome,apiAway,selectedChartMarket,apiLeague,reqId);}).catch(e=>console.error('[Modal] Chart pipeline error:',e));const alarmsPipeline=renderMatchAlarmsSection(apiHome,apiAway,reqId);const livePipeline=_checkModalLiveData(homeTeam,awayTeam,reqId);var arbHash2=selectedMatch.arbworld_hash||'';const mbPipeline2=loadMatchbookChartForModal(apiHome,apiAway,apiLeague,arbHash2,selectedChartMarket,reqId);await Promise.all([chartPipeline,alarmsPipeline,livePipeline,mbPipeline2]);}catch(error){console.error('[openMatchModalFromAPI] Error:',error);showToast('Maç bilgisi alınırken hata oluştu','error');}}
async function openMatchModal(index){const dataSource=filteredMatches.length>0?filteredMatches:matches;if(index>=0&&index<dataSource.length){const reqId=++_modalRequestId;resetModalState();selectedMatch=dataSource[index];selectedChartMarket=currentMarket;var modalTitleHtml=escapeHtml(selectedMatch.home_team)+' vs '+escapeHtml(selectedMatch.away_team);document.getElementById('modalMatchTitle').innerHTML=modalTitleHtml;var modalFtCapsule=_getModalFinishedCapsule(selectedMatch);var isFinishedMatch=modalFtCapsule!=='';const headerDt=toTurkeyTime(selectedMatch.date);let headerDateText='';if(!isFinishedMatch&&headerDt&&headerDt.isValid()){headerDateText=headerDt.format('DD.MM HH:mm');}
var leagueHtml=escapeHtml(selectedMatch.league||'');if(isFinishedMatch)leagueHtml+=modalFtCapsule;else if(headerDateText)leagueHtml+=' • '+headerDateText;document.getElementById('modalLeague').innerHTML=leagueHtml;document.querySelectorAll('#modalChartTabs .chart-tab').forEach(t=>{t.classList.remove('active');if(t.dataset.market===currentMarket){t.classList.add('active');}});document.getElementById('modalOverlay').classList.add('active');const home=selectedMatch.home_team;const away=selectedMatch.away_team;const league=selectedMatch.league||'';const matchIdHash3=selectedMatch.match_id||'';const chartLibsPromise=window.loadChartLibs?window.loadChartLibs().then(()=>registerChartPlugins()):Promise.resolve();const marketsPromise=loadAllMarketsAtOnce(home,away,league,matchIdHash3);const chartPipeline=Promise.all([marketsPromise,chartLibsPromise]).then(()=>{if(reqId!==_modalRequestId)return;return loadChartWithTrends(home,away,selectedChartMarket,league,reqId);}).catch(e=>console.error('[Modal] Chart pipeline error:',e));const alarmsPipeline=renderMatchAlarmsSection(home,away,reqId);const livePipeline=_checkModalLiveData(home,away,reqId);var arbHash3=selectedMatch.arbworld_hash||'';const mbPipeline3=loadMatchbookChartForModal(home,away,league,arbHash3,selectedChartMarket,reqId);await Promise.all([chartPipeline,alarmsPipeline,livePipeline,mbPipeline3]);}}
function resetModalState(){if(chart){chart.destroy();chart=null;}
currentChartHistoryData=[];modalOddsData=null;previousOddsData=null;bulkHistoryCache={};bulkHistoryCacheKey='';chartOddsSource='betfair';_modalLiveData=null;_modalLiveMarket='1x2';const card=document.getElementById('matchInfoCard');if(card)card.innerHTML='';const grid=document.getElementById('smartMoneyGrid');if(grid){grid.innerHTML='';grid.style.display='none';}
const empty=document.getElementById('smartMoneyEmpty');if(empty)empty.style.display='none';var liveTab=document.getElementById('modalLiveTab');if(liveTab)liveTab.style.display='none';var liveBody=document.getElementById('modalLiveBody');if(liveBody)liveBody.innerHTML='';var liveTabs=document.getElementById('modalLiveMarketTabs');if(liveTabs)liveTabs.innerHTML='';const canvas=document.getElementById('oddsChart');if(canvas){const ctx=canvas.getContext('2d');ctx.clearRect(0,0,canvas.width,canvas.height);}
var mbSection=document.getElementById('matchbookChartSection');if(mbSection)mbSection.style.display='none';if(window._mbChart){window._mbChart.destroy();window._mbChart=null;}
showSmartMoneyLoading();showChartLoading();}
let _mbBulkCache={};let _mbBulkCacheKey='';async function loadMatchbookChartForModal(home,away,league,arbHash,market,reqId){var section=document.getElementById('matchbookChartSection');if(!section)return;if(currentSource==='matchbook'){section.style.display='none';return;}
if(!arbHash&&!home){section.style.display='none';return;}
try{var cacheKey=`${home}|${away}|${league}|mb`;if(_mbBulkCacheKey!==cacheKey){var params=`home=${encodeURIComponent(home)}&away=${encodeURIComponent(away)}&league=${encodeURIComponent(league || '')}`;if(arbHash)params+=`&arb_hash=${encodeURIComponent(arbHash)}`;var resp=await fetch('/api/match/history/matchbook?'+params);var data=await resp.json();if(reqId!==_modalRequestId)return;_mbBulkCache=data.markets||{};_mbBulkCacheKey=cacheKey;}
var mktData=_mbBulkCache[market];if(!mktData||!mktData.history||mktData.history.length===0){section.style.display='none';return;}
section.style.display='block';var canvas=document.getElementById('matchbookChart');if(!canvas)return;if(window._mbChart){window._mbChart.destroy();window._mbChart=null;}
var chartData=mktData.chart_data;if(!chartData||!chartData.datasets||chartData.datasets.length===0){section.style.display='none';return;}
await window.loadChartLibs();var ctx=canvas.getContext('2d');window._mbChart=new Chart(ctx,{type:'line',data:{labels:chartData.labels,datasets:chartData.datasets.map(function(ds){return{label:ds.label,data:ds.data,borderColor:ds.borderColor,backgroundColor:'transparent',borderWidth:2,tension:0.1,fill:false,pointRadius:0,pointHoverRadius:4};})},options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},plugins:{legend:{display:true,position:'top',labels:{color:'#7d848c',font:{size:11},boxWidth:12,padding:8}},title:{display:false}},scales:{x:{display:true,ticks:{color:'#5c636b',font:{size:10},maxTicksLimit:8},grid:{color:'rgba(255,255,255,0.04)'}},y:{display:true,ticks:{color:'#5c636b',font:{size:10}},grid:{color:'rgba(255,255,255,0.04)'}}}}});}catch(e){console.error('[MB Chart] Error:',e);if(section)section.style.display='none';}}
...
try{console.log('[Bulk] Fetching all markets for',home,'vs',away,'league:',league,'source:',currentSource);const startTime=performance.now();var url;if(currentSource==='matchbook'){var params=`home=${encodeURIComponent(home)}&away=${encodeURIComponent(away)}&league=${encodeURIComponent(league || '')}`;if(matchIdHash)params+=`&match_id_hash=${encodeURIComponent(matchIdHash)}`;url='/api/match/history/matchbook?'+params;}else{url=`/api/match/history/bulk?home=${encodeURIComponent(home)}&away=${encodeURIComponent(away)}&league=${encodeURIComponent(league || '')}`;}
const response=await fetch(url);const data=await response.json();const elapsed=performance.now()-startTime;console.log(`[Bulk] Loaded all 6 markets in ${elapsed.toFixed(0)}ms`);if(data.markets){bulkHistoryCache=data.markets;bulkHistoryCacheKey=cacheKey;}
...
function setMobileTimeRange(range){mobileTimeRange=range;const rangeMap={'10':'10min','30':'30min','60':'1hour','360':'6hour','720':'12hour','1440':'1day','all':'1day'};chartTimeRange=rangeMap[range]||'1day';const labels={'all':'Tümü','10':'10 dk','30':'30 dk','60':'1 saat','360':'6 saat','720':'12 saat','1440':'1 gün'};const mobTimeLabel=document.getElementById('mobTimeLabel');if(mobTimeLabel)mobTimeLabel.textContent=labels[range]||'Tümü';document.querySelectorAll('.mob-time-option').forEach(opt=>{opt.classList.toggle('active',opt.dataset.range===range);});const mobTimeMenu=document.getElementById('mobTimeMenu');if(mobTimeMenu)mobTimeMenu.classList.remove('open');document.querySelectorAll('.time-pill').forEach(pill=>{pill.classList.toggle('active',pill.dataset.range===range);});if(selectedMatch&&isMobile()){loadChartHistory(selectedMatch.MatchId,selectedChartMarket);}}
...
function renderChartLegendFilters(datasets,market){const container=document.getElementById('chartLegendFilters');if(!container)return;const legendLabels={'moneyway_1x2':[{key:'1',label:'1',color:'#3b82f6'},{key:'X',label:'X',color:'#22c55e'},{key:'2',label:'2',color:'#eab308'}],'moneyway_ou25':[{key:'Under',label:'Alt 2.5',color:'#3b82f6'},{key:'Over',label:'Üst 2.5',color:'#22c55e'}],'moneyway_btts':[{key:'Yes',label:'KG Evet',color:'#22c55e'},{key:'No',label:'KG Hayır',color:'#ef4444'}],'dropping_1x2':[{key:'1',label:'1',color:'#3b82f6'},{key:'X',label:'X',color:'#22c55e'},{key:'2',label:'2',color:'#eab308'}],'dropping_ou25':[{key:'Under',label:'Alt 2.5',color:'#3b82f6'},{key:'Over',label:'Üst 2.5',color:'#22c55e'}],'dropping_btts':[{key:'Yes',label:'KG Evet',color:'#22c55e'},{key:'No',label:'KG Hayır',color:'#ef4444'}]};const items=legendLabels[market]||[];const timeRanges=[{key:'10min',label:'10 dk'},{key:'30min',label:'30 dk'},{key:'1hour',label:'1 saat'},{key:'6hour',label:'6 saat'},{key:'12hour',label:'12 saat'},{key:'1day',label:'1 gün'}];const seriesButtons=items.map(item=>{const stateKey=`${market}_${item.key}`;const isActive=chartVisibleSeries[stateKey]!==false;const activeClass=isActive?'active':'inactive';return`
    ...
    `:'';container.innerHTML=`
        ...
        ${market.startsWith('dropping') ? `<div class="chart-filter-divider"></div><div class="chart-filter-group source-selector"><div class="chart-source-dropdown"><button class="chart-source-btn"onclick="toggleChartSourceDropdown(event)"><span class="chart-source-label">${chartOddsSource==='betfair'?'Betfair':'Matchbook'}</span><svg width="10"height="10"viewBox="0 0 24 24"fill="none"stroke="currentColor"stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg></button><div class="chart-source-menu"id="chartSourceMenu"><button class="${chartOddsSource === 'betfair' ? 'active' : ''}"onclick="setChartOddsSource('betfair')">Betfair</button><button class="${chartOddsSource === 'matchbook' ? 'active' : ''}"onclick="setChartOddsSource('matchbook')">Matchbook</button></div></div></div>` : ''}
        <div class="chart-filter-divider"></div>
    ...
    `;}
function setChartTimeRange(range){chartTimeRange=range;document.querySelectorAll('.chart-time-btn').forEach(btn=>{btn.classList.remove('active');if(btn.dataset.range===range){btn.classList.add('active');}});if(selectedMatch){if(selectedChartMarket.startsWith('dropping')&&chartOddsSource==='matchbook'){loadChartForSource(selectedMatch.home_team,selectedMatch.away_team,selectedChartMarket,selectedMatch.league||'');}else{loadChart(selectedMatch.home_team,selectedMatch.away_team,selectedChartMarket,selectedMatch.league||'');}}}
function toggleChartSourceDropdown(e){e.stopPropagation();var menu=document.getElementById('chartSourceMenu');if(!menu)return;var isOpen=menu.classList.contains('open');menu.classList.toggle('open');if(!isOpen){var closeHandler=function(ev){if(!menu.contains(ev.target)){menu.classList.remove('open');document.removeEventListener('click',closeHandler);}};setTimeout(function(){document.addEventListener('click',closeHandler);},0);}}
function setChartOddsSource(src){if(chartOddsSource===src)return;chartOddsSource=src;var menu=document.getElementById('chartSourceMenu');if(menu)menu.classList.remove('open');var label=document.querySelector('.chart-source-label');if(label)label.textContent=src==='betfair'?'Betfair':'Matchbook';document.querySelectorAll('.chart-source-menu button').forEach(function(b){b.classList.toggle('active',b.textContent.trim().toLowerCase()===src);});bulkHistoryCache={};bulkHistoryCacheKey='';_mbBulkCache={};_mbBulkCacheKey='';if(selectedMatch){loadChartForSource(selectedMatch.home_team,selectedMatch.away_team,selectedChartMarket,selectedMatch.league||'');}}
async function loadChartForSource(home,away,market,league){if(chartOddsSource==='matchbook'){var cacheKey=`${home}|${away}|${league}|mb_chart`;if(_mbBulkCacheKey!==cacheKey){var params=`home=${encodeURIComponent(home)}&away=${encodeURIComponent(away)}&league=${encodeURIComponent(league || '')}`;if(selectedMatch&&selectedMatch.match_id){params+=`&arb_hash=${encodeURIComponent(selectedMatch.match_id)}`;}
try{var resp=await fetch('/api/match/history/matchbook?'+params);var data=await resp.json();_mbBulkCache=data.markets||{};_mbBulkCacheKey=cacheKey;}catch(e){console.error('[ChartSource] Matchbook fetch error:',e);return;}}
var mktKey=market.replace('dropping_','oran_');var mktData=_mbBulkCache[mktKey]||_mbBulkCache[market];if(mktData&&mktData.history&&mktData.history.length>0){bulkHistoryCache[market]=mktData;bulkHistoryCacheKey=`${home}|${away}|${league}`;}else{bulkHistoryCache[market]={history:[]};bulkHistoryCacheKey=`${home}|${away}|${league}`;}
loadChart(home,away,market,league);}else{bulkHistoryCache={};bulkHistoryCacheKey='';var reqId=++_modalRequestId;var marketsPromise=loadAllMarketsAtOnce(home,away,league,selectedMatch?selectedMatch.match_id_hash:'');await marketsPromise;loadChart(home,away,market,league);}}
...
band.classList.add('highlight-mode');const info=getAlertType(alarm);const home=alarm.home||alarm.home_team||'?';const away=alarm.away||alarm.away_team||'?';const rawSel=(alarm.selection||alarm.side||'').toUpperCase();const selMap={'U':'Alt','O':'Üst','Y':'Evet','N':'Hayır','1':'1','X':'X','2':'2','UNDER':'Alt','OVER':'Üst','YES':'Evet','NO':'Hayır'};const selection=selMap[rawSel]||rawSel;const alarmType=alarm._type||'sharp';let value='';if(alarmType==='sharp'){value=(alarm.sharp_score||0).toFixed(1);}else if(alarmType==='volumeshock'){value=`${(alarm.volume_shock_value || alarm.volume_shock || alarm.volume_shock_multiplier || 0).toFixed(1)}x`;}else if(alarmType==='bigmoney'){value=`£${Number(alarm.incoming_money || alarm.stake || 0).toLocaleString('en-GB')}`;}else if(alarmType==='dropping'){value=`▼ ${(alarm.drop_pct || 0).toFixed(1)}%`;}else if(alarmType==='publicmove'){value=`${(alarm.move_score || alarm.trap_score || alarm.sharp_score || 0).toFixed(0)}`;}else if(alarmType==='volumeleader'){value=`%${(alarm.new_leader_share || 0).toFixed(0)}`;}
...
if(type==='volumeshock')return{label:'HACIM SOKU',color:'gold',pillClass:'volumeshock'};if(type==='dropping'){const level=alarm.level||'L1';if(level==='L3')return{label:'DÜŞÜŞ L3',color:'red',pillClass:'dropping-l3'};if(level==='L2')return{label:'DÜŞÜŞ L2',color:'red',pillClass:'dropping-l2'};return{label:'DÜŞÜŞ L1',color:'red',pillClass:'dropping-l1'};}
...
function _updateMobMktRow(cat){var row=document.getElementById('mobMktRow');if(!row)return;var opts=_mobMarkets[cat]||_mobMarkets.moneyway;var html='';opts.forEach(function(o,i){html+='<button class="mob-tab'+(i===0?' active':'')+'" data-mkt="'+o.v+'" onclick="selectMobMkt(\''+o.v+'\')">'+o.l+'</button>';});row.innerHTML=html;_mobCurMkt=opts[0].v;if(cat==='live'){setLiveMarket(opts[0].v);}else{setMobileMarket(opts[0].v);}}
function selectMobMkt(mkt){_mobCurMkt=mkt;document.querySelectorAll('#mobMktRow .mob-tab').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-mkt')===mkt);});if(_mobCurCat==='live'){setLiveMarket(mkt);}else{setMobileMarket(mkt);}}
...
function toggleAlarmsSidebar(){if(alarmsSidebarOpen){closeAlarmsSidebar();}else{openAlarmsSidebar();}}
function openAlarmsSidebar(){alarmsSidebarOpen=true;document.getElementById('alarmsSidebar').classList.add('open');document.getElementById('alarmsSidebarOverlay').classList.add('open');document.body.style.overflow='hidden';const btn=document.getElementById('alarmsBtn');if(btn)btn.classList.add('active');if(_alarmBatchCache&&(Date.now()-_alarmCacheTime)<ALARM_CACHE_TTL){loadAllAlarms(false);}else{loadAllAlarms(true);}}
...
const homeEscaped=home.replace(/'/g,"\\'");const awayEscaped=away.replace(/'/g,"\\'");const marketEscaped=market.replace(/'/g,"\\'");const leagueEscaped=(group.league||'').replace(/'/g,"\\'");const kickoffEscaped=(group.kickoff_utc||group.match_date||'').replace(/'/g,"\\'");const matchTimeFormatted=formatMatchTime3(group.kickoff_utc||group.match_date);const marketLabel2=`${translateMarket(market)} → ${translateSelection(selection, market)}`;const triggerTimeSource=type==='dropping'?(alarm.created_at||alarm.event_time||alarm.trigger_at):(alarm.trigger_at||alarm.event_time||alarm.created_at);const triggerTime=formatTriggerTime(triggerTimeSource);let badgeLabel='';let metricContent='';let historyLine='';if(type==='sharp'){badgeLabel='SHARP';const score=(alarm.sharp_score||0).toFixed(1);const volumeContrib=(alarm.volume_contrib||0).toFixed(1);const oddsContrib=(alarm.odds_contrib||0).toFixed(1);const prevOdds=(alarm.previous_odds||0).toFixed(2);const currOdds=(alarm.current_odds||0).toFixed(2);metricContent=`<div class="acd-grid cols-3">
            ...
            </div>`;historyLine=`${triggerTime}`;}else if(type==='volumeshock'){badgeLabel='HACİM ŞOKU';const newMoney=alarm.incoming_money||0;const shockVal=(alarm.volume_shock_value||alarm.volume_shock||alarm.volume_shock_multiplier||0).toFixed(1);const hoursToKickoff=calculateHoursToKickoff(alarm);metricContent=`<div class="acd-grid cols-2">
                <div class="acd-stat">
...
const totalHtml=selectionTotal>0?`<div class="acd-stat acd-stat-secondary">
            ...
            </div>`;historyLine=`${triggerTime}`;}else if(type==='volumeleader'){badgeLabel='LİDER DEĞİŞTİ';const oldLeader=alarm.old_leader||'-';const newLeader=alarm.new_leader||'-';const oldShare=(alarm.old_leader_share||0).toFixed(0);const newShare=(alarm.new_leader_share||0).toFixed(0);const totalVol=alarm.total_volume||0;metricContent=`<div class="acd-grid cols-2">
                <div class="acd-stat">
...
const fullMatchName=`${home} – ${away}`;let metricValue='';if(type==='sharp'){metricValue=(alarm.sharp_score||0).toFixed(1);}else if(type==='volumeshock'){metricValue=`${(alarm.volume_shock_value || alarm.volume_shock || alarm.volume_shock_multiplier || 0).toFixed(1)}x`;}else if(type==='bigmoney'){metricValue=`£${Number(alarm.incoming_money || alarm.stake || 0).toLocaleString('en-GB')}`;}else if(type==='dropping'){metricValue=`▼ ${(alarm.drop_pct || 0).toFixed(1)}%`;}else if(type==='publicmove'){metricValue=(alarm.move_score||alarm.trap_score||alarm.sharp_score||0).toFixed(0);}else if(type==='volumeleader'){const oldL=alarm.old_leader||'-';const newL=alarm.new_leader||'-';metricValue=`<span class="vl-transition"><span class="vl-old">${oldL}</span><span class="vl-arrow">›</span><span class="vl-new">${newL}</span></span>`;}else if(type==='mim'){const impact=alarm.impact||alarm.impact_score||alarm.money_impact||0;const impactPct=(impact*100).toFixed(0);const incomingMoney=alarm.incoming_volume||((alarm.current_volume||alarm.curr_volume||0)-(alarm.prev_volume||0));metricValue=incomingMoney>0?`+£${Number(incomingMoney).toLocaleString('en-GB')}`:`${impactPct}%`;}
const historyCount=group.history.length;const historyBadge=historyCount>0?`<span class="history-badge history-badge-${type}">×${historyCount + 1}</span>`:'';let historySection='';if(historyCount>0&&isOpen){const historyItems=group.history.map(h=>{const hTime=formatTriggerTimeWithDay(h.trigger_at||h.event_time||h.created_at);let hValue='';if(type==='sharp'){hValue=`Sharp ${(h.sharp_score || 0).toFixed(1)}`;}else if(type==='volumeshock'){hValue=`${(h.volume_shock_value || h.volume_shock || h.volume_shock_multiplier || 0).toFixed(1)}x`;}else if(type==='bigmoney'){hValue=`£${Number(h.incoming_money || h.stake || 0).toLocaleString('en-GB')}`;}else if(type==='dropping'){hValue=`▼ ${(h.drop_pct || 0).toFixed(1)}%`;}else if(type==='publicmove'){hValue=`Trap ${(h.trap_score || h.sharp_score || 0).toFixed(0)}`;}else if(type==='volumeleader'){hValue=`<span class="vl-mini">${h.old_leader || '-'} › ${h.new_leader || '-'}</span>`;}else if(type==='mim'){const hImpact=h.impact||h.impact_score||h.money_impact||0;const hIncoming=(h.current_volume||h.curr_volume||0)-(h.prev_volume||0);hValue=hIncoming>0?`+£${Number(hIncoming).toLocaleString('en-GB')}`:`${(hImpact * 100).toFixed(0)}%`;}
...
function goToMatchFromAlarm(homeTeam,awayTeam,alarmType,alarmMarket,league,kickoff){openAlarmId=null;closeAlarmsSidebar();openMatchModalFromAPI(homeTeam.trim(),awayTeam.trim(),(league||'').trim(),(kickoff||'').trim());}
...
function logoutWebLicense(){if(confirm('Cikis yapmak istediginize emin misiniz?')){fetch('/api/licenses/logout',{method:'POST'}).catch(()=>{});localStorage.removeItem(WEB_LICENSE_KEY);localStorage.removeItem(WEB_LICENSE_VALID_KEY);localStorage.removeItem('license_days_remaining');window.location.reload();}}
...
function renderLiveMatches(matches,goalHashes){if(!goalHashes)goalHashes={};var table=document.querySelector('.matches-table');var tbody=document.getElementById('matchesTableBody');var thead=table?table.querySelector('thead'):null;var colgroup=table?table.querySelector('colgroup'):null;var cardList=document.getElementById('matchCardList');var emptyHtml='<div class="live-empty">'+'<div class="live-empty-icon">\u26BD</div>'+'<div class="live-empty-text">Su an canli mac bulunamadi</div>'+'<div class="live-empty-sub">Canli maclar basladiginda burada gorunecek</div>'+'</div>';if(!matches||matches.length===0){if(tbody)tbody.innerHTML='<tr><td colspan="7" style="text-align:center;padding:40px;">'+emptyHtml+'</td></tr>';if(thead)thead.innerHTML='<tr><th colspan="7" style="text-align:center;">Canli mac bulunamadi</th></tr>';if(cardList)cardList.innerHTML=emptyHtml;var mc=document.getElementById('matchCount');if(mc)mc.textContent='0';return;}
var is1x2=(_liveMarket==='1x2');var ouLine=is1x2?'':_liveMarket.replace('ou','');if(is1x2){if(table)table.setAttribute('data-selection-count','3');if(colgroup)colgroup.innerHTML='<col class="col-date"><col class="col-league"><col class="col-match">'+'<col class="col-selection"><col class="col-selection"><col class="col-selection"><col class="col-volume">';if(thead)thead.innerHTML='<tr>'+'<th class="col-date">DK</th>'+'<th class="col-league">LİG</th>'+'<th class="col-match">MAÇ</th>'+'<th class="col-selection">1</th>'+'<th class="col-selection">X</th>'+'<th class="col-selection">2</th>'+'<th class="col-volume">HACİM</th>'+'</tr>';}else{if(table)table.setAttribute('data-selection-count','2');if(colgroup)colgroup.innerHTML='<col class="col-date"><col class="col-league"><col class="col-match">'+'<col class="col-selection"><col class="col-selection"><col class="col-volume">';if(thead)thead.innerHTML='<tr>'+'<th class="col-date">DK</th>'+'<th class="col-league">LİG</th>'+'<th class="col-match">MAÇ</th>'+'<th class="col-selection">ALT</th>'+'<th class="col-selection">ÜST</th>'+'<th class="col-volume">HACİM</th>'+'</tr>';}
