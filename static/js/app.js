...
var rowsHtml='';var cardsHtml='';for(var i=0;i<matches.length;i++){var m=matches[i];var o=m.odds||{};var onclickAttr='onclick="openLiveDetail(\''+m.match_id_hash+'\',\''+escLiveHtml(m.home_team)+'\',\''+escLiveHtml(m.away_team)+'\')"';if(is1x2){var vol=_liveCalcVol1x2(m);var b1=_liveMwBlock('1',o['1']);var bX=_liveMwBlock('X',o['X']);var b2=_liveMwBlock('2',o['2']);rowsHtml+='<tr '+onclickAttr+'>'+'<td class="live-min-cell"><span class="live-min-badge">'+escLiveHtml(m.minute)+'</span>'+
(m.score?'<span class="live-score-badge">'+escLiveHtml(m.score)+'</span>':'')+'</td>'+'<td class="match-league" title="'+escLiveHtml(m.league)+'">'+escLiveHtml(m.league)+'</td>'+'<td class="match-teams">'+escLiveHtml(m.home_team)+'<span class="vs">-</span>'+escLiveHtml(m.away_team)+'</td>'+'<td class="mw-outcomes-cell" colspan="3"><div class="mw-grid mw-grid-3">'+b1+bX+b2+'</div></td>'+'<td class="volume-cell">'+formatLiveVol(vol)+'</td></tr>';cardsHtml+=_liveMobileCard(m,'1x2',onclickAttr);}else{var ou=_liveGetOu(m,ouLine);var volOU=_liveCalcVolOU(ou);var bU=_liveMwBlock('Alt',ou['U']);var bO=_liveMwBlock('Ust',ou['O']);rowsHtml+='<tr '+onclickAttr+'>'+'<td class="live-min-cell"><span class="live-min-badge">'+escLiveHtml(m.minute)+'</span>'+
...
var is1x2=(market==='1x2');var ouLine=is1x2?'':market.replace('ou','');var html='<div class="live-detail-table-wrap"><table class="live-period-table"><thead><tr>';html+='<th>#</th><th>Zaman</th>';if(is1x2){html+='<th>1</th><th>X</th><th>2</th>';}else{html+='<th>Alt '+escLiveHtml(ouLine)+'</th><th>Ust '+escLiveHtml(ouLine)+'</th>';}
html+='<th>Hacim</th><th>Degisim</th>';html+='</tr></thead><tbody>';var prevVol=0;for(var i=0;i<snapshots.length;i++){var s=snapshots[i];var ts=s.snapshot_at||'';var timeStr='';if(ts){try{var d=new Date(ts);timeStr=String(d.getHours()).padStart(2,'0')+':'+String(d.getMinutes()).padStart(2,'0');}catch(e){}}
html+='<tr>';html+='<td class="period-num">'+(i+1)+'</td>';html+='<td class="period-time">'+timeStr+'</td>';var vol=0;if(is1x2){var o=s['1x2']||{};['1','X','2'].forEach(function(sel){var sd=o[sel]||{};var pct=sd.share||0;var odds=sd.odds?sd.odds.toFixed(2):'-';var hi=pct>=50;html+='<td class="period-odds-cell">'+'<span class="period-odds">'+odds+'</span>'+'<span class="period-pct '+(hi?'pct-high':'pct-low')+'">'+(pct?pct.toFixed(0)+'%':'-')+'</span>'+'</td>';vol+=sd.volume||0;});}else{var ols=s.ou_lines||{};var ouD=ols[ouLine]||{};['U','O'].forEach(function(sel){var sd=ouD[sel]||{};var pct=sd.share||0;var odds=sd.odds?sd.odds.toFixed(2):'-';var hi=pct>=50;html+='<td class="period-odds-cell">'+'<span class="period-odds">'+odds+'</span>'+'<span class="period-pct '+(hi?'pct-high':'pct-low')+'">'+(pct?pct.toFixed(0)+'%':'-')+'</span>'+'</td>';vol+=sd.volume||0;});}
