...
let contentHtml='';const matchDisplay=`${home} - ${away}`;if(alarmType==='volumeleader'){const oldLeader=alarm.old_leader||alarm.previous_leader||'?';const newLeader=alarm.new_leader||alarm.selection||'?';contentHtml=`
...
const matchKey=`${home}_vs_${away}`.replace(/\s+/g,'_');const alarmType=alarm._type||'';const alarmMarket=(alarm.market||'').replace(/'/g,"\\'");const alarmLeague=(alarm.league||'').replace(/'/g,"\\'");const alarmKickoff=(alarm.kickoff_utc||alarm.match_date||alarm.fixture_date||'').replace(/'/g,"\\'");const matchDisplay=`${home} - ${away}`;if(alarm._type==='volumeleader'){const oldLeader=alarm.old_leader||alarm.previous_leader||'?';const newLeader=alarm.new_leader||alarm.selection||'?';return`
