...
var sorted=filtered.slice();sorted.sort(function(a,b){var va=is1x2?_liveCalcVol1x2(a):_liveCalcVolOU(_liveGetOu(a,ouLine));var vb=is1x2?_liveCalcVol1x2(b):_liveCalcVolOU(_liveGetOu(b,ouLine));return vb-va;});var rowsHtml='';var cardsHtml='';for(var i=0;i<sorted.length;i++){var m=sorted[i];var o=m.odds||{};var matchMin=_calcLiveMatchMin(m);var scoreStr=m.score?escLiveHtml(m.score):'';var onclickAttr='onclick="openLiveDetail(\''+m.match_id_hash+'\',\''+escLiveHtml(m.home_team)+'\',\''+escLiveHtml(m.away_team)+'\')"';var showScore=(matchMin==='MS')?(scoreStr||'-'):scoreStr;var goalSide=m.match_id_hash?goalHashes[m.match_id_hash]:null;var scoreHtml='';if(showScore){var sp=showScore.split('-');if(sp.length===2){var hCls=goalSide==='home'||goalSide==='both'?' goal-pop':'';var aCls=goalSide==='away'||goalSide==='both'?' goal-pop':'';scoreHtml='<div class="time-line live-score-badge">'+'<span class="score-h'+hCls+'">'+sp[0].trim()+'</span>'+'<span class="score-sep">-</span>'+'<span class="score-a'+aCls+'">'+sp[1].trim()+'</span>'+'</div>';}else{scoreHtml='<div class="time-line live-score-badge">'+showScore+'</div>';}}
var minExtra='';if(/^HT$/i.test(matchMin)||/^MS$/i.test(matchMin))minExtra=' is-ht';else if(/\d+\+/.test(matchMin))minExtra=' is-extra';var minDisplay=goalSide?'GOL!':escLiveHtml(matchMin);var minCell='<td class="match-date">'+'<div class="dk-cell-inner">'+'<div class="date-line live-min-badge'+minExtra+'"'+(goalSide?' data-real-min="'+escLiveHtml(matchMin)+'"':'')+'>'+minDisplay+'</div>'+
...
html+='<div class="odds-card-volume">'+formatLiveVol(vol)+'</div>';html+='</div>';html+='<div class="odds-card-meta"><span>'+escLiveHtml(m.league)+'</span><span class="meta-sep">\u2022</span>';var cardMin=_calcLiveMatchMin(m);var cardMinDisplay=goalSide?'GOL!':cardMin;html+='<span class="live-min-badge" style="font-size:11px;"'+(goalSide?' data-real-min="'+cardMin+'"':'')+'>'+cardMinDisplay+'</span>';var cardScore=m.score?escLiveHtml(m.score):(cardMin==='MS'?'-':'');if(cardScore){var csp=cardScore.split('-');if(csp.length===2&&goalSide){var chCls=goalSide==='home'||goalSide==='both'?' goal-pop':'';var caCls=goalSide==='away'||goalSide==='both'?' goal-pop':'';html+='<span class="meta-sep">\u2022</span><span class="score-h'+chCls+'" style="font-weight:600;">'+csp[0].trim()+'</span><span class="score-sep" style="font-weight:600;">-</span><span class="score-a'+caCls+'" style="font-weight:600;">'+csp[1].trim()+'</span>';}else{html+='<span class="meta-sep">\u2022</span><span style="color:#f59e0b;font-weight:600;">'+cardScore+'</span>';}}
