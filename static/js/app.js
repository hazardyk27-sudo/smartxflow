...
window._chartLibsLoading=true;window._chartLibsRetry=false;var scripts=['https://cdn.jsdelivr.net/npm/chart.js','https://cdn.jsdelivr.net/npm/hammerjs@2.0.8','https://cdn.jsdelivr.net/npm/chartjs-plugin-zoom@2.0.1/dist/chartjs-plugin-zoom.min.js','https://cdn.jsdelivr.net/npm/html2canvas@1.4.1/dist/html2canvas.min.js'];var loaded=0;var failed=[];function loadNext(){if(loaded>=scripts.length){if(typeof Chart!=='undefined'){window._chartLibsLoaded=true;}else{console.error('[ChartLibs] Chart.js failed to load. Failed scripts:',failed);window._chartLibsLoaded=false;window._chartLibsLoading=false;window._chartLibsRetry=true;}
resolve();return;}
var s=document.createElement('script');s.src=scripts[loaded];s.onload=function(){loaded++;loadNext();};s.onerror=function(){failed.push(scripts[loaded]);loaded++;loadNext();};document.head.appendChild(s);}
loadNext();});};let currentMarket='moneyway_1x2';let matches=[];let filteredMatches=[];let chart=null;let selectedMatch=null;let selectedChartMarket='moneyway_1x2';let autoScrapeRunning=false;let currentSortColumn='volume';let currentSortDirection='desc';let chartVisibleSeries={};let dateFilterMode='ALL';let chartTimeRange='10min';let currentChartHistoryData=[];let chartViewMode='percent';let isClientMode=true;let _modalRequestId=0;let _analysisMatchHashes=[];let _mobileHideEnded=false;let _mobileHideLive=false;let _mobileOnlyLive=false;let _analysisHashesFetched=false;let _licenseReadyResolve;const _licenseReady=new Promise(r=>{_licenseReadyResolve=r;});let _isLicensed=false;function getWebDeviceId(){let did=localStorage.getItem('smartxflow_device_id');if(!did||did.length>16){did='w-'+crypto.randomUUID().replace(/-/g,'').substring(0,14);localStorage.setItem('smartxflow_device_id',did);}
return did;}
...
let apiUrl;if(dateFilterMode==='ALL'){apiUrl=`/api/matches?market=${requestMarket}&bulk=1`;}else if(dateFilterMode==='YESTERDAY'){apiUrl=`/api/matches?market=${requestMarket}&date_filter=yesterday&bulk=1`;}else if(dateFilterMode==='TODAY'){apiUrl=`/api/matches?market=${requestMarket}&date_filter=today&bulk=1`;}else{apiUrl=`/api/matches?market=${requestMarket}&bulk=1`;}
console.log('[Matches] Loading with bulk mode:',apiUrl);const startTime=performance.now();const response=await fetch(apiUrl);const result=await response.json();const elapsed=Math.round(performance.now()-startTime);console.log(`[Matches] Loaded ${result.matches?.length || 0} matches in ${elapsed}ms`);if(currentMarket!==requestMarket){console.log(`[Matches] Race condition detected: request was for ${requestMarket} but current market is ${currentMarket}. Triggering fresh load.`);setTimeout(()=>loadMatches(),0);return;}
...
function mfToggle(el){var val=el.getAttribute('data-value');if(val==='onlyLive'&&!el.classList.contains('active')){el.closest('.mf-chips').querySelectorAll('.mf-chip[data-value="hideLive"]').forEach(function(c){c.classList.remove('active');});}
if(val==='hideLive'&&!el.classList.contains('active')){el.closest('.mf-chips').querySelectorAll('.mf-chip[data-value="onlyLive"]').forEach(function(c){c.classList.remove('active');});}
el.classList.toggle('active');}
function applyMobileFilter(){var dateChip=document.querySelector('.mf-chip[data-group="date"].active');var sortChip=document.querySelector('.mf-chip[data-group="sort"].active');var dateVal=dateChip?dateChip.getAttribute('data-value'):'all';var sortVal=sortChip?sortChip.getAttribute('data-value'):'date';_mobileHideEnded=!!document.querySelector('.mf-chip[data-value="hideEnded"].active');_mobileHideLive=!!document.querySelector('.mf-chip[data-value="hideLive"].active');_mobileOnlyLive=!!document.querySelector('.mf-chip[data-value="onlyLive"].active');var btn=document.getElementById('mobileFilterBtn');if(btn){var hasFilter=(dateVal!=='all')||_mobileHideEnded||_mobileHideLive||_mobileOnlyLive||(sortVal!=='date');btn.classList.toggle('has-filter',hasFilter);}
...
updateTrendSortButtons();updateTableHeaders();if(currentDayFilter===dateVal){filteredMatches=applySorting(matches);renderMatches(filteredMatches);}else{setDayFilter(dateVal);}}
...
document.getElementById('modalLeague').textContent=`${selectedMatch.league || ''} • ${headerDateText}`;document.querySelectorAll('#modalChartTabs .chart-tab').forEach(t=>{t.classList.remove('active');if(t.dataset.market===currentMarket){t.classList.add('active');}});document.getElementById('modalOverlay').classList.add('active');const home=selectedMatch.home_team;const away=selectedMatch.away_team;const league=selectedMatch.league||'';const chartLibsPromise=window.loadChartLibs?window.loadChartLibs().then(()=>registerChartPlugins()):Promise.resolve();const alarmsPromise=fetchAlarmsBatch();const marketsPromise=loadAllMarketsAtOnce(home,away,league);const chartPipeline=Promise.all([marketsPromise,chartLibsPromise]).then(()=>{if(reqId!==_modalRequestId)return;return loadChartWithTrends(home,away,selectedChartMarket,league,reqId);}).catch(e=>console.error('[Modal] Chart pipeline error:',e));const alarmsPipeline=alarmsPromise.then(()=>{if(reqId!==_modalRequestId)return;return renderMatchAlarmsSection(home,away,reqId);}).catch(e=>console.error('[Modal] Alarms pipeline error:',e));await Promise.all([chartPipeline,alarmsPipeline]);}}
...
const dropdown=document.getElementById('alarmDateDropdown');const options=document.getElementById('alarmDateOptions');if(dropdown)dropdown.classList.remove('open');if(options)options.style.display='none';alarmsDisplayCount=30;renderAlarmsList(currentAlarmFilter);}
...
filterAlarms(type);}
document.addEventListener('click',function(e){const dropdown=document.getElementById('alarmFilterDropdown');if(dropdown&&!dropdown.contains(e.target)){dropdown.classList.remove('open');}});function filterAlarms(type){currentAlarmFilter=type;alarmsDisplayCount=30;document.querySelectorAll('.alarm-pill').forEach(btn=>{btn.classList.toggle('active',btn.dataset.type===type);});renderAlarmsList(type);}
function searchAlarms(query){alarmSearchQuery=query.toLowerCase().trim();alarmsDisplayCount=30;renderAlarmsList(currentAlarmFilter);}
...
function loadMoreAlarms(){alarmsDisplayCount+=30;renderAlarmsList(currentAlarmFilter);const container=document.getElementById('alarmsList');const cards=container.querySelectorAll('.alarm-card');if(cards.length>0){cards[cards.length-30]?.scrollIntoView({behavior:'smooth',block:'start'});}}
...
await loadAllAlarmsOnce(false);if(reqId&&reqId!==_modalRequestId)return;const matchAlarms=getMatchAlarms(homeTeam,awayTeam);hideSmartMoneyLoading();smartMoneySectionOpen=true;chevron.textContent='▼';grid.style.display='grid';if(matchAlarms.length===0){grid.style.display='none';empty.style.display='block';return;}
...
const savedKey=localStorage.getItem(WEB_LICENSE_KEY);if(savedKey){try{const resp=await fetch('/api/licenses/validate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({key:savedKey,device_id:getWebDeviceId(),device_name:navigator.userAgent.substring(0,50)})});const data=await resp.json();if(data.valid&&data.days_left!==undefined){localStorage.setItem('license_days_remaining',data.days_left);updateLicenseDaysBadge(data.days_left);}
if(data.plan){window.userPlan=data.plan;localStorage.setItem('license_plan',data.plan);}}catch(e){}}
_isLicensed=true;_licenseReadyResolve();startLicenseStatusRefresh();}}
...
document.addEventListener('DOMContentLoaded',function(){initLicenseCheck();});document.getElementById('licenseKeyInput')?.addEventListener('keypress',function(e){if(e.key==='Enter')validateWebLicense();});