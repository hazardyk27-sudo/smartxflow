...
function renderChartLegendFilters(datasets,market){const container=document.getElementById('chartLegendFilters');if(!container)return;const legendLabels={'moneyway_1x2':[{key:'1',label:'1',color:'#3b82f6'},{key:'X',label:'X',color:'#22c55e'},{key:'2',label:'2',color:'#eab308'}],'moneyway_ou25':[{key:'Under',label:'Alt 2.5',color:'#3b82f6'},{key:'Over',label:'Üst 2.5',color:'#22c55e'}],'moneyway_btts':[{key:'Yes',label:'KG Evet',color:'#22c55e'},{key:'No',label:'KG Hayır',color:'#ef4444'}],'dropping_1x2':[{key:'1',label:'1',color:'#3b82f6'},{key:'X',label:'X',color:'#22c55e'},{key:'2',label:'2',color:'#eab308'}],'dropping_ou25':[{key:'Under',label:'Alt 2.5',color:'#3b82f6'},{key:'Over',label:'Üst 2.5',color:'#22c55e'}],'dropping_btts':[{key:'Yes',label:'KG Evet',color:'#22c55e'},{key:'No',label:'KG Hayır',color:'#ef4444'}]};const items=legendLabels[market]||[];const timeRanges=[{key:'10min',label:'10 dk'},{key:'30min',label:'30 dk'},{key:'1hour',label:'1 saat'},{key:'6hour',label:'6 saat'},{key:'12hour',label:'12 saat'},{key:'1day',label:'1 gün'}];const seriesButtons=items.map(item=>{const stateKey=`${market}_${item.key}`;const isActive=chartVisibleSeries[stateKey]!==false;const activeClass=isActive?'active':'inactive';return`
    ...
    `:'';container.innerHTML=`
        ...
        <div class="chart-export-btns">
            ...
            <button class="chart-export-btn" onclick="exportFullMatchCSV()" title="Tüm market ve alarm verilerini indir" style="background:rgba(52,211,153,0.08);border-color:rgba(52,211,153,0.15);">
...
let alarmsData={};if(matchId){try{const resp=await fetch(`/api/match/${matchId}/snapshot?include=alarms`);if(resp.ok){const data=await resp.json();alarmsData=data.alarms||{};}}catch(e){console.error('[FullCSV] Alarms fetch error:',e);}}
function csvVal(v){if(v===null||v===undefined||v==='')return'';var s=String(v);if(s.includes(',')||s.includes('"')||s.includes('\n'))return'"'+s.replace(/"/g,'""')+'"';return s;}
var csv='\uFEFF';csv+='# SmartXFlow - Full Match Data Export\n';csv+='# Match: '+home+' vs '+away+'\n';csv+='# League: '+league+'\n';csv+='# Match Date: '+matchDate+'\n';csv+='# Match ID: '+matchId+'\n';csv+='# Export Date: '+nowTurkey().format('DD.MM.YYYY HH:mm')+' (TR)\n';csv+='#\n';var marketDefs=[{key:'moneyway_1x2',label:'Moneyway 1X2',cols:['ScrapedAt','Odds1','OddsX','Odds2','Pct1','PctX','Pct2','Amt1','AmtX','Amt2','Volume','Odds1_prev','OddsX_prev','Odds2_prev','Trend1','TrendX','Trend2','DropPct1','DropPctX','DropPct2']},{key:'moneyway_ou25',label:'Moneyway O/U 2.5',cols:['ScrapedAt','Over','Under','PctOver','PctUnder','AmtOver','AmtUnder','Volume','Line','Over_prev','Under_prev','TrendOver','TrendUnder','DropPctOver','DropPctUnder']},{key:'moneyway_btts',label:'Moneyway BTTS',cols:['ScrapedAt','OddsYes','OddsNo','PctYes','PctNo','AmtYes','AmtNo','Volume','OddsYes_prev','OddsNo_prev','TrendYes','TrendNo','DropPctYes','DropPctNo']},{key:'dropping_1x2',label:'Dropping Odds 1X2',cols:['ScrapedAt','Odds1','OddsX','Odds2','Odds1_prev','OddsX_prev','Odds2_prev','Trend1','TrendX','Trend2','DropPct1','DropPctX','DropPct2','Volume']},{key:'dropping_ou25',label:'Dropping Odds O/U 2.5',cols:['ScrapedAt','Over','Under','Over_prev','Under_prev','TrendOver','TrendUnder','DropPctOver','DropPctUnder','Volume','Line']},{key:'dropping_btts',label:'Dropping Odds BTTS',cols:['ScrapedAt','OddsYes','OddsNo','OddsYes_prev','OddsNo_prev','TrendYes','TrendNo','DropPctYes','DropPctNo','Volume']}];csv+='#\n# ===== SNAPSHOT DATA =====\n';marketDefs.forEach(function(md){var mdata=marketsData[md.key];var rows=mdata?(mdata.history||[]):[];csv+='#\n# --- '+md.label+' ('+rows.length+' snapshots) ---\n';if(rows.length>0){csv+=md.cols.map(csvVal).join(',')+'\n';rows.forEach(function(row){csv+=md.cols.map(function(col){return csvVal(row[col]);}).join(',')+'\n';});}});csv+='#\n# ===== ALARM DATA =====\n';var alarmTypes=['sharp','bigmoney','volumeshock','dropping','volumeleader','mim'];var hasAlarms=false;alarmTypes.forEach(function(atype){var alarms=alarmsData[atype]||[];if(alarms.length>0)hasAlarms=true;});if(hasAlarms){csv+='alarm_type,timestamp,market,side,odds,volume,details\n';alarmTypes.forEach(function(atype){var alarms=alarmsData[atype]||[];alarms.forEach(function(a){var ts=a.trigger_at||a.triggered_at||a.created_at||a.scraped_at||'';var market=a.market||'';var side=a.selection||a.side||'';var odds=a.odds||a.current_odds||'';var vol=a.volume||a.total_volume||a.amount||'';var details='';if(atype==='sharp')details='score='+(a.sharp_score||a.score||'');else if(atype==='bigmoney')details='amount='+(a.incoming_amount||a.amount||'');else if(atype==='volumeshock')details='shock='+(a.shock_multiplier||a.shock||'')+'x';else if(atype==='dropping')details='drop='+(a.drop_pct||a.drop_percent||'')+'%';else if(atype==='volumeleader')details='from='+(a.from_side||'')+',to='+(a.to_side||'')+',pct='+(a.to_pct||'');else if(atype==='mim')details='impact='+(a.impact_score||a.impact||'');csv+=[atype,csvVal(ts),csvVal(market),csvVal(side),csvVal(odds),csvVal(vol),csvVal(details)].join(',')+'\n';});});}else{csv+='# No alarm data available for this match\n';}
var blob=new Blob([csv],{type:'text/csv;charset=utf-8;'});var link=document.createElement('a');link.href=URL.createObjectURL(blob);var safeName=(home+'_vs_'+away).replace(/[^a-zA-Z0-9_\-]/g,'_');link.download=safeName+'_full_data.csv';link.click();URL.revokeObjectURL(link.href);}
