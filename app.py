...
def _get_matchbook_matches_bulk(market, date_filter, force_refresh, start_time, t):
    cache_key = f"mb_{market}_{date_filter or 'all'}"
    cached_data, from_cache = get_cached_matches(cache_key, force_refresh)
    
    if cached_data:
        elapsed = (t.time() - start_time) * 1000
        print(f"[Matches/MB] Cache HIT for {market} - {elapsed:.0f}ms, {len(cached_data)} matches")
        return jsonify({'matches': cached_data, 'total': len(cached_data), 'has_more': False, 'source': 'matchbook'})
    
    is_dropping = market.startswith('dropping')
    mb_matches = db.get_matchbook_matches_with_latest(market, date_filter=date_filter)
    ...
    for m in mb_matches:
        ...
        if latest:
            if market in ['moneyway_1x2', 'dropping_1x2']:
                odds = {
                    'Odds1': latest.get('Odds1', '-'),
                    'OddsX': latest.get('OddsX', '-'),
                    'Odds2': latest.get('Odds2', '-'),
                    'Pct1': latest.get('Pct1', ''),
                    'Amt1': latest.get('Amt1', ''),
                    'PctX': latest.get('PctX', ''),
                    'AmtX': latest.get('AmtX', ''),
                ...
                odds = {
                    'OddsYes': latest.get('OddsYes', '-'),
                    'OddsNo': latest.get('OddsNo', '-'),
                    'PctYes': latest.get('PctYes', ''),
                    'AmtYes': latest.get('AmtYes', ''),
                    'PctNo': latest.get('PctNo', ''),
                    'AmtNo': latest.get('AmtNo', ''),
        ...
        all_matches.append({
            ...
            'arbworld_hash': m.get('arbworld_hash', ''),
            'odds': {**odds, **prev_odds},
            'history_count': 1,
            'source': 'matchbook'
        })
    ...
    elapsed = (t.time() - start_time) * 1000
    print(f"[Matches/MB] Cache MISS for {market} - fetched {len(all_matches)} matches in {elapsed:.0f}ms")
    
    return jsonify({'matches': all_matches, 'total': len(all_matches), 'has_more': False, 'source': 'matchbook'})
...
@app.route('/api/match/history/matchbook')
@license_required
def get_matchbook_history_bulk():
    home = request.args.get('home', '')
    away = request.args.get('away', '')
    league = request.args.get('league', '')
    ...
    def _build_mb_market_data(market_name):
        ...
        if history:
            ...
            if market_name in ['moneyway_1x2', 'dropping_1x2']:
                for idx, (key, color) in enumerate([('Odds1', '#f97316'), ('OddsX', '#a855f7'), ('Odds2', '#06b6d4')]):
                    alt_key = ['1', 'X', '2'][idx]
                    values = []
                    for h in history:
                        val = h.get(key, h.get(alt_key, ''))
            ...
            elif market_name in ['moneyway_ou25', 'dropping_ou25']:
                for key, color, label in [('Under', '#06b6d4', 'Under'), ('Over', '#f97316', 'Over')]:
                    values = []
                    for h in history:
                        val = h.get(key, '')
                        try:
        ...
        return market_name, {'history': history, 'chart_data': chart_data}
    
    from concurrent.futures import ThreadPoolExecutor
    result = {}
    with ThreadPoolExecutor(max_workers=6) as executor:
        futures = {executor.submit(_build_mb_market_data, m): m for m in all_markets}
        for future in futures:
            market_name, market_data = future.result()
            result[market_name] = market_data
    ...
    elapsed = int((time.time() - start_time) * 1000)
    total_history = sum(len(v.get('history') or []) for v in result.values())
    print(f"[History/MB] Fetched matchbook history in {elapsed}ms, {total_history} rows")
    
    return jsonify({'markets': result, 'source': 'matchbook'})
